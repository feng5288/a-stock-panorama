const CLASSIFICATION_210_QUALITY_DIAGNOSIS_V1 = {
  "generated_at": "2026-06-27 12:59:59",
  "source": "C:\\Users\\LYDN\\a-stock-panorama\\classification_v2_next_integrated.js",
  "rules_version": "2026-06-26.alpha2",
  "summary": {
    "total": 210,
    "high": 0,
    "medium": 57,
    "low": 153,
    "ok": 0,
    "p0": 80,
    "next44": 44,
    "existing86": 86,
    "existing_only": 0,
    "parent_only": 0,
    "possible_mismatch": 0,
    "leaf_without_business_support": 0,
    "missing_evidence_card": 0,
    "excessive_mounts": 0,
    "cross_root_overload": 12,
    "financial_weak": 37
  },
  "by_bucket": [
    {
      "bucket": "地产 / 建筑建材",
      "total": 11,
      "high": 0,
      "medium": 6,
      "missing_evidence": 0,
      "avg_theme": 94.2
    },
    {
      "bucket": "计算机 / 软件服务",
      "total": 11,
      "high": 0,
      "medium": 5,
      "missing_evidence": 0,
      "avg_theme": 91.4
    },
    {
      "bucket": "AI算力产业链",
      "total": 8,
      "high": 0,
      "medium": 5,
      "missing_evidence": 0,
      "avg_theme": 78.4
    },
    {
      "bucket": "医药医疗",
      "total": 26,
      "high": 0,
      "medium": 4,
      "missing_evidence": 0,
      "avg_theme": 89.5
    },
    {
      "bucket": "电力与公用事业",
      "total": 15,
      "high": 0,
      "medium": 4,
      "missing_evidence": 0,
      "avg_theme": 89.1
    },
    {
      "bucket": "传媒互联网",
      "total": 8,
      "high": 0,
      "medium": 4,
      "missing_evidence": 0,
      "avg_theme": 93.5
    },
    {
      "bucket": "农业养殖",
      "total": 8,
      "high": 0,
      "medium": 4,
      "missing_evidence": 0,
      "avg_theme": 94.0
    },
    {
      "bucket": "高端装备与工业",
      "total": 5,
      "high": 0,
      "medium": 4,
      "missing_evidence": 0,
      "avg_theme": 84.6
    },
    {
      "bucket": "国防军工与航空航天",
      "total": 4,
      "high": 0,
      "medium": 4,
      "missing_evidence": 0,
      "avg_theme": 77.5
    },
    {
      "bucket": "消费电子与终端",
      "total": 4,
      "high": 0,
      "medium": 3,
      "missing_evidence": 0,
      "avg_theme": 89.8
    },
    {
      "bucket": "资源周期",
      "total": 9,
      "high": 0,
      "medium": 2,
      "missing_evidence": 0,
      "avg_theme": 90.7
    },
    {
      "bucket": "商贸零售 / 社服",
      "total": 8,
      "high": 0,
      "medium": 2,
      "missing_evidence": 0,
      "avg_theme": 91.8
    },
    {
      "bucket": "消费白马",
      "total": 4,
      "high": 0,
      "medium": 2,
      "missing_evidence": 0,
      "avg_theme": 82.5
    },
    {
      "bucket": "人形机器人产业链",
      "total": 2,
      "high": 0,
      "medium": 2,
      "missing_evidence": 0,
      "avg_theme": 72.0
    },
    {
      "bucket": "半导体产业链",
      "total": 10,
      "high": 0,
      "medium": 1,
      "missing_evidence": 0,
      "avg_theme": 90.8
    },
    {
      "bucket": "纺服轻工",
      "total": 9,
      "high": 0,
      "medium": 1,
      "missing_evidence": 0,
      "avg_theme": 94.1
    },
    {
      "bucket": "化工材料",
      "total": 7,
      "high": 0,
      "medium": 1,
      "missing_evidence": 0,
      "avg_theme": 88.6
    },
    {
      "bucket": "半导体",
      "total": 5,
      "high": 0,
      "medium": 1,
      "missing_evidence": 0,
      "avg_theme": 89.2
    },
    {
      "bucket": "新能源发电",
      "total": 1,
      "high": 0,
      "medium": 1,
      "missing_evidence": 0,
      "avg_theme": 85.0
    },
    {
      "bucket": "有色金属与新材料",
      "total": 1,
      "high": 0,
      "medium": 1,
      "missing_evidence": 0,
      "avg_theme": 86.0
    },
    {
      "bucket": "交运物流",
      "total": 18,
      "high": 0,
      "medium": 0,
      "missing_evidence": 0,
      "avg_theme": 93.6
    },
    {
      "bucket": "环保 / 水务固废",
      "total": 10,
      "high": 0,
      "medium": 0,
      "missing_evidence": 0,
      "avg_theme": 93.6
    },
    {
      "bucket": "金融",
      "total": 7,
      "high": 0,
      "medium": 0,
      "missing_evidence": 0,
      "avg_theme": 91.7
    },
    {
      "bucket": "新能源汽车",
      "total": 5,
      "high": 0,
      "medium": 0,
      "missing_evidence": 0,
      "avg_theme": 94.8
    },
    {
      "bucket": "AI算力",
      "total": 4,
      "high": 0,
      "medium": 0,
      "missing_evidence": 0,
      "avg_theme": 92.0
    },
    {
      "bucket": "机器人 / 自动化",
      "total": 3,
      "high": 0,
      "medium": 0,
      "missing_evidence": 0,
      "avg_theme": 89.0
    },
    {
      "bucket": "消费电子",
      "total": 3,
      "high": 0,
      "medium": 0,
      "missing_evidence": 0,
      "avg_theme": 93.7
    },
    {
      "bucket": "新能源产业链",
      "total": 2,
      "high": 0,
      "medium": 0,
      "missing_evidence": 0,
      "avg_theme": 95.0
    },
    {
      "bucket": "新能源汽车产业链",
      "total": 2,
      "high": 0,
      "medium": 0,
      "missing_evidence": 0,
      "avg_theme": 94.5
    }
  ],
  "method_note": "210家并行分类树质量诊断：检查叶子节点挂载、主营/产品证据命中、证据卡覆盖、重复挂载、跨主线和财务弱项；用于修规则和安排下一轮轻量证据卡，不作为投资评级。",
  "rows": [
    {
      "code": "300308",
      "name": "中际旭创",
      "batch": "EXISTING86",
      "primary_industry": "AI算力产业链",
      "primary_leaf": "光模块/CPO",
      "mount_count": 3,
      "root_count": 2,
      "leaf_count": 2,
      "severity": "medium",
      "diagnosis_label": "主题分偏低 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "theme_low",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡主题纯度低于 75，需要观察是否为边缘概念。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 72.0,
      "fundamental_score": 95.0,
      "net_profit_yi": 63.17,
      "net_profit_yoy_pct": 2.62,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "AI算力产业链",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "ai_compute",
          "node_name": "AI算力",
          "path": [
            "AI算力"
          ]
        },
        {
          "name": "光模块",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "optical_module",
          "node_name": "光模块/CPO",
          "path": [
            "AI算力",
            "光模块/CPO"
          ]
        },
        {
          "name": "CPO",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "optical_module",
          "node_name": "光模块/CPO",
          "path": [
            "AI算力",
            "光模块/CPO"
          ]
        },
        {
          "name": "高速光通信",
          "role": "二级赛道",
          "weight": 0.78,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "optical_module",
          "node_name": "光模块/CPO",
          "path": [
            "AI算力",
            "光模块/CPO"
          ]
        },
        {
          "name": "AI",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "ai_compute",
          "node_name": "AI算力",
          "path": [
            "AI算力"
          ]
        },
        {
          "name": "算力",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "ai_compute",
          "node_name": "AI算力",
          "path": [
            "AI算力"
          ]
        },
        {
          "name": "数据中心",
          "role": "相关概念",
          "weight": 0.77,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "ai_server",
          "node_name": "AI服务器链",
          "path": [
            "AI算力",
            "AI服务器链"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "光模块",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "optical_module",
          "node_name": "光模块/CPO",
          "path": [
            "AI算力",
            "光模块/CPO"
          ]
        },
        {
          "name": "CPO",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "optical_module",
          "node_name": "光模块/CPO",
          "path": [
            "AI算力",
            "光模块/CPO"
          ]
        },
        {
          "name": "高速光通信",
          "role": "二级赛道",
          "weight": 0.78,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "optical_module",
          "node_name": "光模块/CPO",
          "path": [
            "AI算力",
            "光模块/CPO"
          ]
        },
        {
          "name": "数据中心",
          "role": "相关概念",
          "weight": 0.77,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "ai_server",
          "node_name": "AI服务器链",
          "path": [
            "AI算力",
            "AI服务器链"
          ]
        }
      ]
    },
    {
      "code": "601138",
      "name": "工业富联",
      "batch": "EXISTING86",
      "primary_industry": "AI算力产业链",
      "primary_leaf": "AI服务器链",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "主题分偏低 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "theme_low",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡主题纯度低于 75，需要观察是否为边缘概念。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 72.0,
      "fundamental_score": 95.0,
      "net_profit_yi": 105.97,
      "net_profit_yoy_pct": 1.03,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "AI算力产业链",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "ai_compute",
          "node_name": "AI算力",
          "path": [
            "AI算力"
          ]
        },
        {
          "name": "AI服务器",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "ai_server",
          "node_name": "AI服务器链",
          "path": [
            "AI算力",
            "AI服务器链"
          ]
        },
        {
          "name": "算力",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "ai_compute",
          "node_name": "AI算力",
          "path": [
            "AI算力"
          ]
        },
        {
          "name": "云端硬件制造",
          "role": "二级赛道",
          "weight": 0.78,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "ai_server",
          "node_name": "AI服务器链",
          "path": [
            "AI算力",
            "AI服务器链"
          ]
        },
        {
          "name": "AI",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "ai_compute",
          "node_name": "AI算力",
          "path": [
            "AI算力"
          ]
        },
        {
          "name": "服务器",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "ai_server",
          "node_name": "AI服务器链",
          "path": [
            "AI算力",
            "AI服务器链"
          ]
        },
        {
          "name": "云计算",
          "role": "相关概念",
          "weight": 0.67,
          "evidence_level": "中",
          "basis": "相关标签",
          "node_id": "ai_compute",
          "node_name": "AI算力",
          "path": [
            "AI算力"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "AI服务器",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "ai_server",
          "node_name": "AI服务器链",
          "path": [
            "AI算力",
            "AI服务器链"
          ]
        },
        {
          "name": "云端硬件制造",
          "role": "二级赛道",
          "weight": 0.78,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "ai_server",
          "node_name": "AI服务器链",
          "path": [
            "AI算力",
            "AI服务器链"
          ]
        },
        {
          "name": "服务器",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "ai_server",
          "node_name": "AI服务器链",
          "path": [
            "AI算力",
            "AI服务器链"
          ]
        }
      ]
    },
    {
      "code": "300502",
      "name": "新易盛",
      "batch": "EXISTING86",
      "primary_industry": "AI算力产业链",
      "primary_leaf": "光模块/CPO",
      "mount_count": 3,
      "root_count": 2,
      "leaf_count": 2,
      "severity": "medium",
      "diagnosis_label": "主题分偏低 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "theme_low",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡主题纯度低于 75，需要观察是否为边缘概念。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 72.0,
      "fundamental_score": 95.0,
      "net_profit_yi": 27.74,
      "net_profit_yoy_pct": 0.77,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "AI算力产业链",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "ai_compute",
          "node_name": "AI算力",
          "path": [
            "AI算力"
          ]
        },
        {
          "name": "光模块",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "optical_module",
          "node_name": "光模块/CPO",
          "path": [
            "AI算力",
            "光模块/CPO"
          ]
        },
        {
          "name": "CPO",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "optical_module",
          "node_name": "光模块/CPO",
          "path": [
            "AI算力",
            "光模块/CPO"
          ]
        },
        {
          "name": "高速光通信",
          "role": "二级赛道",
          "weight": 0.78,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "optical_module",
          "node_name": "光模块/CPO",
          "path": [
            "AI算力",
            "光模块/CPO"
          ]
        },
        {
          "name": "AI",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "ai_compute",
          "node_name": "AI算力",
          "path": [
            "AI算力"
          ]
        },
        {
          "name": "算力",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "ai_compute",
          "node_name": "AI算力",
          "path": [
            "AI算力"
          ]
        },
        {
          "name": "数据中心",
          "role": "相关概念",
          "weight": 0.77,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "ai_server",
          "node_name": "AI服务器链",
          "path": [
            "AI算力",
            "AI服务器链"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "光模块",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "optical_module",
          "node_name": "光模块/CPO",
          "path": [
            "AI算力",
            "光模块/CPO"
          ]
        },
        {
          "name": "CPO",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "optical_module",
          "node_name": "光模块/CPO",
          "path": [
            "AI算力",
            "光模块/CPO"
          ]
        },
        {
          "name": "高速光通信",
          "role": "二级赛道",
          "weight": 0.78,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "optical_module",
          "node_name": "光模块/CPO",
          "path": [
            "AI算力",
            "光模块/CPO"
          ]
        },
        {
          "name": "数据中心",
          "role": "相关概念",
          "weight": 0.77,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "ai_server",
          "node_name": "AI服务器链",
          "path": [
            "AI算力",
            "AI服务器链"
          ]
        }
      ]
    },
    {
      "code": "688041",
      "name": "海光信息",
      "batch": "EXISTING86",
      "primary_industry": "AI算力产业链",
      "primary_leaf": "AI芯片/国产算力芯片",
      "mount_count": 4,
      "root_count": 2,
      "leaf_count": 2,
      "severity": "medium",
      "diagnosis_label": "主题分偏低 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "theme_low",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡主题纯度低于 75，需要观察是否为边缘概念。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 72.0,
      "fundamental_score": 90.0,
      "net_profit_yi": 8.77,
      "net_profit_yoy_pct": 0.36,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "AI算力产业链",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "ai_compute",
          "node_name": "AI算力",
          "path": [
            "AI算力"
          ]
        },
        {
          "name": "国产CPU",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "ai_chip",
          "node_name": "AI芯片/国产算力芯片",
          "path": [
            "AI算力",
            "AI芯片/国产算力芯片"
          ]
        },
        {
          "name": "GPU",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "ai_chip",
          "node_name": "AI芯片/国产算力芯片",
          "path": [
            "AI算力",
            "AI芯片/国产算力芯片"
          ]
        },
        {
          "name": "服务器芯片",
          "role": "二级赛道",
          "weight": 0.78,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "ai_server",
          "node_name": "AI服务器链",
          "path": [
            "AI算力",
            "AI服务器链"
          ]
        },
        {
          "name": "AI",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "ai_compute",
          "node_name": "AI算力",
          "path": [
            "AI算力"
          ]
        },
        {
          "name": "半导体",
          "role": "相关概念",
          "weight": 0.67,
          "evidence_level": "中",
          "basis": "相关标签",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "国产替代",
          "role": "相关概念",
          "weight": 0.61,
          "evidence_level": "需复核",
          "basis": "主题标签",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "算力",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "ai_compute",
          "node_name": "AI算力",
          "path": [
            "AI算力"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "国产CPU",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "ai_chip",
          "node_name": "AI芯片/国产算力芯片",
          "path": [
            "AI算力",
            "AI芯片/国产算力芯片"
          ]
        },
        {
          "name": "GPU",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "ai_chip",
          "node_name": "AI芯片/国产算力芯片",
          "path": [
            "AI算力",
            "AI芯片/国产算力芯片"
          ]
        },
        {
          "name": "服务器芯片",
          "role": "二级赛道",
          "weight": 0.78,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "ai_server",
          "node_name": "AI服务器链",
          "path": [
            "AI算力",
            "AI服务器链"
          ]
        }
      ]
    },
    {
      "code": "002851",
      "name": "麦格米特",
      "batch": "EXISTING86",
      "primary_industry": "AI算力产业链",
      "primary_leaf": "AI服务器链",
      "mount_count": 4,
      "root_count": 3,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "跨主线过多 / 主题分偏低 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "cross_root_overload",
        "theme_low",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前跨 3 条主线，可能需要压缩非核心标签。",
        "轻量证据卡主题纯度低于 75，需要观察是否为边缘概念。"
      ],
      "recommended_action": "压缩弱标签，只保留主业和强证据概念进入叶子节点。",
      "theme_purity_score": 72.0,
      "fundamental_score": 85.0,
      "net_profit_yi": 1.28,
      "net_profit_yoy_pct": 0.07,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "AI算力产业链",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "ai_compute",
          "node_name": "AI算力",
          "path": [
            "AI算力"
          ]
        },
        {
          "name": "AI服务器",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "ai_server",
          "node_name": "AI服务器链",
          "path": [
            "AI算力",
            "AI服务器链"
          ]
        },
        {
          "name": "算力",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "ai_compute",
          "node_name": "AI算力",
          "path": [
            "AI算力"
          ]
        },
        {
          "name": "AI服务器电源",
          "role": "二级赛道",
          "weight": 0.78,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "ai_server",
          "node_name": "AI服务器链",
          "path": [
            "AI算力",
            "AI服务器链"
          ]
        },
        {
          "name": "AI",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "ai_compute",
          "node_name": "AI算力",
          "path": [
            "AI算力"
          ]
        },
        {
          "name": "半导体",
          "role": "相关概念",
          "weight": 0.65,
          "evidence_level": "中",
          "basis": "相关标签",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "新能源",
          "role": "相关概念",
          "weight": 0.65,
          "evidence_level": "中",
          "basis": "相关标签",
          "node_id": "renewable_power",
          "node_name": "电力与公用事业",
          "path": [
            "电力与公用事业"
          ]
        },
        {
          "name": "电源",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "ai_server",
          "node_name": "AI服务器链",
          "path": [
            "AI算力",
            "AI服务器链"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "AI服务器",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "ai_server",
          "node_name": "AI服务器链",
          "path": [
            "AI算力",
            "AI服务器链"
          ]
        },
        {
          "name": "AI服务器电源",
          "role": "二级赛道",
          "weight": 0.78,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "ai_server",
          "node_name": "AI服务器链",
          "path": [
            "AI算力",
            "AI服务器链"
          ]
        },
        {
          "name": "电源",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "ai_server",
          "node_name": "AI服务器链",
          "path": [
            "AI算力",
            "AI服务器链"
          ]
        }
      ]
    },
    {
      "code": "002050",
      "name": "三花智控",
      "batch": "EXISTING86",
      "primary_industry": "人形机器人产业链",
      "primary_leaf": "执行器",
      "mount_count": 5,
      "root_count": 3,
      "leaf_count": 3,
      "severity": "medium",
      "diagnosis_label": "跨主线过多 / 主题分偏低 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "cross_root_overload",
        "theme_low",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前跨 3 条主线，可能需要压缩非核心标签。",
        "轻量证据卡主题纯度低于 75，需要观察是否为边缘概念。"
      ],
      "recommended_action": "压缩弱标签，只保留主业和强证据概念进入叶子节点。",
      "theme_purity_score": 72.0,
      "fundamental_score": 94.0,
      "net_profit_yi": 9.51,
      "net_profit_yoy_pct": 0.03,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "人形机器人产业链",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "robotics",
          "node_name": "机器人/自动化",
          "path": [
            "机器人/自动化"
          ]
        },
        {
          "name": "减速器",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "robot_actuator_formal",
          "node_name": "执行器",
          "path": [
            "机器人/自动化",
            "执行器"
          ]
        },
        {
          "name": "丝杠",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "robot_actuator_formal",
          "node_name": "执行器",
          "path": [
            "机器人/自动化",
            "执行器"
          ]
        },
        {
          "name": "执行器",
          "role": "二级赛道",
          "weight": 0.78,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "robot_actuator_formal",
          "node_name": "执行器",
          "path": [
            "机器人/自动化",
            "执行器"
          ]
        },
        {
          "name": "储能",
          "role": "相关概念",
          "weight": 0.62,
          "evidence_level": "中",
          "basis": "相关标签",
          "node_id": "battery_storage",
          "node_name": "动力电池/储能",
          "path": [
            "新能源汽车",
            "动力电池/储能"
          ]
        },
        {
          "name": "新能源",
          "role": "相关概念",
          "weight": 0.62,
          "evidence_level": "中",
          "basis": "相关标签",
          "node_id": "renewable_power",
          "node_name": "电力与公用事业",
          "path": [
            "电力与公用事业"
          ]
        },
        {
          "name": "机器人",
          "role": "核心概念",
          "weight": 0.86,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "robot_humanoid",
          "node_name": "人形机器人",
          "path": [
            "机器人/自动化",
            "人形机器人"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "减速器",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "robot_actuator_formal",
          "node_name": "执行器",
          "path": [
            "机器人/自动化",
            "执行器"
          ]
        },
        {
          "name": "丝杠",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "robot_actuator_formal",
          "node_name": "执行器",
          "path": [
            "机器人/自动化",
            "执行器"
          ]
        },
        {
          "name": "执行器",
          "role": "二级赛道",
          "weight": 0.78,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "robot_actuator_formal",
          "node_name": "执行器",
          "path": [
            "机器人/自动化",
            "执行器"
          ]
        },
        {
          "name": "储能",
          "role": "相关概念",
          "weight": 0.62,
          "evidence_level": "中",
          "basis": "相关标签",
          "node_id": "battery_storage",
          "node_name": "动力电池/储能",
          "path": [
            "新能源汽车",
            "动力电池/储能"
          ]
        },
        {
          "name": "机器人",
          "role": "核心概念",
          "weight": 0.86,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "robot_humanoid",
          "node_name": "人形机器人",
          "path": [
            "机器人/自动化",
            "人形机器人"
          ]
        }
      ]
    },
    {
      "code": "605566",
      "name": "福莱蒽特",
      "batch": "EXISTING86",
      "primary_industry": "人形机器人产业链",
      "primary_leaf": "传感器 / 电子皮肤",
      "mount_count": 3,
      "root_count": 2,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "主题分偏低 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "theme_low",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡主题纯度低于 75，需要观察是否为边缘概念。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 72.0,
      "fundamental_score": 79.0,
      "net_profit_yi": 0.25,
      "net_profit_yoy_pct": 0.83,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "人形机器人产业链",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "robotics",
          "node_name": "机器人/自动化",
          "path": [
            "机器人/自动化"
          ]
        },
        {
          "name": "机器人传感器",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "robot_sensor_formal",
          "node_name": "传感器 / 电子皮肤",
          "path": [
            "机器人/自动化",
            "传感器 / 电子皮肤"
          ]
        },
        {
          "name": "力矩传感器",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "robot_sensor_formal",
          "node_name": "传感器 / 电子皮肤",
          "path": [
            "机器人/自动化",
            "传感器 / 电子皮肤"
          ]
        },
        {
          "name": "电子皮肤",
          "role": "二级赛道",
          "weight": 0.78,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "robot_sensor_formal",
          "node_name": "传感器 / 电子皮肤",
          "path": [
            "机器人/自动化",
            "传感器 / 电子皮肤"
          ]
        },
        {
          "name": "AI",
          "role": "相关概念",
          "weight": 0.61,
          "evidence_level": "需复核",
          "basis": "主题标签",
          "node_id": "ai_compute",
          "node_name": "AI算力",
          "path": [
            "AI算力"
          ]
        },
        {
          "name": "机器人",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "robot_sensor_formal",
          "node_name": "传感器 / 电子皮肤",
          "path": [
            "机器人/自动化",
            "传感器 / 电子皮肤"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "机器人传感器",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "robot_sensor_formal",
          "node_name": "传感器 / 电子皮肤",
          "path": [
            "机器人/自动化",
            "传感器 / 电子皮肤"
          ]
        },
        {
          "name": "力矩传感器",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "robot_sensor_formal",
          "node_name": "传感器 / 电子皮肤",
          "path": [
            "机器人/自动化",
            "传感器 / 电子皮肤"
          ]
        },
        {
          "name": "电子皮肤",
          "role": "二级赛道",
          "weight": 0.78,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "robot_sensor_formal",
          "node_name": "传感器 / 电子皮肤",
          "path": [
            "机器人/自动化",
            "传感器 / 电子皮肤"
          ]
        },
        {
          "name": "机器人",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "robot_sensor_formal",
          "node_name": "传感器 / 电子皮肤",
          "path": [
            "机器人/自动化",
            "传感器 / 电子皮肤"
          ]
        }
      ]
    },
    {
      "code": "600309",
      "name": "万华化学",
      "batch": "EXISTING86",
      "primary_industry": "化工材料",
      "primary_leaf": "新材料化工",
      "mount_count": 4,
      "root_count": 3,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "跨主线过多 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "cross_root_overload",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前跨 3 条主线，可能需要压缩非核心标签。"
      ],
      "recommended_action": "压缩弱标签，只保留主业和强证据概念进入叶子节点。",
      "theme_purity_score": 93.0,
      "fundamental_score": 95.0,
      "net_profit_yi": 43.62,
      "net_profit_yoy_pct": 0.21,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "化工材料",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "chemical_materials",
          "node_name": "化工材料",
          "path": [
            "化工材料"
          ]
        },
        {
          "name": "MDI",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "chemical_new_material",
          "node_name": "新材料化工",
          "path": [
            "化工材料",
            "新材料化工"
          ]
        },
        {
          "name": "聚氨酯",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "chemical_new_material",
          "node_name": "新材料化工",
          "path": [
            "化工材料",
            "新材料化工"
          ]
        },
        {
          "name": "精细化工",
          "role": "二级赛道",
          "weight": 0.78,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "chemical_new_material",
          "node_name": "新材料化工",
          "path": [
            "化工材料",
            "新材料化工"
          ]
        },
        {
          "name": "化工",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "chemical_new_material",
          "node_name": "新材料化工",
          "path": [
            "化工材料",
            "新材料化工"
          ]
        },
        {
          "name": "新材料",
          "role": "相关概念",
          "weight": 0.75,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "advanced_materials",
          "node_name": "有色金属与新材料",
          "path": [
            "有色金属与新材料"
          ]
        },
        {
          "name": "周期",
          "role": "相关概念",
          "weight": 0.61,
          "evidence_level": "中",
          "basis": "相关标签",
          "node_id": "resources",
          "node_name": "资源周期",
          "path": [
            "资源周期"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "MDI",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "chemical_new_material",
          "node_name": "新材料化工",
          "path": [
            "化工材料",
            "新材料化工"
          ]
        },
        {
          "name": "聚氨酯",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "chemical_new_material",
          "node_name": "新材料化工",
          "path": [
            "化工材料",
            "新材料化工"
          ]
        },
        {
          "name": "精细化工",
          "role": "二级赛道",
          "weight": 0.78,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "chemical_new_material",
          "node_name": "新材料化工",
          "path": [
            "化工材料",
            "新材料化工"
          ]
        },
        {
          "name": "化工",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "chemical_new_material",
          "node_name": "新材料化工",
          "path": [
            "化工材料",
            "新材料化工"
          ]
        }
      ]
    },
    {
      "code": "603392",
      "name": "万泰生物",
      "batch": "EXISTING86",
      "primary_industry": "医药医疗",
      "primary_leaf": "创新药/生物药",
      "mount_count": 3,
      "root_count": 1,
      "leaf_count": 2,
      "severity": "medium",
      "diagnosis_label": "基本面偏弱 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "financial_weak",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡基本面草分低于 60。"
      ],
      "recommended_action": "分类可保留，但报告和卡片里标为基本面观察，不作为核心优先样本。",
      "theme_purity_score": 93.0,
      "fundamental_score": 41.0,
      "net_profit_yi": -0.45,
      "net_profit_yoy_pct": 0.19,
      "improvement": "仍为亏损，需结合同比和现金流判断是否改善",
      "tags": [
        {
          "name": "医药医疗",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        },
        {
          "name": "创新药",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "innovative_drug",
          "node_name": "创新药/生物药",
          "path": [
            "医药医疗",
            "创新药/生物药"
          ]
        },
        {
          "name": "生物药",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "innovative_drug",
          "node_name": "创新药/生物药",
          "path": [
            "医药医疗",
            "创新药/生物药"
          ]
        },
        {
          "name": "疫苗",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "innovative_drug",
          "node_name": "创新药/生物药",
          "path": [
            "医药医疗",
            "创新药/生物药"
          ]
        },
        {
          "name": "医药",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        },
        {
          "name": "IVD",
          "role": "相关概念",
          "weight": 0.75,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "medical_device",
          "node_name": "医疗器械/IVD",
          "path": [
            "医药医疗",
            "医疗器械/IVD"
          ]
        },
        {
          "name": "体外诊断",
          "role": "相关概念",
          "weight": 0.75,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "medical_device",
          "node_name": "医疗器械/IVD",
          "path": [
            "医药医疗",
            "医疗器械/IVD"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "创新药",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "innovative_drug",
          "node_name": "创新药/生物药",
          "path": [
            "医药医疗",
            "创新药/生物药"
          ]
        },
        {
          "name": "生物药",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "innovative_drug",
          "node_name": "创新药/生物药",
          "path": [
            "医药医疗",
            "创新药/生物药"
          ]
        },
        {
          "name": "疫苗",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "innovative_drug",
          "node_name": "创新药/生物药",
          "path": [
            "医药医疗",
            "创新药/生物药"
          ]
        },
        {
          "name": "IVD",
          "role": "相关概念",
          "weight": 0.75,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "medical_device",
          "node_name": "医疗器械/IVD",
          "path": [
            "医药医疗",
            "医疗器械/IVD"
          ]
        },
        {
          "name": "体外诊断",
          "role": "相关概念",
          "weight": 0.75,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "medical_device",
          "node_name": "医疗器械/IVD",
          "path": [
            "医药医疗",
            "医疗器械/IVD"
          ]
        }
      ]
    },
    {
      "code": "688114",
      "name": "华大智造",
      "batch": "EXISTING86",
      "primary_industry": "医药医疗",
      "primary_leaf": "医疗器械/IVD",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "基本面偏弱 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "financial_weak",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡基本面草分低于 60。"
      ],
      "recommended_action": "分类可保留，但报告和卡片里标为基本面观察，不作为核心优先样本。",
      "theme_purity_score": 93.0,
      "fundamental_score": 41.0,
      "net_profit_yi": -1.05,
      "net_profit_yoy_pct": 0.38,
      "improvement": "仍为亏损，需结合同比和现金流判断是否改善",
      "tags": [
        {
          "name": "医药医疗",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        },
        {
          "name": "医疗器械",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "medical_device",
          "node_name": "医疗器械/IVD",
          "path": [
            "医药医疗",
            "医疗器械/IVD"
          ]
        },
        {
          "name": "生命科学仪器",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "medical_device",
          "node_name": "医疗器械/IVD",
          "path": [
            "医药医疗",
            "医疗器械/IVD"
          ]
        },
        {
          "name": "医药",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "医疗器械",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "medical_device",
          "node_name": "医疗器械/IVD",
          "path": [
            "医药医疗",
            "医疗器械/IVD"
          ]
        },
        {
          "name": "生命科学仪器",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "medical_device",
          "node_name": "医疗器械/IVD",
          "path": [
            "医药医疗",
            "医疗器械/IVD"
          ]
        }
      ]
    },
    {
      "code": "300122",
      "name": "智飞生物",
      "batch": "EXISTING86",
      "primary_industry": "医药医疗",
      "primary_leaf": "创新药/生物药",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "基本面偏弱 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "financial_weak",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡基本面草分低于 60。"
      ],
      "recommended_action": "分类可保留，但报告和卡片里标为基本面观察，不作为核心优先样本。",
      "theme_purity_score": 93.0,
      "fundamental_score": 20.0,
      "net_profit_yi": -3.94,
      "net_profit_yoy_pct": -0.19,
      "improvement": "仍为亏损，需结合同比和现金流判断是否改善",
      "tags": [
        {
          "name": "医药医疗",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        },
        {
          "name": "创新药",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "innovative_drug",
          "node_name": "创新药/生物药",
          "path": [
            "医药医疗",
            "创新药/生物药"
          ]
        },
        {
          "name": "生物药",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "innovative_drug",
          "node_name": "创新药/生物药",
          "path": [
            "医药医疗",
            "创新药/生物药"
          ]
        },
        {
          "name": "疫苗",
          "role": "核心概念",
          "weight": 0.83,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "innovative_drug",
          "node_name": "创新药/生物药",
          "path": [
            "医药医疗",
            "创新药/生物药"
          ]
        },
        {
          "name": "医药",
          "role": "核心概念",
          "weight": 0.83,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        },
        {
          "name": "生物制药",
          "role": "相关概念",
          "weight": 0.69,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "innovative_drug",
          "node_name": "创新药/生物药",
          "path": [
            "医药医疗",
            "创新药/生物药"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "创新药",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "innovative_drug",
          "node_name": "创新药/生物药",
          "path": [
            "医药医疗",
            "创新药/生物药"
          ]
        },
        {
          "name": "生物药",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "innovative_drug",
          "node_name": "创新药/生物药",
          "path": [
            "医药医疗",
            "创新药/生物药"
          ]
        },
        {
          "name": "疫苗",
          "role": "核心概念",
          "weight": 0.83,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "innovative_drug",
          "node_name": "创新药/生物药",
          "path": [
            "医药医疗",
            "创新药/生物药"
          ]
        },
        {
          "name": "生物制药",
          "role": "相关概念",
          "weight": 0.69,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "innovative_drug",
          "node_name": "创新药/生物药",
          "path": [
            "医药医疗",
            "创新药/生物药"
          ]
        }
      ]
    },
    {
      "code": "300347",
      "name": "泰格医药",
      "batch": "EXISTING86",
      "primary_industry": "医药医疗",
      "primary_leaf": "CXO/CRO/CDMO",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "基本面偏弱 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "financial_weak",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡基本面草分低于 60。"
      ],
      "recommended_action": "分类可保留，但报告和卡片里标为基本面观察，不作为核心优先样本。",
      "theme_purity_score": 93.0,
      "fundamental_score": 49.0,
      "net_profit_yi": 3.3,
      "net_profit_yoy_pct": -0.7,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "医药医疗",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        },
        {
          "name": "CXO",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "cxo",
          "node_name": "CXO/CRO/CDMO",
          "path": [
            "医药医疗",
            "CXO/CRO/CDMO"
          ]
        },
        {
          "name": "临床CRO",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "cxo",
          "node_name": "CXO/CRO/CDMO",
          "path": [
            "医药医疗",
            "CXO/CRO/CDMO"
          ]
        },
        {
          "name": "医药",
          "role": "核心概念",
          "weight": 0.8,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        },
        {
          "name": "CRO",
          "role": "核心概念",
          "weight": 0.8,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "cxo",
          "node_name": "CXO/CRO/CDMO",
          "path": [
            "医药医疗",
            "CXO/CRO/CDMO"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "CXO",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "cxo",
          "node_name": "CXO/CRO/CDMO",
          "path": [
            "医药医疗",
            "CXO/CRO/CDMO"
          ]
        },
        {
          "name": "临床CRO",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "cxo",
          "node_name": "CXO/CRO/CDMO",
          "path": [
            "医药医疗",
            "CXO/CRO/CDMO"
          ]
        },
        {
          "name": "CRO",
          "role": "核心概念",
          "weight": 0.8,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "cxo",
          "node_name": "CXO/CRO/CDMO",
          "path": [
            "医药医疗",
            "CXO/CRO/CDMO"
          ]
        }
      ]
    },
    {
      "code": "688126",
      "name": "沪硅产业",
      "batch": "EXISTING86",
      "primary_industry": "半导体产业链",
      "primary_leaf": "半导体材料",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "基本面偏弱 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "financial_weak",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡基本面草分低于 60。"
      ],
      "recommended_action": "分类可保留，但报告和卡片里标为基本面观察，不作为核心优先样本。",
      "theme_purity_score": 94.0,
      "fundamental_score": 20.0,
      "net_profit_yi": -5.28,
      "net_profit_yoy_pct": -1.32,
      "improvement": "仍为亏损，需结合同比和现金流判断是否改善",
      "tags": [
        {
          "name": "半导体产业链",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "半导体材料",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "硅片",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "半导体",
          "role": "核心概念",
          "weight": 0.8,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "半导体材料",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "中",
          "basis": "210家质量诊断修正：原样本只挂父级，补充明确叶子节点",
          "node_id": "semimaterial",
          "node_name": "半导体材料",
          "path": [
            "半导体",
            "半导体材料"
          ]
        },
        {
          "name": "硅片",
          "role": "核心概念",
          "weight": 0.88,
          "evidence_level": "中",
          "basis": "210家质量诊断修正：原样本只挂父级，补充明确叶子节点",
          "node_id": "semimaterial",
          "node_name": "半导体材料",
          "path": [
            "半导体",
            "半导体材料"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "半导体材料",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "中",
          "basis": "210家质量诊断修正：原样本只挂父级，补充明确叶子节点",
          "node_id": "semimaterial",
          "node_name": "半导体材料",
          "path": [
            "半导体",
            "半导体材料"
          ]
        },
        {
          "name": "硅片",
          "role": "核心概念",
          "weight": 0.88,
          "evidence_level": "中",
          "basis": "210家质量诊断修正：原样本只挂父级，补充明确叶子节点",
          "node_id": "semimaterial",
          "node_name": "半导体材料",
          "path": [
            "半导体",
            "半导体材料"
          ]
        }
      ]
    },
    {
      "code": "600760",
      "name": "中航沈飞",
      "batch": "EXISTING86",
      "primary_industry": "国防军工与航空航天",
      "primary_leaf": "航空装备 / 大飞机",
      "mount_count": 3,
      "root_count": 2,
      "leaf_count": 2,
      "severity": "medium",
      "diagnosis_label": "主题分偏低 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "theme_low",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡主题纯度低于 75，需要观察是否为边缘概念。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 72.0,
      "fundamental_score": 67.0,
      "net_profit_yi": 1.65,
      "net_profit_yoy_pct": -0.62,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "国防军工与航空航天",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "defense_aerospace",
          "node_name": "国防军工与航空航天",
          "path": [
            "国防军工与航空航天"
          ]
        },
        {
          "name": "航空装备",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "defense_aircraft",
          "node_name": "航空装备 / 大飞机",
          "path": [
            "国防军工与航空航天",
            "航空装备 / 大飞机"
          ]
        },
        {
          "name": "军工主机",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "defense_aircraft",
          "node_name": "航空装备 / 大飞机",
          "path": [
            "国防军工与航空航天",
            "航空装备 / 大飞机"
          ]
        },
        {
          "name": "国防军工",
          "role": "核心概念",
          "weight": 0.8,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "defense_aircraft",
          "node_name": "航空装备 / 大飞机",
          "path": [
            "国防军工与航空航天",
            "航空装备 / 大飞机"
          ]
        },
        {
          "name": "航空航天",
          "role": "核心概念",
          "weight": 0.8,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "defense_aircraft",
          "node_name": "航空装备 / 大飞机",
          "path": [
            "国防军工与航空航天",
            "航空装备 / 大飞机"
          ]
        },
        {
          "name": "高端装备",
          "role": "相关概念",
          "weight": 0.66,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "industrial_machinery",
          "node_name": "工程机械 / 矿山装备",
          "path": [
            "高端装备与工业",
            "工程机械 / 矿山装备"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "航空装备",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "defense_aircraft",
          "node_name": "航空装备 / 大飞机",
          "path": [
            "国防军工与航空航天",
            "航空装备 / 大飞机"
          ]
        },
        {
          "name": "军工主机",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "defense_aircraft",
          "node_name": "航空装备 / 大飞机",
          "path": [
            "国防军工与航空航天",
            "航空装备 / 大飞机"
          ]
        },
        {
          "name": "国防军工",
          "role": "核心概念",
          "weight": 0.8,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "defense_aircraft",
          "node_name": "航空装备 / 大飞机",
          "path": [
            "国防军工与航空航天",
            "航空装备 / 大飞机"
          ]
        },
        {
          "name": "航空航天",
          "role": "核心概念",
          "weight": 0.8,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "defense_aircraft",
          "node_name": "航空装备 / 大飞机",
          "path": [
            "国防军工与航空航天",
            "航空装备 / 大飞机"
          ]
        },
        {
          "name": "高端装备",
          "role": "相关概念",
          "weight": 0.66,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "industrial_machinery",
          "node_name": "工程机械 / 矿山装备",
          "path": [
            "高端装备与工业",
            "工程机械 / 矿山装备"
          ]
        }
      ]
    },
    {
      "code": "000768",
      "name": "中航西飞",
      "batch": "EXISTING86",
      "primary_industry": "国防军工与航空航天",
      "primary_leaf": "航空装备 / 大飞机",
      "mount_count": 3,
      "root_count": 2,
      "leaf_count": 2,
      "severity": "medium",
      "diagnosis_label": "主题分偏低 / 基本面偏弱 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "theme_low",
        "financial_weak",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡主题纯度低于 75，需要观察是否为边缘概念。",
        "轻量证据卡基本面草分低于 60。"
      ],
      "recommended_action": "分类可保留，但报告和卡片里标为基本面观察，不作为核心优先样本。",
      "theme_purity_score": 72.0,
      "fundamental_score": 57.0,
      "net_profit_yi": 2.46,
      "net_profit_yoy_pct": -0.15,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "国防军工与航空航天",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "defense_aerospace",
          "node_name": "国防军工与航空航天",
          "path": [
            "国防军工与航空航天"
          ]
        },
        {
          "name": "航空装备",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "defense_aircraft",
          "node_name": "航空装备 / 大飞机",
          "path": [
            "国防军工与航空航天",
            "航空装备 / 大飞机"
          ]
        },
        {
          "name": "大飞机",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "defense_aircraft",
          "node_name": "航空装备 / 大飞机",
          "path": [
            "国防军工与航空航天",
            "航空装备 / 大飞机"
          ]
        },
        {
          "name": "国防军工",
          "role": "核心概念",
          "weight": 0.83,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "defense_aircraft",
          "node_name": "航空装备 / 大飞机",
          "path": [
            "国防军工与航空航天",
            "航空装备 / 大飞机"
          ]
        },
        {
          "name": "航空航天",
          "role": "核心概念",
          "weight": 0.83,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "defense_aircraft",
          "node_name": "航空装备 / 大飞机",
          "path": [
            "国防军工与航空航天",
            "航空装备 / 大飞机"
          ]
        },
        {
          "name": "高端装备",
          "role": "相关概念",
          "weight": 0.69,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "industrial_machinery",
          "node_name": "工程机械 / 矿山装备",
          "path": [
            "高端装备与工业",
            "工程机械 / 矿山装备"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "航空装备",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "defense_aircraft",
          "node_name": "航空装备 / 大飞机",
          "path": [
            "国防军工与航空航天",
            "航空装备 / 大飞机"
          ]
        },
        {
          "name": "大飞机",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "defense_aircraft",
          "node_name": "航空装备 / 大飞机",
          "path": [
            "国防军工与航空航天",
            "航空装备 / 大飞机"
          ]
        },
        {
          "name": "国防军工",
          "role": "核心概念",
          "weight": 0.83,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "defense_aircraft",
          "node_name": "航空装备 / 大飞机",
          "path": [
            "国防军工与航空航天",
            "航空装备 / 大飞机"
          ]
        },
        {
          "name": "航空航天",
          "role": "核心概念",
          "weight": 0.83,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "defense_aircraft",
          "node_name": "航空装备 / 大飞机",
          "path": [
            "国防军工与航空航天",
            "航空装备 / 大飞机"
          ]
        },
        {
          "name": "高端装备",
          "role": "相关概念",
          "weight": 0.69,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "industrial_machinery",
          "node_name": "工程机械 / 矿山装备",
          "path": [
            "高端装备与工业",
            "工程机械 / 矿山装备"
          ]
        }
      ]
    },
    {
      "code": "600893",
      "name": "航发动力",
      "batch": "EXISTING86",
      "primary_industry": "国防军工与航空航天",
      "primary_leaf": "航空发动机",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "基本面偏弱 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "financial_weak",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡基本面草分低于 60。"
      ],
      "recommended_action": "分类可保留，但报告和卡片里标为基本面观察，不作为核心优先样本。",
      "theme_purity_score": 94.0,
      "fundamental_score": 53.0,
      "net_profit_yi": 0.19,
      "net_profit_yoy_pct": -0.66,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "国防军工与航空航天",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "defense_aerospace",
          "node_name": "国防军工与航空航天",
          "path": [
            "国防军工与航空航天"
          ]
        },
        {
          "name": "航空发动机",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "defense_engine",
          "node_name": "航空发动机",
          "path": [
            "国防军工与航空航天",
            "航空发动机"
          ]
        },
        {
          "name": "国防军工",
          "role": "核心概念",
          "weight": 0.8,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "defense_engine",
          "node_name": "航空发动机",
          "path": [
            "国防军工与航空航天",
            "航空发动机"
          ]
        },
        {
          "name": "航空航天",
          "role": "核心概念",
          "weight": 0.8,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "defense_engine",
          "node_name": "航空发动机",
          "path": [
            "国防军工与航空航天",
            "航空发动机"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "航空发动机",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "defense_engine",
          "node_name": "航空发动机",
          "path": [
            "国防军工与航空航天",
            "航空发动机"
          ]
        },
        {
          "name": "国防军工",
          "role": "核心概念",
          "weight": 0.8,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "defense_engine",
          "node_name": "航空发动机",
          "path": [
            "国防军工与航空航天",
            "航空发动机"
          ]
        },
        {
          "name": "航空航天",
          "role": "核心概念",
          "weight": 0.8,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "defense_engine",
          "node_name": "航空发动机",
          "path": [
            "国防军工与航空航天",
            "航空发动机"
          ]
        }
      ]
    },
    {
      "code": "688122",
      "name": "西部超导",
      "batch": "EXISTING86",
      "primary_industry": "国防军工与航空航天",
      "primary_leaf": "航空装备 / 大飞机",
      "mount_count": 5,
      "root_count": 4,
      "leaf_count": 2,
      "severity": "medium",
      "diagnosis_label": "跨主线过多 / 主题分偏低 / 基本面偏弱 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "cross_root_overload",
        "theme_low",
        "financial_weak",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前跨 4 条主线，可能需要压缩非核心标签。",
        "轻量证据卡主题纯度低于 75，需要观察是否为边缘概念。",
        "轻量证据卡基本面草分低于 60。"
      ],
      "recommended_action": "压缩弱标签，只保留主业和强证据概念进入叶子节点。",
      "theme_purity_score": 72.0,
      "fundamental_score": 57.0,
      "net_profit_yi": 0.69,
      "net_profit_yoy_pct": -0.74,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "国防军工与航空航天",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "defense_aerospace",
          "node_name": "国防军工与航空航天",
          "path": [
            "国防军工与航空航天"
          ]
        },
        {
          "name": "航空装备",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "defense_aircraft",
          "node_name": "航空装备 / 大飞机",
          "path": [
            "国防军工与航空航天",
            "航空装备 / 大飞机"
          ]
        },
        {
          "name": "钛合金",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "advanced_metal_alloy",
          "node_name": "高端金属 / 合金",
          "path": [
            "有色金属与新材料",
            "高端金属 / 合金"
          ]
        },
        {
          "name": "高温合金",
          "role": "二级赛道",
          "weight": 0.78,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "defense_aircraft",
          "node_name": "航空装备 / 大飞机",
          "path": [
            "国防军工与航空航天",
            "航空装备 / 大飞机"
          ]
        },
        {
          "name": "光伏",
          "role": "观察概念",
          "weight": 0.56,
          "evidence_level": "中",
          "basis": "相关标签",
          "node_id": "renewable_power",
          "node_name": "电力与公用事业",
          "path": [
            "电力与公用事业"
          ]
        },
        {
          "name": "军工",
          "role": "核心概念",
          "weight": 0.8,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "defense_aircraft",
          "node_name": "航空装备 / 大飞机",
          "path": [
            "国防军工与航空航天",
            "航空装备 / 大飞机"
          ]
        },
        {
          "name": "半导体",
          "role": "观察概念",
          "weight": 0.56,
          "evidence_level": "中",
          "basis": "相关标签",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "新能源",
          "role": "观察概念",
          "weight": 0.56,
          "evidence_level": "中",
          "basis": "相关标签",
          "node_id": "renewable_power",
          "node_name": "电力与公用事业",
          "path": [
            "电力与公用事业"
          ]
        },
        {
          "name": "钛",
          "role": "核心概念",
          "weight": 0.8,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "advanced_metal_alloy",
          "node_name": "高端金属 / 合金",
          "path": [
            "有色金属与新材料",
            "高端金属 / 合金"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "航空装备",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "defense_aircraft",
          "node_name": "航空装备 / 大飞机",
          "path": [
            "国防军工与航空航天",
            "航空装备 / 大飞机"
          ]
        },
        {
          "name": "钛合金",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "advanced_metal_alloy",
          "node_name": "高端金属 / 合金",
          "path": [
            "有色金属与新材料",
            "高端金属 / 合金"
          ]
        },
        {
          "name": "高温合金",
          "role": "二级赛道",
          "weight": 0.78,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "defense_aircraft",
          "node_name": "航空装备 / 大飞机",
          "path": [
            "国防军工与航空航天",
            "航空装备 / 大飞机"
          ]
        },
        {
          "name": "军工",
          "role": "核心概念",
          "weight": 0.8,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "defense_aircraft",
          "node_name": "航空装备 / 大飞机",
          "path": [
            "国防军工与航空航天",
            "航空装备 / 大飞机"
          ]
        },
        {
          "name": "钛",
          "role": "核心概念",
          "weight": 0.8,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "advanced_metal_alloy",
          "node_name": "高端金属 / 合金",
          "path": [
            "有色金属与新材料",
            "高端金属 / 合金"
          ]
        }
      ]
    },
    {
      "code": "002534",
      "name": "西子洁能",
      "batch": "EXISTING86",
      "primary_industry": "新能源发电",
      "primary_leaf": "绿电运营",
      "mount_count": 3,
      "root_count": 3,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "跨主线过多 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "cross_root_overload",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前跨 3 条主线，可能需要压缩非核心标签。"
      ],
      "recommended_action": "压缩弱标签，只保留主业和强证据概念进入叶子节点。",
      "theme_purity_score": 85.0,
      "fundamental_score": 85.0,
      "net_profit_yi": 0.72,
      "net_profit_yoy_pct": 0.12,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "新能源发电",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "renewable_operator",
          "node_name": "绿电运营",
          "path": [
            "电力与公用事业",
            "绿电运营"
          ]
        },
        {
          "name": "光热发电",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "renewable_operator",
          "node_name": "绿电运营",
          "path": [
            "电力与公用事业",
            "绿电运营"
          ]
        },
        {
          "name": "半导体",
          "role": "相关概念",
          "weight": 0.65,
          "evidence_level": "中",
          "basis": "相关标签",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "新能源",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "renewable_operator",
          "node_name": "绿电运营",
          "path": [
            "电力与公用事业",
            "绿电运营"
          ]
        },
        {
          "name": "机器人",
          "role": "观察概念",
          "weight": 0.59,
          "evidence_level": "需复核",
          "basis": "主题标签",
          "node_id": "robotics",
          "node_name": "机器人/自动化",
          "path": [
            "机器人/自动化"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "新能源发电",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "renewable_operator",
          "node_name": "绿电运营",
          "path": [
            "电力与公用事业",
            "绿电运营"
          ]
        },
        {
          "name": "光热发电",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "renewable_operator",
          "node_name": "绿电运营",
          "path": [
            "电力与公用事业",
            "绿电运营"
          ]
        },
        {
          "name": "新能源",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "renewable_operator",
          "node_name": "绿电运营",
          "path": [
            "电力与公用事业",
            "绿电运营"
          ]
        }
      ]
    },
    {
      "code": "600456",
      "name": "宝钛股份",
      "batch": "EXISTING86",
      "primary_industry": "有色金属与新材料",
      "primary_leaf": "新能源材料",
      "mount_count": 4,
      "root_count": 3,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "跨主线过多 / 基本面偏弱 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "cross_root_overload",
        "financial_weak",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前跨 3 条主线，可能需要压缩非核心标签。",
        "轻量证据卡基本面草分低于 60。"
      ],
      "recommended_action": "压缩弱标签，只保留主业和强证据概念进入叶子节点。",
      "theme_purity_score": 86.0,
      "fundamental_score": 57.0,
      "net_profit_yi": 0.76,
      "net_profit_yoy_pct": -0.27,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "有色金属与新材料",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "advanced_materials",
          "node_name": "有色金属与新材料",
          "path": [
            "有色金属与新材料"
          ]
        },
        {
          "name": "稀有",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "advanced_new_energy_material",
          "node_name": "新能源材料",
          "path": [
            "有色金属与新材料",
            "新能源材料"
          ]
        },
        {
          "name": "战略金属",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "advanced_new_energy_material",
          "node_name": "新能源材料",
          "path": [
            "有色金属与新材料",
            "新能源材料"
          ]
        },
        {
          "name": "钛",
          "role": "核心概念",
          "weight": 0.83,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "advanced_new_energy_material",
          "node_name": "新能源材料",
          "path": [
            "有色金属与新材料",
            "新能源材料"
          ]
        },
        {
          "name": "光伏",
          "role": "观察概念",
          "weight": 0.59,
          "evidence_level": "中",
          "basis": "相关标签",
          "node_id": "renewable_power",
          "node_name": "电力与公用事业",
          "path": [
            "电力与公用事业"
          ]
        },
        {
          "name": "半导体",
          "role": "观察概念",
          "weight": 0.59,
          "evidence_level": "中",
          "basis": "相关标签",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "新能源",
          "role": "观察概念",
          "weight": 0.59,
          "evidence_level": "中",
          "basis": "相关标签",
          "node_id": "renewable_power",
          "node_name": "电力与公用事业",
          "path": [
            "电力与公用事业"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "稀有",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "advanced_new_energy_material",
          "node_name": "新能源材料",
          "path": [
            "有色金属与新材料",
            "新能源材料"
          ]
        },
        {
          "name": "战略金属",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "advanced_new_energy_material",
          "node_name": "新能源材料",
          "path": [
            "有色金属与新材料",
            "新能源材料"
          ]
        },
        {
          "name": "钛",
          "role": "核心概念",
          "weight": 0.83,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "advanced_new_energy_material",
          "node_name": "新能源材料",
          "path": [
            "有色金属与新材料",
            "新能源材料"
          ]
        }
      ]
    },
    {
      "code": "000100",
      "name": "TCL科技",
      "batch": "EXISTING86",
      "primary_industry": "消费电子与终端",
      "primary_leaf": "面板 / 显示",
      "mount_count": 5,
      "root_count": 4,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "跨主线过多 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "cross_root_overload",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前跨 4 条主线，可能需要压缩非核心标签。"
      ],
      "recommended_action": "压缩弱标签，只保留主业和强证据概念进入叶子节点。",
      "theme_purity_score": 93.0,
      "fundamental_score": 94.0,
      "net_profit_yi": 6.08,
      "net_profit_yoy_pct": 0.54,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "消费电子与终端",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "consumer_electronics",
          "node_name": "消费电子",
          "path": [
            "消费电子"
          ]
        },
        {
          "name": "面板",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "consumer_display",
          "node_name": "面板 / 显示",
          "path": [
            "消费电子",
            "面板 / 显示"
          ]
        },
        {
          "name": "显示",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "consumer_display",
          "node_name": "面板 / 显示",
          "path": [
            "消费电子",
            "面板 / 显示"
          ]
        },
        {
          "name": "LCD液晶面板",
          "role": "二级赛道",
          "weight": 0.78,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "consumer_display",
          "node_name": "面板 / 显示",
          "path": [
            "消费电子",
            "面板 / 显示"
          ]
        },
        {
          "name": "面板/显示",
          "role": "相关概念",
          "weight": 0.77,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "consumer_display",
          "node_name": "面板 / 显示",
          "path": [
            "消费电子",
            "面板 / 显示"
          ]
        },
        {
          "name": "OLED",
          "role": "相关概念",
          "weight": 0.77,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "consumer_display",
          "node_name": "面板 / 显示",
          "path": [
            "消费电子",
            "面板 / 显示"
          ]
        },
        {
          "name": "Mini LED",
          "role": "相关概念",
          "weight": 0.77,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "consumer_display",
          "node_name": "面板 / 显示",
          "path": [
            "消费电子",
            "面板 / 显示"
          ]
        },
        {
          "name": "光伏",
          "role": "相关概念",
          "weight": 0.67,
          "evidence_level": "中",
          "basis": "相关标签",
          "node_id": "renewable_power",
          "node_name": "电力与公用事业",
          "path": [
            "电力与公用事业"
          ]
        },
        {
          "name": "半导体材料",
          "role": "相关概念",
          "weight": 0.77,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "AI",
          "role": "相关概念",
          "weight": 0.61,
          "evidence_level": "需复核",
          "basis": "主题标签",
          "node_id": "ai_compute",
          "node_name": "AI算力",
          "path": [
            "AI算力"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "面板",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "consumer_display",
          "node_name": "面板 / 显示",
          "path": [
            "消费电子",
            "面板 / 显示"
          ]
        },
        {
          "name": "显示",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "consumer_display",
          "node_name": "面板 / 显示",
          "path": [
            "消费电子",
            "面板 / 显示"
          ]
        },
        {
          "name": "LCD液晶面板",
          "role": "二级赛道",
          "weight": 0.78,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "consumer_display",
          "node_name": "面板 / 显示",
          "path": [
            "消费电子",
            "面板 / 显示"
          ]
        },
        {
          "name": "面板/显示",
          "role": "相关概念",
          "weight": 0.77,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "consumer_display",
          "node_name": "面板 / 显示",
          "path": [
            "消费电子",
            "面板 / 显示"
          ]
        },
        {
          "name": "OLED",
          "role": "相关概念",
          "weight": 0.77,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "consumer_display",
          "node_name": "面板 / 显示",
          "path": [
            "消费电子",
            "面板 / 显示"
          ]
        },
        {
          "name": "Mini LED",
          "role": "相关概念",
          "weight": 0.77,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "consumer_display",
          "node_name": "面板 / 显示",
          "path": [
            "消费电子",
            "面板 / 显示"
          ]
        }
      ]
    },
    {
      "code": "002475",
      "name": "立讯精密",
      "batch": "EXISTING86",
      "primary_industry": "消费电子与终端",
      "primary_leaf": "终端制造 / 精密件",
      "mount_count": 5,
      "root_count": 3,
      "leaf_count": 4,
      "severity": "medium",
      "diagnosis_label": "跨主线过多 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "cross_root_overload",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前跨 3 条主线，可能需要压缩非核心标签。"
      ],
      "recommended_action": "压缩弱标签，只保留主业和强证据概念进入叶子节点。",
      "theme_purity_score": 85.0,
      "fundamental_score": 89.0,
      "net_profit_yi": 39.68,
      "net_profit_yoy_pct": 0.2,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "消费电子与终端",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "consumer_electronics",
          "node_name": "消费电子",
          "path": [
            "消费电子"
          ]
        },
        {
          "name": "苹果链",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "consumer_terminal_manufacturing",
          "node_name": "终端制造 / 精密件",
          "path": [
            "消费电子",
            "终端制造 / 精密件"
          ]
        },
        {
          "name": "连接器",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "auto_parts",
          "node_name": "汽车零部件",
          "path": [
            "新能源汽车",
            "汽车零部件"
          ]
        },
        {
          "name": "精密制造",
          "role": "二级赛道",
          "weight": 0.78,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "industrial_precision_equipment",
          "node_name": "精密制造 / 高端设备",
          "path": [
            "高端装备与工业",
            "精密制造 / 高端设备"
          ]
        },
        {
          "name": "消费电子",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "consumer_ai_terminal",
          "node_name": "AI终端 / 消费电子",
          "path": [
            "消费电子",
            "AI终端 / 消费电子"
          ]
        },
        {
          "name": "汽车电子",
          "role": "相关概念",
          "weight": 0.75,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "auto_parts",
          "node_name": "汽车零部件",
          "path": [
            "新能源汽车",
            "汽车零部件"
          ]
        },
        {
          "name": "AI终端",
          "role": "相关概念",
          "weight": 0.75,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "consumer_ai_terminal",
          "node_name": "AI终端 / 消费电子",
          "path": [
            "消费电子",
            "AI终端 / 消费电子"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "苹果链",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "consumer_terminal_manufacturing",
          "node_name": "终端制造 / 精密件",
          "path": [
            "消费电子",
            "终端制造 / 精密件"
          ]
        },
        {
          "name": "连接器",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "auto_parts",
          "node_name": "汽车零部件",
          "path": [
            "新能源汽车",
            "汽车零部件"
          ]
        },
        {
          "name": "精密制造",
          "role": "二级赛道",
          "weight": 0.78,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "industrial_precision_equipment",
          "node_name": "精密制造 / 高端设备",
          "path": [
            "高端装备与工业",
            "精密制造 / 高端设备"
          ]
        },
        {
          "name": "消费电子",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "consumer_ai_terminal",
          "node_name": "AI终端 / 消费电子",
          "path": [
            "消费电子",
            "AI终端 / 消费电子"
          ]
        },
        {
          "name": "汽车电子",
          "role": "相关概念",
          "weight": 0.75,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "auto_parts",
          "node_name": "汽车零部件",
          "path": [
            "新能源汽车",
            "汽车零部件"
          ]
        },
        {
          "name": "AI终端",
          "role": "相关概念",
          "weight": 0.75,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "consumer_ai_terminal",
          "node_name": "AI终端 / 消费电子",
          "path": [
            "消费电子",
            "AI终端 / 消费电子"
          ]
        }
      ]
    },
    {
      "code": "002138",
      "name": "顺络电子",
      "batch": "EXISTING86",
      "primary_industry": "消费电子与终端",
      "primary_leaf": "电子元器件/被动元器件",
      "mount_count": 3,
      "root_count": 3,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "跨主线过多 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "cross_root_overload",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前跨 3 条主线，可能需要压缩非核心标签。"
      ],
      "recommended_action": "压缩弱标签，只保留主业和强证据概念进入叶子节点。",
      "theme_purity_score": 88.0,
      "fundamental_score": 63.0,
      "net_profit_yi": 1.98,
      "net_profit_yoy_pct": -0.24,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "消费电子与终端",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "consumer_electronics",
          "node_name": "消费电子",
          "path": [
            "消费电子"
          ]
        },
        {
          "name": "电感",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "passive_components",
          "node_name": "电子元器件/被动元器件",
          "path": [
            "半导体",
            "电子元器件/被动元器件"
          ]
        },
        {
          "name": "被动元器件",
          "role": "二级赛道",
          "weight": 0.78,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "passive_components",
          "node_name": "电子元器件/被动元器件",
          "path": [
            "半导体",
            "电子元器件/被动元器件"
          ]
        },
        {
          "name": "AI",
          "role": "观察概念",
          "weight": 0.53,
          "evidence_level": "需复核",
          "basis": "主题标签",
          "node_id": "ai_compute",
          "node_name": "AI算力",
          "path": [
            "AI算力"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "电感",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "passive_components",
          "node_name": "电子元器件/被动元器件",
          "path": [
            "半导体",
            "电子元器件/被动元器件"
          ]
        },
        {
          "name": "被动元器件",
          "role": "二级赛道",
          "weight": 0.78,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "passive_components",
          "node_name": "电子元器件/被动元器件",
          "path": [
            "半导体",
            "电子元器件/被动元器件"
          ]
        }
      ]
    },
    {
      "code": "600519",
      "name": "贵州茅台",
      "batch": "EXISTING86",
      "primary_industry": "消费白马",
      "primary_leaf": "白酒 / 高端消费",
      "mount_count": 3,
      "root_count": 2,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "主题分偏低 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "theme_low",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡主题纯度低于 75，需要观察是否为边缘概念。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 72.0,
      "fundamental_score": 95.0,
      "net_profit_yi": 281.54,
      "net_profit_yoy_pct": 0.01,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "消费白马",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "consumer_bluechip",
          "node_name": "消费白马",
          "path": [
            "消费白马"
          ]
        },
        {
          "name": "白酒",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "consumer_liquor_premium",
          "node_name": "白酒 / 高端消费",
          "path": [
            "消费白马",
            "白酒 / 高端消费"
          ]
        },
        {
          "name": "高端消费",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "consumer_liquor_premium",
          "node_name": "白酒 / 高端消费",
          "path": [
            "消费白马",
            "白酒 / 高端消费"
          ]
        },
        {
          "name": "消费",
          "role": "核心概念",
          "weight": 0.86,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "consumer_liquor_premium",
          "node_name": "白酒 / 高端消费",
          "path": [
            "消费白马",
            "白酒 / 高端消费"
          ]
        },
        {
          "name": "高股息",
          "role": "相关概念",
          "weight": 0.72,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "finance",
          "node_name": "金融",
          "path": [
            "金融"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "白酒",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "consumer_liquor_premium",
          "node_name": "白酒 / 高端消费",
          "path": [
            "消费白马",
            "白酒 / 高端消费"
          ]
        },
        {
          "name": "高端消费",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "consumer_liquor_premium",
          "node_name": "白酒 / 高端消费",
          "path": [
            "消费白马",
            "白酒 / 高端消费"
          ]
        },
        {
          "name": "消费",
          "role": "核心概念",
          "weight": 0.86,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "consumer_liquor_premium",
          "node_name": "白酒 / 高端消费",
          "path": [
            "消费白马",
            "白酒 / 高端消费"
          ]
        }
      ]
    },
    {
      "code": "600938",
      "name": "中国海油",
      "batch": "EXISTING86",
      "primary_industry": "资源周期",
      "primary_leaf": "能源资源",
      "mount_count": 3,
      "root_count": 2,
      "leaf_count": 2,
      "severity": "medium",
      "diagnosis_label": "主题分偏低 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "theme_low",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡主题纯度低于 75，需要观察是否为边缘概念。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 72.0,
      "fundamental_score": 95.0,
      "net_profit_yi": 391.84,
      "net_profit_yoy_pct": 0.07,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "资源周期",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "resources",
          "node_name": "资源周期",
          "path": [
            "资源周期"
          ]
        },
        {
          "name": "油气开采",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "resources_energy",
          "node_name": "能源资源",
          "path": [
            "资源周期",
            "能源资源"
          ]
        },
        {
          "name": "高股息",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "finance_high_dividend",
          "node_name": "高股息金融",
          "path": [
            "金融",
            "高股息金融"
          ]
        },
        {
          "name": "资源",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "resources_energy",
          "node_name": "能源资源",
          "path": [
            "资源周期",
            "能源资源"
          ]
        },
        {
          "name": "油气",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "resources_energy",
          "node_name": "能源资源",
          "path": [
            "资源周期",
            "能源资源"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "油气开采",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "resources_energy",
          "node_name": "能源资源",
          "path": [
            "资源周期",
            "能源资源"
          ]
        },
        {
          "name": "高股息",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "finance_high_dividend",
          "node_name": "高股息金融",
          "path": [
            "金融",
            "高股息金融"
          ]
        },
        {
          "name": "资源",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "resources_energy",
          "node_name": "能源资源",
          "path": [
            "资源周期",
            "能源资源"
          ]
        },
        {
          "name": "油气",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "resources_energy",
          "node_name": "能源资源",
          "path": [
            "资源周期",
            "能源资源"
          ]
        }
      ]
    },
    {
      "code": "002460",
      "name": "赣锋锂业",
      "batch": "EXISTING86",
      "primary_industry": "资源周期",
      "primary_leaf": "金属矿产",
      "mount_count": 5,
      "root_count": 4,
      "leaf_count": 3,
      "severity": "medium",
      "diagnosis_label": "跨主线过多 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "cross_root_overload",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前跨 4 条主线，可能需要压缩非核心标签。"
      ],
      "recommended_action": "压缩弱标签，只保留主业和强证据概念进入叶子节点。",
      "theme_purity_score": 94.0,
      "fundamental_score": 95.0,
      "net_profit_yi": 19.22,
      "net_profit_yoy_pct": 6.16,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "资源周期",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "resources",
          "node_name": "资源周期",
          "path": [
            "资源周期"
          ]
        },
        {
          "name": "锂资源",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "resources_metal_mining",
          "node_name": "金属矿产",
          "path": [
            "资源周期",
            "金属矿产"
          ]
        },
        {
          "name": "新能源材料",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "advanced_new_energy_material",
          "node_name": "新能源材料",
          "path": [
            "有色金属与新材料",
            "新能源材料"
          ]
        },
        {
          "name": "资源",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "resources_metal_mining",
          "node_name": "金属矿产",
          "path": [
            "资源周期",
            "金属矿产"
          ]
        },
        {
          "name": "锂电池",
          "role": "相关概念",
          "weight": 0.77,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "battery_storage",
          "node_name": "动力电池/储能",
          "path": [
            "新能源汽车",
            "动力电池/储能"
          ]
        },
        {
          "name": "新能源",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "renewable_power",
          "node_name": "电力与公用事业",
          "path": [
            "电力与公用事业"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "锂资源",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "resources_metal_mining",
          "node_name": "金属矿产",
          "path": [
            "资源周期",
            "金属矿产"
          ]
        },
        {
          "name": "新能源材料",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "advanced_new_energy_material",
          "node_name": "新能源材料",
          "path": [
            "有色金属与新材料",
            "新能源材料"
          ]
        },
        {
          "name": "资源",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "resources_metal_mining",
          "node_name": "金属矿产",
          "path": [
            "资源周期",
            "金属矿产"
          ]
        },
        {
          "name": "锂电池",
          "role": "相关概念",
          "weight": 0.77,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "battery_storage",
          "node_name": "动力电池/储能",
          "path": [
            "新能源汽车",
            "动力电池/储能"
          ]
        }
      ]
    },
    {
      "code": "601618",
      "name": "中国中冶",
      "batch": "EXISTING86",
      "primary_industry": "高端装备与工业",
      "primary_leaf": "工程机械 / 矿山装备",
      "mount_count": 4,
      "root_count": 3,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "跨主线过多 / 主题分偏低 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "cross_root_overload",
        "theme_low",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前跨 3 条主线，可能需要压缩非核心标签。",
        "轻量证据卡主题纯度低于 75，需要观察是否为边缘概念。"
      ],
      "recommended_action": "压缩弱标签，只保留主业和强证据概念进入叶子节点。",
      "theme_purity_score": 72.0,
      "fundamental_score": 84.0,
      "net_profit_yi": 19.74,
      "net_profit_yoy_pct": 0.02,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "高端装备与工业",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "industrial_equipment",
          "node_name": "高端装备与工业",
          "path": [
            "高端装备与工业"
          ]
        },
        {
          "name": "工程",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "industrial_machinery",
          "node_name": "工程机械 / 矿山装备",
          "path": [
            "高端装备与工业",
            "工程机械 / 矿山装备"
          ]
        },
        {
          "name": "矿山机械",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "industrial_machinery",
          "node_name": "工程机械 / 矿山装备",
          "path": [
            "高端装备与工业",
            "工程机械 / 矿山装备"
          ]
        },
        {
          "name": "冶金工程",
          "role": "二级赛道",
          "weight": 0.78,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "industrial_machinery",
          "node_name": "工程机械 / 矿山装备",
          "path": [
            "高端装备与工业",
            "工程机械 / 矿山装备"
          ]
        },
        {
          "name": "光伏",
          "role": "相关概念",
          "weight": 0.62,
          "evidence_level": "中",
          "basis": "相关标签",
          "node_id": "renewable_power",
          "node_name": "电力与公用事业",
          "path": [
            "电力与公用事业"
          ]
        },
        {
          "name": "半导体",
          "role": "相关概念",
          "weight": 0.62,
          "evidence_level": "中",
          "basis": "相关标签",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "新能源",
          "role": "相关概念",
          "weight": 0.62,
          "evidence_level": "中",
          "basis": "相关标签",
          "node_id": "renewable_power",
          "node_name": "电力与公用事业",
          "path": [
            "电力与公用事业"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "工程",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "industrial_machinery",
          "node_name": "工程机械 / 矿山装备",
          "path": [
            "高端装备与工业",
            "工程机械 / 矿山装备"
          ]
        },
        {
          "name": "矿山机械",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "industrial_machinery",
          "node_name": "工程机械 / 矿山装备",
          "path": [
            "高端装备与工业",
            "工程机械 / 矿山装备"
          ]
        },
        {
          "name": "冶金工程",
          "role": "二级赛道",
          "weight": 0.78,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "industrial_machinery",
          "node_name": "工程机械 / 矿山装备",
          "path": [
            "高端装备与工业",
            "工程机械 / 矿山装备"
          ]
        }
      ]
    },
    {
      "code": "300124",
      "name": "汇川技术",
      "batch": "EXISTING86",
      "primary_industry": "高端装备与工业",
      "primary_leaf": "工业自动化 / 工业母机",
      "mount_count": 4,
      "root_count": 3,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "跨主线过多 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "cross_root_overload",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前跨 3 条主线，可能需要压缩非核心标签。"
      ],
      "recommended_action": "压缩弱标签，只保留主业和强证据概念进入叶子节点。",
      "theme_purity_score": 93.0,
      "fundamental_score": 68.0,
      "net_profit_yi": 10.23,
      "net_profit_yoy_pct": -0.23,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "高端装备与工业",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "industrial_equipment",
          "node_name": "高端装备与工业",
          "path": [
            "高端装备与工业"
          ]
        },
        {
          "name": "工业自动化",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "robot_industrial_auto",
          "node_name": "工业自动化 / 工业母机",
          "path": [
            "机器人/自动化",
            "工业自动化 / 工业母机"
          ]
        },
        {
          "name": "机器人",
          "role": "观察概念",
          "weight": 0.53,
          "evidence_level": "需复核",
          "basis": "主题标签",
          "node_id": "robotics",
          "node_name": "机器人/自动化",
          "path": [
            "机器人/自动化"
          ]
        },
        {
          "name": "工业母机",
          "role": "相关概念",
          "weight": 0.69,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "robot_industrial_auto",
          "node_name": "工业自动化 / 工业母机",
          "path": [
            "机器人/自动化",
            "工业自动化 / 工业母机"
          ]
        },
        {
          "name": "新能源车",
          "role": "核心概念",
          "weight": 0.83,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "new_energy_vehicle",
          "node_name": "新能源汽车",
          "path": [
            "新能源汽车"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "工业自动化",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "robot_industrial_auto",
          "node_name": "工业自动化 / 工业母机",
          "path": [
            "机器人/自动化",
            "工业自动化 / 工业母机"
          ]
        },
        {
          "name": "工业母机",
          "role": "相关概念",
          "weight": 0.69,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "robot_industrial_auto",
          "node_name": "工业自动化 / 工业母机",
          "path": [
            "机器人/自动化",
            "工业自动化 / 工业母机"
          ]
        }
      ]
    },
    {
      "code": "002230",
      "name": "科大讯飞",
      "batch": "EXISTING86",
      "primary_industry": "高端装备与工业",
      "primary_leaf": "工业软件 / 企业软件",
      "mount_count": 3,
      "root_count": 2,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "主题分偏低 / 基本面偏弱 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "theme_low",
        "financial_weak",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡主题纯度低于 75，需要观察是否为边缘概念。",
        "轻量证据卡基本面草分低于 60。"
      ],
      "recommended_action": "分类可保留，但报告和卡片里标为基本面观察，不作为核心优先样本。",
      "theme_purity_score": 72.0,
      "fundamental_score": 41.0,
      "net_profit_yi": -1.88,
      "net_profit_yoy_pct": 0.12,
      "improvement": "仍为亏损，需结合同比和现金流判断是否改善",
      "tags": [
        {
          "name": "高端装备与工业",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "industrial_equipment",
          "node_name": "高端装备与工业",
          "path": [
            "高端装备与工业"
          ]
        },
        {
          "name": "AI应用",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "industrial_software",
          "node_name": "工业软件 / 企业软件",
          "path": [
            "高端装备与工业",
            "工业软件 / 企业软件"
          ]
        },
        {
          "name": "教育软件",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "industrial_software",
          "node_name": "工业软件 / 企业软件",
          "path": [
            "高端装备与工业",
            "工业软件 / 企业软件"
          ]
        },
        {
          "name": "办公软件",
          "role": "相关概念",
          "weight": 0.75,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "industrial_software",
          "node_name": "工业软件 / 企业软件",
          "path": [
            "高端装备与工业",
            "工业软件 / 企业软件"
          ]
        },
        {
          "name": "人工智能",
          "role": "相关概念",
          "weight": 0.75,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "ai_compute",
          "node_name": "AI算力",
          "path": [
            "AI算力"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "AI应用",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "industrial_software",
          "node_name": "工业软件 / 企业软件",
          "path": [
            "高端装备与工业",
            "工业软件 / 企业软件"
          ]
        },
        {
          "name": "教育软件",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "industrial_software",
          "node_name": "工业软件 / 企业软件",
          "path": [
            "高端装备与工业",
            "工业软件 / 企业软件"
          ]
        },
        {
          "name": "办公软件",
          "role": "相关概念",
          "weight": 0.75,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "industrial_software",
          "node_name": "工业软件 / 企业软件",
          "path": [
            "高端装备与工业",
            "工业软件 / 企业软件"
          ]
        }
      ]
    },
    {
      "code": "300251",
      "name": "光线传媒",
      "batch": "NEXT44",
      "primary_industry": "传媒互联网",
      "primary_leaf": "游戏/内容",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "基本面偏弱 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "financial_weak",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡基本面草分低于 60。"
      ],
      "recommended_action": "分类可保留，但报告和卡片里标为基本面观察，不作为核心优先样本。",
      "theme_purity_score": 96.0,
      "fundamental_score": 43.0,
      "net_profit_yi": 0.24,
      "net_profit_yoy_pct": -0.99,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "传媒互联网",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "media_internet",
          "node_name": "传媒互联网",
          "path": [
            "传媒互联网"
          ]
        },
        {
          "name": "游戏 / 内容",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "game_content",
          "node_name": "游戏/内容",
          "path": [
            "传媒互联网",
            "游戏 / 内容"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "游戏 / 内容",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "game_content",
          "node_name": "游戏/内容",
          "path": [
            "传媒互联网",
            "游戏 / 内容"
          ]
        }
      ]
    },
    {
      "code": "002624",
      "name": "完美世界",
      "batch": "NEXT44",
      "primary_industry": "传媒互联网",
      "primary_leaf": "游戏/内容",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "基本面偏弱 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "financial_weak",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡基本面草分低于 60。"
      ],
      "recommended_action": "分类可保留，但报告和卡片里标为基本面观察，不作为核心优先样本。",
      "theme_purity_score": 94.0,
      "fundamental_score": 57.0,
      "net_profit_yi": 1.02,
      "net_profit_yoy_pct": -0.66,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "传媒互联网",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "media_internet",
          "node_name": "传媒互联网",
          "path": [
            "传媒互联网"
          ]
        },
        {
          "name": "游戏 / 内容",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "game_content",
          "node_name": "游戏/内容",
          "path": [
            "传媒互联网",
            "游戏 / 内容"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "游戏 / 内容",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "game_content",
          "node_name": "游戏/内容",
          "path": [
            "传媒互联网",
            "游戏 / 内容"
          ]
        }
      ]
    },
    {
      "code": "300413",
      "name": "芒果超媒",
      "batch": "NEXT44",
      "primary_industry": "传媒互联网",
      "primary_leaf": "游戏/内容",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "基本面偏弱 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "financial_weak",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡基本面草分低于 60。"
      ],
      "recommended_action": "分类可保留，但报告和卡片里标为基本面观察，不作为核心优先样本。",
      "theme_purity_score": 93.0,
      "fundamental_score": 57.0,
      "net_profit_yi": 1.99,
      "net_profit_yoy_pct": -0.47,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "传媒互联网",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "media_internet",
          "node_name": "传媒互联网",
          "path": [
            "传媒互联网"
          ]
        },
        {
          "name": "游戏 / 内容",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "game_content",
          "node_name": "游戏/内容",
          "path": [
            "传媒互联网",
            "游戏 / 内容"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "游戏 / 内容",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "game_content",
          "node_name": "游戏/内容",
          "path": [
            "传媒互联网",
            "游戏 / 内容"
          ]
        }
      ]
    },
    {
      "code": "000876",
      "name": "新希望",
      "batch": "NEXT44",
      "primary_industry": "农业养殖",
      "primary_leaf": "猪禽水产养殖",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "基本面偏弱 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "financial_weak",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡基本面草分低于 60。"
      ],
      "recommended_action": "分类可保留，但报告和卡片里标为基本面观察，不作为核心优先样本。",
      "theme_purity_score": 94.0,
      "fundamental_score": 20.0,
      "net_profit_yi": -8.85,
      "net_profit_yoy_pct": -3.02,
      "improvement": "仍为亏损，需结合同比和现金流判断是否改善",
      "tags": [
        {
          "name": "农业养殖",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "agriculture_food",
          "node_name": "农业养殖",
          "path": [
            "农业养殖"
          ]
        },
        {
          "name": "猪禽水产养殖",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "pig_poultry_aquaculture",
          "node_name": "猪禽水产养殖",
          "path": [
            "农业养殖",
            "猪禽水产养殖"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "猪禽水产养殖",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "pig_poultry_aquaculture",
          "node_name": "猪禽水产养殖",
          "path": [
            "农业养殖",
            "猪禽水产养殖"
          ]
        }
      ]
    },
    {
      "code": "300498",
      "name": "温氏股份",
      "batch": "NEXT44",
      "primary_industry": "农业养殖",
      "primary_leaf": "猪禽水产养殖",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "基本面偏弱 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "financial_weak",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡基本面草分低于 60。"
      ],
      "recommended_action": "分类可保留，但报告和卡片里标为基本面观察，不作为核心优先样本。",
      "theme_purity_score": 93.0,
      "fundamental_score": 20.0,
      "net_profit_yi": -10.88,
      "net_profit_yoy_pct": -1.53,
      "improvement": "仍为亏损，需结合同比和现金流判断是否改善",
      "tags": [
        {
          "name": "农业养殖",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "agriculture_food",
          "node_name": "农业养殖",
          "path": [
            "农业养殖"
          ]
        },
        {
          "name": "猪禽水产养殖",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "pig_poultry_aquaculture",
          "node_name": "猪禽水产养殖",
          "path": [
            "农业养殖",
            "猪禽水产养殖"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "猪禽水产养殖",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "pig_poultry_aquaculture",
          "node_name": "猪禽水产养殖",
          "path": [
            "农业养殖",
            "猪禽水产养殖"
          ]
        }
      ]
    },
    {
      "code": "002714",
      "name": "牧原股份",
      "batch": "NEXT44",
      "primary_industry": "农业养殖",
      "primary_leaf": "猪禽水产养殖",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "基本面偏弱 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "financial_weak",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡基本面草分低于 60。"
      ],
      "recommended_action": "分类可保留，但报告和卡片里标为基本面观察，不作为核心优先样本。",
      "theme_purity_score": 94.0,
      "fundamental_score": 20.0,
      "net_profit_yi": -12.22,
      "net_profit_yoy_pct": -1.27,
      "improvement": "仍为亏损，需结合同比和现金流判断是否改善",
      "tags": [
        {
          "name": "农业养殖",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "agriculture_food",
          "node_name": "农业养殖",
          "path": [
            "农业养殖"
          ]
        },
        {
          "name": "猪禽水产养殖",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "pig_poultry_aquaculture",
          "node_name": "猪禽水产养殖",
          "path": [
            "农业养殖",
            "猪禽水产养殖"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "猪禽水产养殖",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "pig_poultry_aquaculture",
          "node_name": "猪禽水产养殖",
          "path": [
            "农业养殖",
            "猪禽水产养殖"
          ]
        }
      ]
    },
    {
      "code": "000998",
      "name": "隆平高科",
      "batch": "NEXT44",
      "primary_industry": "农业养殖",
      "primary_leaf": "种业/种植",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "基本面偏弱 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "financial_weak",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡基本面草分低于 60。"
      ],
      "recommended_action": "分类可保留，但报告和卡片里标为基本面观察，不作为核心优先样本。",
      "theme_purity_score": 94.0,
      "fundamental_score": 20.0,
      "net_profit_yi": -1.94,
      "net_profit_yoy_pct": -61.46,
      "improvement": "仍为亏损，需结合同比和现金流判断是否改善",
      "tags": [
        {
          "name": "农业养殖",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "agriculture_food",
          "node_name": "农业养殖",
          "path": [
            "农业养殖"
          ]
        },
        {
          "name": "种业 / 种植",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "seed_planting",
          "node_name": "种业/种植",
          "path": [
            "农业养殖",
            "种业 / 种植"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "种业 / 种植",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "seed_planting",
          "node_name": "种业/种植",
          "path": [
            "农业养殖",
            "种业 / 种植"
          ]
        }
      ]
    },
    {
      "code": "688347",
      "name": "华虹公司",
      "batch": "NEXT44",
      "primary_industry": "半导体",
      "primary_leaf": "半导体设备",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "基本面偏弱 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "financial_weak",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡基本面草分低于 60。"
      ],
      "recommended_action": "分类可保留，但报告和卡片里标为基本面观察，不作为核心优先样本。",
      "theme_purity_score": 85.0,
      "fundamental_score": 49.0,
      "net_profit_yi": -1.27,
      "net_profit_yoy_pct": 5.13,
      "improvement": "仍为亏损，需结合同比和现金流判断是否改善",
      "tags": [
        {
          "name": "半导体",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "中",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "半导体设备",
          "role": "二级赛道",
          "weight": 0.82,
          "evidence_level": "中",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "semicap",
          "node_name": "半导体设备",
          "path": [
            "半导体",
            "半导体设备"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "半导体设备",
          "role": "二级赛道",
          "weight": 0.82,
          "evidence_level": "中",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "semicap",
          "node_name": "半导体设备",
          "path": [
            "半导体",
            "半导体设备"
          ]
        }
      ]
    },
    {
      "code": "600859",
      "name": "王府井",
      "batch": "NEXT44",
      "primary_industry": "商贸零售 / 社服",
      "primary_leaf": "零售/电商",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "基本面偏弱 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "financial_weak",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡基本面草分低于 60。"
      ],
      "recommended_action": "分类可保留，但报告和卡片里标为基本面观察，不作为核心优先样本。",
      "theme_purity_score": 95.0,
      "fundamental_score": 53.0,
      "net_profit_yi": 0.66,
      "net_profit_yoy_pct": -0.07,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "商贸零售 / 社服",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "commercial_retail_social_service",
          "node_name": "商贸零售/社服",
          "path": [
            "商贸零售 / 社服"
          ]
        },
        {
          "name": "零售 / 电商",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "retail_ecommerce",
          "node_name": "零售/电商",
          "path": [
            "商贸零售 / 社服",
            "零售 / 电商"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "零售 / 电商",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "retail_ecommerce",
          "node_name": "零售/电商",
          "path": [
            "商贸零售 / 社服",
            "零售 / 电商"
          ]
        }
      ]
    },
    {
      "code": "603711",
      "name": "香飘飘",
      "batch": "NEXT44",
      "primary_industry": "商贸零售 / 社服",
      "primary_leaf": "大众消费",
      "mount_count": 2,
      "root_count": 2,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "主题分偏低 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "theme_low",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡主题纯度低于 75，需要观察是否为边缘概念。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 72.0,
      "fundamental_score": 93.0,
      "net_profit_yi": 0.93,
      "net_profit_yoy_pct": 5.97,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "商贸零售 / 社服",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "需复核",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "commercial_retail_social_service",
          "node_name": "商贸零售/社服",
          "path": [
            "商贸零售 / 社服"
          ]
        },
        {
          "name": "大众消费",
          "role": "二级赛道",
          "weight": 0.72,
          "evidence_level": "需复核",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "consumer_mass",
          "node_name": "大众消费",
          "path": [
            "消费白马",
            "大众消费"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "大众消费",
          "role": "二级赛道",
          "weight": 0.72,
          "evidence_level": "需复核",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "consumer_mass",
          "node_name": "大众消费",
          "path": [
            "消费白马",
            "大众消费"
          ]
        }
      ]
    },
    {
      "code": "600048",
      "name": "保利发展",
      "batch": "NEXT44",
      "primary_industry": "地产 / 建筑建材",
      "primary_leaf": "房地产开发",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "基本面偏弱 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "financial_weak",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡基本面草分低于 60。"
      ],
      "recommended_action": "分类可保留，但报告和卡片里标为基本面观察，不作为核心优先样本。",
      "theme_purity_score": 95.0,
      "fundamental_score": 58.0,
      "net_profit_yi": 11.91,
      "net_profit_yoy_pct": -0.57,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "地产 / 建筑建材",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "real_estate_construction",
          "node_name": "地产/建筑建材",
          "path": [
            "地产 / 建筑建材"
          ]
        },
        {
          "name": "房地产开发",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "real_estate_developer",
          "node_name": "房地产开发",
          "path": [
            "地产 / 建筑建材",
            "房地产开发"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "房地产开发",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "real_estate_developer",
          "node_name": "房地产开发",
          "path": [
            "地产 / 建筑建材",
            "房地产开发"
          ]
        }
      ]
    },
    {
      "code": "001979",
      "name": "招商蛇口",
      "batch": "NEXT44",
      "primary_industry": "地产 / 建筑建材",
      "primary_leaf": "房地产开发",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "基本面偏弱 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "financial_weak",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡基本面草分低于 60。"
      ],
      "recommended_action": "分类可保留，但报告和卡片里标为基本面观察，不作为核心优先样本。",
      "theme_purity_score": 95.0,
      "fundamental_score": 43.0,
      "net_profit_yi": 2.06,
      "net_profit_yoy_pct": -0.91,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "地产 / 建筑建材",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "real_estate_construction",
          "node_name": "地产/建筑建材",
          "path": [
            "地产 / 建筑建材"
          ]
        },
        {
          "name": "房地产开发",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "real_estate_developer",
          "node_name": "房地产开发",
          "path": [
            "地产 / 建筑建材",
            "房地产开发"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "房地产开发",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "real_estate_developer",
          "node_name": "房地产开发",
          "path": [
            "地产 / 建筑建材",
            "房地产开发"
          ]
        }
      ]
    },
    {
      "code": "600600",
      "name": "青岛啤酒",
      "batch": "NEXT44",
      "primary_industry": "消费白马",
      "primary_leaf": "大众消费",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "主题分偏低 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "theme_low",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡主题纯度低于 75，需要观察是否为边缘概念。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 72.0,
      "fundamental_score": 94.0,
      "net_profit_yi": 18.33,
      "net_profit_yoy_pct": 0.05,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "消费白马",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "需复核",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "consumer_bluechip",
          "node_name": "消费白马",
          "path": [
            "消费白马"
          ]
        },
        {
          "name": "大众消费",
          "role": "二级赛道",
          "weight": 0.72,
          "evidence_level": "需复核",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "consumer_mass",
          "node_name": "大众消费",
          "path": [
            "消费白马",
            "大众消费"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "大众消费",
          "role": "二级赛道",
          "weight": 0.72,
          "evidence_level": "需复核",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "consumer_mass",
          "node_name": "大众消费",
          "path": [
            "消费白马",
            "大众消费"
          ]
        }
      ]
    },
    {
      "code": "688223",
      "name": "晶科能源",
      "batch": "NEXT44",
      "primary_industry": "电力与公用事业",
      "primary_leaf": "绿电运营",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "主题分偏低 / 基本面偏弱 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "theme_low",
        "financial_weak",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡主题纯度低于 75，需要观察是否为边缘概念。",
        "轻量证据卡基本面草分低于 60。"
      ],
      "recommended_action": "分类可保留，但报告和卡片里标为基本面观察，不作为核心优先样本。",
      "theme_purity_score": 72.0,
      "fundamental_score": 41.0,
      "net_profit_yi": -13.88,
      "net_profit_yoy_pct": 0.03,
      "improvement": "仍为亏损，需结合同比和现金流判断是否改善",
      "tags": [
        {
          "name": "电力与公用事业",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "需复核",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "renewable_power",
          "node_name": "电力与公用事业",
          "path": [
            "电力与公用事业"
          ]
        },
        {
          "name": "绿电运营",
          "role": "二级赛道",
          "weight": 0.72,
          "evidence_level": "需复核",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "renewable_operator",
          "node_name": "绿电运营",
          "path": [
            "电力与公用事业",
            "绿电运营"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "绿电运营",
          "role": "二级赛道",
          "weight": 0.72,
          "evidence_level": "需复核",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "renewable_operator",
          "node_name": "绿电运营",
          "path": [
            "电力与公用事业",
            "绿电运营"
          ]
        }
      ]
    },
    {
      "code": "600438",
      "name": "通威股份",
      "batch": "NEXT44",
      "primary_industry": "电力与公用事业",
      "primary_leaf": "绿电运营",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "基本面偏弱 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "financial_weak",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡基本面草分低于 60。"
      ],
      "recommended_action": "分类可保留，但报告和卡片里标为基本面观察，不作为核心优先样本。",
      "theme_purity_score": 86.0,
      "fundamental_score": 41.0,
      "net_profit_yi": -32.14,
      "net_profit_yoy_pct": 0.06,
      "improvement": "仍为亏损，需结合同比和现金流判断是否改善",
      "tags": [
        {
          "name": "电力与公用事业",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "中",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "renewable_power",
          "node_name": "电力与公用事业",
          "path": [
            "电力与公用事业"
          ]
        },
        {
          "name": "绿电运营",
          "role": "二级赛道",
          "weight": 0.82,
          "evidence_level": "中",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "renewable_operator",
          "node_name": "绿电运营",
          "path": [
            "电力与公用事业",
            "绿电运营"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "绿电运营",
          "role": "二级赛道",
          "weight": 0.82,
          "evidence_level": "中",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "renewable_operator",
          "node_name": "绿电运营",
          "path": [
            "电力与公用事业",
            "绿电运营"
          ]
        }
      ]
    },
    {
      "code": "601012",
      "name": "隆基绿能",
      "batch": "NEXT44",
      "primary_industry": "电力与公用事业",
      "primary_leaf": "绿电运营",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "主题分偏低 / 基本面偏弱 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "theme_low",
        "financial_weak",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡主题纯度低于 75，需要观察是否为边缘概念。",
        "轻量证据卡基本面草分低于 60。"
      ],
      "recommended_action": "分类可保留，但报告和卡片里标为基本面观察，不作为核心优先样本。",
      "theme_purity_score": 72.0,
      "fundamental_score": 20.0,
      "net_profit_yi": -19.87,
      "net_profit_yoy_pct": -0.34,
      "improvement": "仍为亏损，需结合同比和现金流判断是否改善",
      "tags": [
        {
          "name": "电力与公用事业",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "需复核",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "renewable_power",
          "node_name": "电力与公用事业",
          "path": [
            "电力与公用事业"
          ]
        },
        {
          "name": "绿电运营",
          "role": "二级赛道",
          "weight": 0.72,
          "evidence_level": "需复核",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "renewable_operator",
          "node_name": "绿电运营",
          "path": [
            "电力与公用事业",
            "绿电运营"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "绿电运营",
          "role": "二级赛道",
          "weight": 0.72,
          "evidence_level": "需复核",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "renewable_operator",
          "node_name": "绿电运营",
          "path": [
            "电力与公用事业",
            "绿电运营"
          ]
        }
      ]
    },
    {
      "code": "603833",
      "name": "欧派家居",
      "batch": "NEXT44",
      "primary_industry": "纺服轻工",
      "primary_leaf": "家居/轻工制造",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "基本面偏弱 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "financial_weak",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡基本面草分低于 60。"
      ],
      "recommended_action": "分类可保留，但报告和卡片里标为基本面观察，不作为核心优先样本。",
      "theme_purity_score": 93.0,
      "fundamental_score": 57.0,
      "net_profit_yi": 1.56,
      "net_profit_yoy_pct": -0.5,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "纺服轻工",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "textile_light_industry",
          "node_name": "纺服轻工",
          "path": [
            "纺服轻工"
          ]
        },
        {
          "name": "家居 / 轻工制造",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "home_light_industry",
          "node_name": "家居/轻工制造",
          "path": [
            "纺服轻工",
            "家居 / 轻工制造"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "家居 / 轻工制造",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "home_light_industry",
          "node_name": "家居/轻工制造",
          "path": [
            "纺服轻工",
            "家居 / 轻工制造"
          ]
        }
      ]
    },
    {
      "code": "300454",
      "name": "深信服",
      "batch": "NEXT44",
      "primary_industry": "计算机 / 软件服务",
      "primary_leaf": "网络安全/信创",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "基本面偏弱 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "financial_weak",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡基本面草分低于 60。"
      ],
      "recommended_action": "分类可保留，但报告和卡片里标为基本面观察，不作为核心优先样本。",
      "theme_purity_score": 93.0,
      "fundamental_score": 41.0,
      "net_profit_yi": -0.65,
      "net_profit_yoy_pct": 0.74,
      "improvement": "仍为亏损，需结合同比和现金流判断是否改善",
      "tags": [
        {
          "name": "计算机 / 软件服务",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "computer_software_it",
          "node_name": "计算机/软件服务",
          "path": [
            "计算机 / 软件服务"
          ]
        },
        {
          "name": "网络安全 / 信创",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "cybersecurity_xinchuang",
          "node_name": "网络安全/信创",
          "path": [
            "计算机/软件服务",
            "网络安全/信创"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "网络安全 / 信创",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "cybersecurity_xinchuang",
          "node_name": "网络安全/信创",
          "path": [
            "计算机/软件服务",
            "网络安全/信创"
          ]
        }
      ]
    },
    {
      "code": "601929",
      "name": "吉视传媒",
      "batch": "P0",
      "primary_industry": "传媒互联网",
      "primary_leaf": "游戏/内容",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "基本面偏弱 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "financial_weak",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡基本面草分低于 60。"
      ],
      "recommended_action": "分类可保留，但报告和卡片里标为基本面观察，不作为核心优先样本。",
      "theme_purity_score": 93.0,
      "fundamental_score": 41.0,
      "net_profit_yi": -1.0,
      "net_profit_yoy_pct": 0.03,
      "improvement": "仍为亏损，需结合同比和现金流判断是否改善",
      "tags": [
        {
          "name": "传媒互联网",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "media_internet",
          "node_name": "传媒互联网",
          "path": [
            "传媒互联网"
          ]
        },
        {
          "name": "广电 / 内容平台",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "game_content",
          "node_name": "游戏/内容",
          "path": [
            "传媒互联网",
            "游戏 / 内容"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "广电 / 内容平台",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "game_content",
          "node_name": "游戏/内容",
          "path": [
            "传媒互联网",
            "游戏 / 内容"
          ]
        }
      ]
    },
    {
      "code": "000002",
      "name": "万科A",
      "batch": "P0",
      "primary_industry": "地产 / 建筑建材",
      "primary_leaf": "房地产开发",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "基本面偏弱 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "financial_weak",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡基本面草分低于 60。"
      ],
      "recommended_action": "分类可保留，但报告和卡片里标为基本面观察，不作为核心优先样本。",
      "theme_purity_score": 95.0,
      "fundamental_score": 41.0,
      "net_profit_yi": -60.92,
      "net_profit_yoy_pct": 0.05,
      "improvement": "仍为亏损，需结合同比和现金流判断是否改善",
      "tags": [
        {
          "name": "地产",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "real_estate_construction",
          "node_name": "地产/建筑建材",
          "path": [
            "地产 / 建筑建材"
          ]
        },
        {
          "name": "房地产开发",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "real_estate_developer",
          "node_name": "房地产开发",
          "path": [
            "地产 / 建筑建材",
            "房地产开发"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "房地产开发",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "real_estate_developer",
          "node_name": "房地产开发",
          "path": [
            "地产 / 建筑建材",
            "房地产开发"
          ]
        }
      ]
    },
    {
      "code": "600170",
      "name": "上海建工",
      "batch": "P0",
      "primary_industry": "地产 / 建筑建材",
      "primary_leaf": "建筑工程/基建",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "基本面偏弱 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "financial_weak",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡基本面草分低于 60。"
      ],
      "recommended_action": "分类可保留，但报告和卡片里标为基本面观察，不作为核心优先样本。",
      "theme_purity_score": 94.0,
      "fundamental_score": 45.0,
      "net_profit_yi": -1.38,
      "net_profit_yoy_pct": 1.01,
      "improvement": "仍为亏损，需结合同比和现金流判断是否改善",
      "tags": [
        {
          "name": "地产",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "real_estate_construction",
          "node_name": "地产/建筑建材",
          "path": [
            "地产 / 建筑建材"
          ]
        },
        {
          "name": "建筑工程 / 基建",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "construction_engineering",
          "node_name": "建筑工程/基建",
          "path": [
            "地产 / 建筑建材",
            "建筑工程 / 基建"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "建筑工程 / 基建",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "construction_engineering",
          "node_name": "建筑工程/基建",
          "path": [
            "地产 / 建筑建材",
            "建筑工程 / 基建"
          ]
        }
      ]
    },
    {
      "code": "600449",
      "name": "宁夏建材",
      "batch": "P0",
      "primary_industry": "地产 / 建筑建材",
      "primary_leaf": "建材/水泥玻璃",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "基本面偏弱 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "financial_weak",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡基本面草分低于 60。"
      ],
      "recommended_action": "分类可保留，但报告和卡片里标为基本面观察，不作为核心优先样本。",
      "theme_purity_score": 94.0,
      "fundamental_score": 41.0,
      "net_profit_yi": -0.06,
      "net_profit_yoy_pct": 0.46,
      "improvement": "仍为亏损，需结合同比和现金流判断是否改善",
      "tags": [
        {
          "name": "地产",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "real_estate_construction",
          "node_name": "地产/建筑建材",
          "path": [
            "地产 / 建筑建材"
          ]
        },
        {
          "name": "建材 / 水泥玻璃",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "building_materials",
          "node_name": "建材/水泥玻璃",
          "path": [
            "地产 / 建筑建材",
            "建材 / 水泥玻璃"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "建材 / 水泥玻璃",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "building_materials",
          "node_name": "建材/水泥玻璃",
          "path": [
            "地产 / 建筑建材",
            "建材 / 水泥玻璃"
          ]
        }
      ]
    },
    {
      "code": "001322",
      "name": "箭牌家居",
      "batch": "P0",
      "primary_industry": "地产 / 建筑建材",
      "primary_leaf": "建材/水泥玻璃",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "基本面偏弱 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "financial_weak",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡基本面草分低于 60。"
      ],
      "recommended_action": "分类可保留，但报告和卡片里标为基本面观察，不作为核心优先样本。",
      "theme_purity_score": 96.0,
      "fundamental_score": 41.0,
      "net_profit_yi": -0.65,
      "net_profit_yoy_pct": 0.12,
      "improvement": "仍为亏损，需结合同比和现金流判断是否改善",
      "tags": [
        {
          "name": "地产",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "real_estate_construction",
          "node_name": "地产/建筑建材",
          "path": [
            "地产 / 建筑建材"
          ]
        },
        {
          "name": "建材 / 家居材料",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "building_materials",
          "node_name": "建材/水泥玻璃",
          "path": [
            "地产 / 建筑建材",
            "建材 / 水泥玻璃"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "建材 / 家居材料",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "building_materials",
          "node_name": "建材/水泥玻璃",
          "path": [
            "地产 / 建筑建材",
            "建材 / 水泥玻璃"
          ]
        }
      ]
    },
    {
      "code": "000539",
      "name": "粤电力A",
      "batch": "P0",
      "primary_industry": "电力与公用事业",
      "primary_leaf": "火电 / 煤电",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "基本面偏弱 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "financial_weak",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡基本面草分低于 60。"
      ],
      "recommended_action": "分类可保留，但报告和卡片里标为基本面观察，不作为核心优先样本。",
      "theme_purity_score": 93.0,
      "fundamental_score": 41.0,
      "net_profit_yi": -1.8,
      "net_profit_yoy_pct": 0.6,
      "improvement": "仍为亏损，需结合同比和现金流判断是否改善",
      "tags": [
        {
          "name": "电力与公用事业",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "renewable_power",
          "node_name": "电力与公用事业",
          "path": [
            "电力与公用事业"
          ]
        },
        {
          "name": "火电 / 绿电运营",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "power_thermal",
          "node_name": "火电 / 煤电",
          "path": [
            "电力与公用事业",
            "火电 / 煤电"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "火电 / 绿电运营",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "power_thermal",
          "node_name": "火电 / 煤电",
          "path": [
            "电力与公用事业",
            "火电 / 煤电"
          ]
        }
      ]
    },
    {
      "code": "002279",
      "name": "久其软件",
      "batch": "P0",
      "primary_industry": "计算机 / 软件服务",
      "primary_leaf": "行业软件/应用软件",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "基本面偏弱 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "financial_weak",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡基本面草分低于 60。"
      ],
      "recommended_action": "分类可保留，但报告和卡片里标为基本面观察，不作为核心优先样本。",
      "theme_purity_score": 95.0,
      "fundamental_score": 41.0,
      "net_profit_yi": -0.94,
      "net_profit_yoy_pct": 0.26,
      "improvement": "仍为亏损，需结合同比和现金流判断是否改善",
      "tags": [
        {
          "name": "计算机",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "computer_software_it",
          "node_name": "计算机/软件服务",
          "path": [
            "计算机 / 软件服务"
          ]
        },
        {
          "name": "行业软件 / 应用软件",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "software_application",
          "node_name": "行业软件/应用软件",
          "path": [
            "计算机 / 软件服务",
            "行业软件 / 应用软件"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "行业软件 / 应用软件",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "software_application",
          "node_name": "行业软件/应用软件",
          "path": [
            "计算机 / 软件服务",
            "行业软件 / 应用软件"
          ]
        }
      ]
    },
    {
      "code": "002232",
      "name": "启明信息",
      "batch": "P0",
      "primary_industry": "计算机 / 软件服务",
      "primary_leaf": "行业软件/应用软件",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "基本面偏弱 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "financial_weak",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡基本面草分低于 60。"
      ],
      "recommended_action": "分类可保留，但报告和卡片里标为基本面观察，不作为核心优先样本。",
      "theme_purity_score": 95.0,
      "fundamental_score": 39.0,
      "net_profit_yi": 0.0,
      "net_profit_yoy_pct": 1.0,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "计算机",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "computer_software_it",
          "node_name": "计算机/软件服务",
          "path": [
            "计算机 / 软件服务"
          ]
        },
        {
          "name": "汽车IT / 车联网",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "software_application",
          "node_name": "行业软件/应用软件",
          "path": [
            "计算机 / 软件服务",
            "行业软件 / 应用软件"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "汽车IT / 车联网",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "software_application",
          "node_name": "行业软件/应用软件",
          "path": [
            "计算机 / 软件服务",
            "行业软件 / 应用软件"
          ]
        }
      ]
    },
    {
      "code": "600588",
      "name": "用友网络",
      "batch": "P0",
      "primary_industry": "计算机 / 软件服务",
      "primary_leaf": "行业软件/应用软件",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "基本面偏弱 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "financial_weak",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡基本面草分低于 60。"
      ],
      "recommended_action": "分类可保留，但报告和卡片里标为基本面观察，不作为核心优先样本。",
      "theme_purity_score": 87.0,
      "fundamental_score": 41.0,
      "net_profit_yi": -7.44,
      "net_profit_yoy_pct": 0.02,
      "improvement": "仍为亏损，需结合同比和现金流判断是否改善",
      "tags": [
        {
          "name": "计算机",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "中",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "computer_software_it",
          "node_name": "计算机/软件服务",
          "path": [
            "计算机 / 软件服务"
          ]
        },
        {
          "name": "行业软件 / 应用软件",
          "role": "二级赛道",
          "weight": 0.82,
          "evidence_level": "中",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "software_application",
          "node_name": "行业软件/应用软件",
          "path": [
            "计算机 / 软件服务",
            "行业软件 / 应用软件"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "行业软件 / 应用软件",
          "role": "二级赛道",
          "weight": 0.82,
          "evidence_level": "中",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "software_application",
          "node_name": "行业软件/应用软件",
          "path": [
            "计算机 / 软件服务",
            "行业软件 / 应用软件"
          ]
        }
      ]
    },
    {
      "code": "600271",
      "name": "航天信息",
      "batch": "P0",
      "primary_industry": "计算机 / 软件服务",
      "primary_leaf": "行业软件/应用软件",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "基本面偏弱 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "financial_weak",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡基本面草分低于 60。"
      ],
      "recommended_action": "分类可保留，但报告和卡片里标为基本面观察，不作为核心优先样本。",
      "theme_purity_score": 87.0,
      "fundamental_score": 41.0,
      "net_profit_yi": -3.72,
      "net_profit_yoy_pct": 0.17,
      "improvement": "仍为亏损，需结合同比和现金流判断是否改善",
      "tags": [
        {
          "name": "计算机",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "中",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "computer_software_it",
          "node_name": "计算机/软件服务",
          "path": [
            "计算机 / 软件服务"
          ]
        },
        {
          "name": "信创 / 电子政务",
          "role": "二级赛道",
          "weight": 0.82,
          "evidence_level": "中",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "software_application",
          "node_name": "行业软件/应用软件",
          "path": [
            "计算机 / 软件服务",
            "行业软件 / 应用软件"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "信创 / 电子政务",
          "role": "二级赛道",
          "weight": 0.82,
          "evidence_level": "中",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "software_application",
          "node_name": "行业软件/应用软件",
          "path": [
            "计算机 / 软件服务",
            "行业软件 / 应用软件"
          ]
        }
      ]
    },
    {
      "code": "600072",
      "name": "中船科技",
      "batch": "P0",
      "primary_industry": "高端装备与工业",
      "primary_leaf": "轨交/船舶/海工装备",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "medium",
      "diagnosis_label": "基本面偏弱 / 收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "financial_weak",
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "轻量证据卡基本面草分低于 60。"
      ],
      "recommended_action": "分类可保留，但报告和卡片里标为基本面观察，不作为核心优先样本。",
      "theme_purity_score": 93.0,
      "fundamental_score": 41.0,
      "net_profit_yi": -2.11,
      "net_profit_yoy_pct": 0.36,
      "improvement": "仍为亏损，需结合同比和现金流判断是否改善",
      "tags": [
        {
          "name": "高端装备与工业",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "industrial_equipment",
          "node_name": "高端装备与工业",
          "path": [
            "高端装备与工业"
          ]
        },
        {
          "name": "轨交 / 船舶 / 海工装备",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "rail_transit_equipment",
          "node_name": "轨交/船舶/海工装备",
          "path": [
            "高端装备与工业",
            "轨交 / 船舶 / 海工装备"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "轨交 / 船舶 / 海工装备",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "rail_transit_equipment",
          "node_name": "轨交/船舶/海工装备",
          "path": [
            "高端装备与工业",
            "轨交 / 船舶 / 海工装备"
          ]
        }
      ]
    },
    {
      "code": "688256",
      "name": "寒武纪",
      "batch": "EXISTING86",
      "primary_industry": "AI算力产业链",
      "primary_leaf": "AI芯片/国产算力芯片",
      "mount_count": 3,
      "root_count": 2,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 85.0,
      "fundamental_score": 94.0,
      "net_profit_yi": 10.13,
      "net_profit_yoy_pct": 1.85,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "AI算力产业链",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "ai_compute",
          "node_name": "AI算力",
          "path": [
            "AI算力"
          ]
        },
        {
          "name": "AI芯片",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "ai_chip",
          "node_name": "AI芯片/国产算力芯片",
          "path": [
            "AI算力",
            "AI芯片/国产算力芯片"
          ]
        },
        {
          "name": "智能计算",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "ai_chip",
          "node_name": "AI芯片/国产算力芯片",
          "path": [
            "AI算力",
            "AI芯片/国产算力芯片"
          ]
        },
        {
          "name": "AI",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "ai_compute",
          "node_name": "AI算力",
          "path": [
            "AI算力"
          ]
        },
        {
          "name": "半导体",
          "role": "相关概念",
          "weight": 0.67,
          "evidence_level": "中",
          "basis": "相关标签",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "算力",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "ai_compute",
          "node_name": "AI算力",
          "path": [
            "AI算力"
          ]
        },
        {
          "name": "国产替代",
          "role": "相关概念",
          "weight": 0.61,
          "evidence_level": "需复核",
          "basis": "主题标签",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "AI芯片",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "ai_chip",
          "node_name": "AI芯片/国产算力芯片",
          "path": [
            "AI算力",
            "AI芯片/国产算力芯片"
          ]
        },
        {
          "name": "智能计算",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "ai_chip",
          "node_name": "AI芯片/国产算力芯片",
          "path": [
            "AI算力",
            "AI芯片/国产算力芯片"
          ]
        }
      ]
    },
    {
      "code": "002463",
      "name": "沪电股份",
      "batch": "EXISTING86",
      "primary_industry": "AI算力产业链",
      "primary_leaf": "AI服务器链",
      "mount_count": 3,
      "root_count": 2,
      "leaf_count": 2,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 86.0,
      "fundamental_score": 90.0,
      "net_profit_yi": 12.41,
      "net_profit_yoy_pct": 0.63,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "AI算力产业链",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "ai_compute",
          "node_name": "AI算力",
          "path": [
            "AI算力"
          ]
        },
        {
          "name": "高速PCB",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "ai_compute",
          "node_name": "AI算力",
          "path": [
            "AI算力"
          ]
        },
        {
          "name": "服务器链",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "ai_server",
          "node_name": "AI服务器链",
          "path": [
            "AI算力",
            "AI服务器链"
          ]
        },
        {
          "name": "AI",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "ai_compute",
          "node_name": "AI算力",
          "path": [
            "AI算力"
          ]
        },
        {
          "name": "PCB",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "ai_compute",
          "node_name": "AI算力",
          "path": [
            "AI算力"
          ]
        },
        {
          "name": "服务器",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "ai_server",
          "node_name": "AI服务器链",
          "path": [
            "AI算力",
            "AI服务器链"
          ]
        },
        {
          "name": "汽车电子",
          "role": "相关概念",
          "weight": 0.77,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "auto_parts",
          "node_name": "汽车零部件",
          "path": [
            "新能源汽车",
            "汽车零部件"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "服务器链",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "ai_server",
          "node_name": "AI服务器链",
          "path": [
            "AI算力",
            "AI服务器链"
          ]
        },
        {
          "name": "服务器",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "ai_server",
          "node_name": "AI服务器链",
          "path": [
            "AI算力",
            "AI服务器链"
          ]
        },
        {
          "name": "汽车电子",
          "role": "相关概念",
          "weight": 0.77,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "auto_parts",
          "node_name": "汽车零部件",
          "path": [
            "新能源汽车",
            "汽车零部件"
          ]
        }
      ]
    },
    {
      "code": "600584",
      "name": "长电科技",
      "batch": "EXISTING86",
      "primary_industry": "AI算力产业链",
      "primary_leaf": "封装测试/先进封装",
      "mount_count": 3,
      "root_count": 2,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 96.0,
      "fundamental_score": 89.0,
      "net_profit_yi": 2.79,
      "net_profit_yoy_pct": 0.43,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "AI算力产业链",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "ai_compute",
          "node_name": "AI算力",
          "path": [
            "AI算力"
          ]
        },
        {
          "name": "半导体",
          "role": "相关概念",
          "weight": 0.67,
          "evidence_level": "中",
          "basis": "相关标签",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "封装测试",
          "role": "相关概念",
          "weight": 0.77,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "advanced_packaging",
          "node_name": "封装测试/先进封装",
          "path": [
            "半导体",
            "封装测试/先进封装"
          ]
        },
        {
          "name": "先进封装",
          "role": "相关概念",
          "weight": 0.77,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "advanced_packaging",
          "node_name": "封装测试/先进封装",
          "path": [
            "半导体",
            "封装测试/先进封装"
          ]
        },
        {
          "name": "Chiplet",
          "role": "相关概念",
          "weight": 0.77,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "advanced_packaging",
          "node_name": "封装测试/先进封装",
          "path": [
            "半导体",
            "封装测试/先进封装"
          ]
        },
        {
          "name": "AI算力",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "ai_compute",
          "node_name": "AI算力",
          "path": [
            "AI算力"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "封装测试",
          "role": "相关概念",
          "weight": 0.77,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "advanced_packaging",
          "node_name": "封装测试/先进封装",
          "path": [
            "半导体",
            "封装测试/先进封装"
          ]
        },
        {
          "name": "先进封装",
          "role": "相关概念",
          "weight": 0.77,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "advanced_packaging",
          "node_name": "封装测试/先进封装",
          "path": [
            "半导体",
            "封装测试/先进封装"
          ]
        },
        {
          "name": "Chiplet",
          "role": "相关概念",
          "weight": 0.77,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "advanced_packaging",
          "node_name": "封装测试/先进封装",
          "path": [
            "半导体",
            "封装测试/先进封装"
          ]
        }
      ]
    },
    {
      "code": "000301",
      "name": "东方盛虹",
      "batch": "EXISTING86",
      "primary_industry": "化工材料",
      "primary_leaf": "石化 / 煤化工",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 95.0,
      "net_profit_yi": 14.48,
      "net_profit_yoy_pct": 3.2,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "化工材料",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "chemical_materials",
          "node_name": "化工材料",
          "path": [
            "化工材料"
          ]
        },
        {
          "name": "石化",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "chemical_petro_coal",
          "node_name": "石化 / 煤化工",
          "path": [
            "化工材料",
            "石化 / 煤化工"
          ]
        },
        {
          "name": "化纤",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "chemical_petro_coal",
          "node_name": "石化 / 煤化工",
          "path": [
            "化工材料",
            "石化 / 煤化工"
          ]
        },
        {
          "name": "化工",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "chemical_petro_coal",
          "node_name": "石化 / 煤化工",
          "path": [
            "化工材料",
            "石化 / 煤化工"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "石化",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "chemical_petro_coal",
          "node_name": "石化 / 煤化工",
          "path": [
            "化工材料",
            "石化 / 煤化工"
          ]
        },
        {
          "name": "化纤",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "chemical_petro_coal",
          "node_name": "石化 / 煤化工",
          "path": [
            "化工材料",
            "石化 / 煤化工"
          ]
        },
        {
          "name": "化工",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "chemical_petro_coal",
          "node_name": "石化 / 煤化工",
          "path": [
            "化工材料",
            "石化 / 煤化工"
          ]
        }
      ]
    },
    {
      "code": "600426",
      "name": "华鲁恒升",
      "batch": "EXISTING86",
      "primary_industry": "化工材料",
      "primary_leaf": "新材料化工",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 86.0,
      "fundamental_score": 90.0,
      "net_profit_yi": 11.94,
      "net_profit_yoy_pct": 0.58,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "化工材料",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "chemical_materials",
          "node_name": "化工材料",
          "path": [
            "化工材料"
          ]
        },
        {
          "name": "煤化工",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "chemical_new_material",
          "node_name": "新材料化工",
          "path": [
            "化工材料",
            "新材料化工"
          ]
        },
        {
          "name": "新材料化工",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "chemical_new_material",
          "node_name": "新材料化工",
          "path": [
            "化工材料",
            "新材料化工"
          ]
        },
        {
          "name": "化工",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "chemical_new_material",
          "node_name": "新材料化工",
          "path": [
            "化工材料",
            "新材料化工"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "煤化工",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "chemical_new_material",
          "node_name": "新材料化工",
          "path": [
            "化工材料",
            "新材料化工"
          ]
        },
        {
          "name": "新材料化工",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "chemical_new_material",
          "node_name": "新材料化工",
          "path": [
            "化工材料",
            "新材料化工"
          ]
        },
        {
          "name": "化工",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "chemical_new_material",
          "node_name": "新材料化工",
          "path": [
            "化工材料",
            "新材料化工"
          ]
        }
      ]
    },
    {
      "code": "002648",
      "name": "卫星化学",
      "batch": "EXISTING86",
      "primary_industry": "化工材料",
      "primary_leaf": "新材料化工",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 85.0,
      "fundamental_score": 95.0,
      "net_profit_yi": 21.17,
      "net_profit_yoy_pct": 0.35,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "化工材料",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "chemical_materials",
          "node_name": "化工材料",
          "path": [
            "化工材料"
          ]
        },
        {
          "name": "烯烃",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "chemical_new_material",
          "node_name": "新材料化工",
          "path": [
            "化工材料",
            "新材料化工"
          ]
        },
        {
          "name": "新材料化工",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "chemical_new_material",
          "node_name": "新材料化工",
          "path": [
            "化工材料",
            "新材料化工"
          ]
        },
        {
          "name": "化工",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "chemical_new_material",
          "node_name": "新材料化工",
          "path": [
            "化工材料",
            "新材料化工"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "烯烃",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "chemical_new_material",
          "node_name": "新材料化工",
          "path": [
            "化工材料",
            "新材料化工"
          ]
        },
        {
          "name": "新材料化工",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "chemical_new_material",
          "node_name": "新材料化工",
          "path": [
            "化工材料",
            "新材料化工"
          ]
        },
        {
          "name": "化工",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "chemical_new_material",
          "node_name": "新材料化工",
          "path": [
            "化工材料",
            "新材料化工"
          ]
        }
      ]
    },
    {
      "code": "002493",
      "name": "荣盛石化",
      "batch": "EXISTING86",
      "primary_industry": "化工材料",
      "primary_leaf": "石化 / 煤化工",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 85.0,
      "fundamental_score": 95.0,
      "net_profit_yi": 50.12,
      "net_profit_yoy_pct": 3.78,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "化工材料",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "chemical_materials",
          "node_name": "化工材料",
          "path": [
            "化工材料"
          ]
        },
        {
          "name": "石化",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "chemical_petro_coal",
          "node_name": "石化 / 煤化工",
          "path": [
            "化工材料",
            "石化 / 煤化工"
          ]
        },
        {
          "name": "炼化一体化",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "chemical_petro_coal",
          "node_name": "石化 / 煤化工",
          "path": [
            "化工材料",
            "石化 / 煤化工"
          ]
        },
        {
          "name": "化工",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "chemical_petro_coal",
          "node_name": "石化 / 煤化工",
          "path": [
            "化工材料",
            "石化 / 煤化工"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "石化",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "chemical_petro_coal",
          "node_name": "石化 / 煤化工",
          "path": [
            "化工材料",
            "石化 / 煤化工"
          ]
        },
        {
          "name": "炼化一体化",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "chemical_petro_coal",
          "node_name": "石化 / 煤化工",
          "path": [
            "化工材料",
            "石化 / 煤化工"
          ]
        },
        {
          "name": "化工",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "chemical_petro_coal",
          "node_name": "石化 / 煤化工",
          "path": [
            "化工材料",
            "石化 / 煤化工"
          ]
        }
      ]
    },
    {
      "code": "601607",
      "name": "上海医药",
      "batch": "EXISTING86",
      "primary_industry": "医药医疗",
      "primary_leaf": "医药商业/药房",
      "mount_count": 2,
      "root_count": 2,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 86.0,
      "fundamental_score": 84.0,
      "net_profit_yi": 18.5,
      "net_profit_yoy_pct": 0.06,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "医药医疗",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        },
        {
          "name": "医药商业",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "pharma_distribution",
          "node_name": "医药商业/药房",
          "path": [
            "医药医疗",
            "医药商业/药房"
          ]
        },
        {
          "name": "综合医药",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        },
        {
          "name": "医药",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        },
        {
          "name": "医药流通",
          "role": "相关概念",
          "weight": 0.75,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "pharma_distribution",
          "node_name": "医药商业/药房",
          "path": [
            "医药医疗",
            "医药商业/药房"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "医药商业",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "pharma_distribution",
          "node_name": "医药商业/药房",
          "path": [
            "医药医疗",
            "医药商业/药房"
          ]
        },
        {
          "name": "医药流通",
          "role": "相关概念",
          "weight": 0.75,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "pharma_distribution",
          "node_name": "医药商业/药房",
          "path": [
            "医药医疗",
            "医药商业/药房"
          ]
        }
      ]
    },
    {
      "code": "000538",
      "name": "云南白药",
      "batch": "EXISTING86",
      "primary_industry": "医药医疗",
      "primary_leaf": "中药/品牌中药",
      "mount_count": 2,
      "root_count": 2,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 85.0,
      "fundamental_score": 95.0,
      "net_profit_yi": 20.31,
      "net_profit_yoy_pct": 0.05,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "医药医疗",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        },
        {
          "name": "中药",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "tcm",
          "node_name": "中药/品牌中药",
          "path": [
            "医药医疗",
            "中药/品牌中药"
          ]
        },
        {
          "name": "品牌中药",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "tcm",
          "node_name": "中药/品牌中药",
          "path": [
            "医药医疗",
            "中药/品牌中药"
          ]
        },
        {
          "name": "医药",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "中药",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "tcm",
          "node_name": "中药/品牌中药",
          "path": [
            "医药医疗",
            "中药/品牌中药"
          ]
        },
        {
          "name": "品牌中药",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "tcm",
          "node_name": "中药/品牌中药",
          "path": [
            "医药医疗",
            "中药/品牌中药"
          ]
        }
      ]
    },
    {
      "code": "002603",
      "name": "以岭药业",
      "batch": "EXISTING86",
      "primary_industry": "医药医疗",
      "primary_leaf": "中药/品牌中药",
      "mount_count": 3,
      "root_count": 2,
      "leaf_count": 2,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 86.0,
      "fundamental_score": 89.0,
      "net_profit_yi": 4.09,
      "net_profit_yoy_pct": 0.25,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "医药医疗",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        },
        {
          "name": "中药",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "tcm",
          "node_name": "中药/品牌中药",
          "path": [
            "医药医疗",
            "中药/品牌中药"
          ]
        },
        {
          "name": "创新中药",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "tcm",
          "node_name": "中药/品牌中药",
          "path": [
            "医药医疗",
            "中药/品牌中药"
          ]
        },
        {
          "name": "医药",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        },
        {
          "name": "创新药",
          "role": "相关概念",
          "weight": 0.75,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "innovative_drug",
          "node_name": "创新药/生物药",
          "path": [
            "医药医疗",
            "创新药/生物药"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "中药",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "tcm",
          "node_name": "中药/品牌中药",
          "path": [
            "医药医疗",
            "中药/品牌中药"
          ]
        },
        {
          "name": "创新中药",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "tcm",
          "node_name": "中药/品牌中药",
          "path": [
            "医药医疗",
            "中药/品牌中药"
          ]
        },
        {
          "name": "创新药",
          "role": "相关概念",
          "weight": 0.75,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "innovative_drug",
          "node_name": "创新药/生物药",
          "path": [
            "医药医疗",
            "创新药/生物药"
          ]
        }
      ]
    },
    {
      "code": "002821",
      "name": "凯莱英",
      "batch": "EXISTING86",
      "primary_industry": "医药医疗",
      "primary_leaf": "CXO/CRO/CDMO",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 67.0,
      "net_profit_yi": 3.01,
      "net_profit_yoy_pct": -0.07,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "医药医疗",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        },
        {
          "name": "CXO",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "cxo",
          "node_name": "CXO/CRO/CDMO",
          "path": [
            "医药医疗",
            "CXO/CRO/CDMO"
          ]
        },
        {
          "name": "CDMO",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "cxo",
          "node_name": "CXO/CRO/CDMO",
          "path": [
            "医药医疗",
            "CXO/CRO/CDMO"
          ]
        },
        {
          "name": "医药",
          "role": "核心概念",
          "weight": 0.83,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        },
        {
          "name": "医药外包",
          "role": "相关概念",
          "weight": 0.69,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "cxo",
          "node_name": "CXO/CRO/CDMO",
          "path": [
            "医药医疗",
            "CXO/CRO/CDMO"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "CXO",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "cxo",
          "node_name": "CXO/CRO/CDMO",
          "path": [
            "医药医疗",
            "CXO/CRO/CDMO"
          ]
        },
        {
          "name": "CDMO",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "cxo",
          "node_name": "CXO/CRO/CDMO",
          "path": [
            "医药医疗",
            "CXO/CRO/CDMO"
          ]
        },
        {
          "name": "医药外包",
          "role": "相关概念",
          "weight": 0.69,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "cxo",
          "node_name": "CXO/CRO/CDMO",
          "path": [
            "医药医疗",
            "CXO/CRO/CDMO"
          ]
        }
      ]
    },
    {
      "code": "000963",
      "name": "华东医药",
      "batch": "EXISTING86",
      "primary_industry": "医药医疗",
      "primary_leaf": "创新药/生物药",
      "mount_count": 3,
      "root_count": 1,
      "leaf_count": 2,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 85.0,
      "fundamental_score": 84.0,
      "net_profit_yi": 9.97,
      "net_profit_yoy_pct": 0.1,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "医药医疗",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        },
        {
          "name": "创新药",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "innovative_drug",
          "node_name": "创新药/生物药",
          "path": [
            "医药医疗",
            "创新药/生物药"
          ]
        },
        {
          "name": "生物药",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "innovative_drug",
          "node_name": "创新药/生物药",
          "path": [
            "医药医疗",
            "创新药/生物药"
          ]
        },
        {
          "name": "医美",
          "role": "二级赛道",
          "weight": 0.78,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "medical_service",
          "node_name": "医疗服务",
          "path": [
            "医药医疗",
            "医疗服务"
          ]
        },
        {
          "name": "医药",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        },
        {
          "name": "医疗服务",
          "role": "相关概念",
          "weight": 0.75,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "medical_service",
          "node_name": "医疗服务",
          "path": [
            "医药医疗",
            "医疗服务"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "创新药",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "innovative_drug",
          "node_name": "创新药/生物药",
          "path": [
            "医药医疗",
            "创新药/生物药"
          ]
        },
        {
          "name": "生物药",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "innovative_drug",
          "node_name": "创新药/生物药",
          "path": [
            "医药医疗",
            "创新药/生物药"
          ]
        },
        {
          "name": "医美",
          "role": "二级赛道",
          "weight": 0.78,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "medical_service",
          "node_name": "医疗服务",
          "path": [
            "医药医疗",
            "医疗服务"
          ]
        },
        {
          "name": "医疗服务",
          "role": "相关概念",
          "weight": 0.75,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "medical_service",
          "node_name": "医疗服务",
          "path": [
            "医药医疗",
            "医疗服务"
          ]
        }
      ]
    },
    {
      "code": "600085",
      "name": "同仁堂",
      "batch": "EXISTING86",
      "primary_industry": "医药医疗",
      "primary_leaf": "中药/品牌中药",
      "mount_count": 2,
      "root_count": 2,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 86.0,
      "fundamental_score": 72.0,
      "net_profit_yi": 6.17,
      "net_profit_yoy_pct": -0.19,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "医药医疗",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        },
        {
          "name": "中药",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "tcm",
          "node_name": "中药/品牌中药",
          "path": [
            "医药医疗",
            "中药/品牌中药"
          ]
        },
        {
          "name": "品牌中药",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "tcm",
          "node_name": "中药/品牌中药",
          "path": [
            "医药医疗",
            "中药/品牌中药"
          ]
        },
        {
          "name": "医药",
          "role": "核心概念",
          "weight": 0.83,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "中药",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "tcm",
          "node_name": "中药/品牌中药",
          "path": [
            "医药医疗",
            "中药/品牌中药"
          ]
        },
        {
          "name": "品牌中药",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "tcm",
          "node_name": "中药/品牌中药",
          "path": [
            "医药医疗",
            "中药/品牌中药"
          ]
        }
      ]
    },
    {
      "code": "600196",
      "name": "复星医药",
      "batch": "EXISTING86",
      "primary_industry": "医药医疗",
      "primary_leaf": "创新药/生物药",
      "mount_count": 3,
      "root_count": 1,
      "leaf_count": 2,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 86.0,
      "fundamental_score": 94.0,
      "net_profit_yi": 10.8,
      "net_profit_yoy_pct": 0.14,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "医药医疗",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        },
        {
          "name": "创新药",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "innovative_drug",
          "node_name": "创新药/生物药",
          "path": [
            "医药医疗",
            "创新药/生物药"
          ]
        },
        {
          "name": "生物药",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "innovative_drug",
          "node_name": "创新药/生物药",
          "path": [
            "医药医疗",
            "创新药/生物药"
          ]
        },
        {
          "name": "综合医药",
          "role": "二级赛道",
          "weight": 0.78,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        },
        {
          "name": "医药",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        },
        {
          "name": "医疗器械",
          "role": "相关概念",
          "weight": 0.75,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "medical_device",
          "node_name": "医疗器械/IVD",
          "path": [
            "医药医疗",
            "医疗器械/IVD"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "创新药",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "innovative_drug",
          "node_name": "创新药/生物药",
          "path": [
            "医药医疗",
            "创新药/生物药"
          ]
        },
        {
          "name": "生物药",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "innovative_drug",
          "node_name": "创新药/生物药",
          "path": [
            "医药医疗",
            "创新药/生物药"
          ]
        },
        {
          "name": "医疗器械",
          "role": "相关概念",
          "weight": 0.75,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "medical_device",
          "node_name": "医疗器械/IVD",
          "path": [
            "医药医疗",
            "医疗器械/IVD"
          ]
        }
      ]
    },
    {
      "code": "603233",
      "name": "大参林",
      "batch": "EXISTING86",
      "primary_industry": "医药医疗",
      "primary_leaf": "医药商业/药房",
      "mount_count": 2,
      "root_count": 2,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 85.0,
      "fundamental_score": 94.0,
      "net_profit_yi": 5.44,
      "net_profit_yoy_pct": 0.11,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "医药医疗",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        },
        {
          "name": "医药商业",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "pharma_distribution",
          "node_name": "医药商业/药房",
          "path": [
            "医药医疗",
            "医药商业/药房"
          ]
        },
        {
          "name": "药房",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "pharma_distribution",
          "node_name": "医药商业/药房",
          "path": [
            "医药医疗",
            "医药商业/药房"
          ]
        },
        {
          "name": "医药",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "医药商业",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "pharma_distribution",
          "node_name": "医药商业/药房",
          "path": [
            "医药医疗",
            "医药商业/药房"
          ]
        },
        {
          "name": "药房",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "pharma_distribution",
          "node_name": "医药商业/药房",
          "path": [
            "医药医疗",
            "医药商业/药房"
          ]
        }
      ]
    },
    {
      "code": "300759",
      "name": "康龙化成",
      "batch": "EXISTING86",
      "primary_industry": "医药医疗",
      "primary_leaf": "CXO/CRO/CDMO",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 89.0,
      "net_profit_yi": 3.1,
      "net_profit_yoy_pct": 0.1,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "医药医疗",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        },
        {
          "name": "CXO",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "cxo",
          "node_name": "CXO/CRO/CDMO",
          "path": [
            "医药医疗",
            "CXO/CRO/CDMO"
          ]
        },
        {
          "name": "CRO",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "cxo",
          "node_name": "CXO/CRO/CDMO",
          "path": [
            "医药医疗",
            "CXO/CRO/CDMO"
          ]
        },
        {
          "name": "CDMO",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "cxo",
          "node_name": "CXO/CRO/CDMO",
          "path": [
            "医药医疗",
            "CXO/CRO/CDMO"
          ]
        },
        {
          "name": "医药",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "CXO",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "cxo",
          "node_name": "CXO/CRO/CDMO",
          "path": [
            "医药医疗",
            "CXO/CRO/CDMO"
          ]
        },
        {
          "name": "CRO",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "cxo",
          "node_name": "CXO/CRO/CDMO",
          "path": [
            "医药医疗",
            "CXO/CRO/CDMO"
          ]
        },
        {
          "name": "CDMO",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "cxo",
          "node_name": "CXO/CRO/CDMO",
          "path": [
            "医药医疗",
            "CXO/CRO/CDMO"
          ]
        }
      ]
    },
    {
      "code": "600276",
      "name": "恒瑞医药",
      "batch": "EXISTING86",
      "primary_industry": "医药医疗",
      "primary_leaf": "创新药/生物药",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 85.0,
      "fundamental_score": 95.0,
      "net_profit_yi": 22.81,
      "net_profit_yoy_pct": 0.22,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "医药医疗",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        },
        {
          "name": "创新药",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "innovative_drug",
          "node_name": "创新药/生物药",
          "path": [
            "医药医疗",
            "创新药/生物药"
          ]
        },
        {
          "name": "化学制药",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "innovative_drug",
          "node_name": "创新药/生物药",
          "path": [
            "医药医疗",
            "创新药/生物药"
          ]
        },
        {
          "name": "医药",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        },
        {
          "name": "医疗",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "创新药",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "innovative_drug",
          "node_name": "创新药/生物药",
          "path": [
            "医药医疗",
            "创新药/生物药"
          ]
        },
        {
          "name": "化学制药",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "innovative_drug",
          "node_name": "创新药/生物药",
          "path": [
            "医药医疗",
            "创新药/生物药"
          ]
        }
      ]
    },
    {
      "code": "300832",
      "name": "新产业",
      "batch": "EXISTING86",
      "primary_industry": "医药医疗",
      "primary_leaf": "医疗器械/IVD",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 94.0,
      "fundamental_score": 85.0,
      "net_profit_yi": 4.42,
      "net_profit_yoy_pct": 0.01,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "医药医疗",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        },
        {
          "name": "医疗器械",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "medical_device",
          "node_name": "医疗器械/IVD",
          "path": [
            "医药医疗",
            "医疗器械/IVD"
          ]
        },
        {
          "name": "IVD",
          "role": "核心概念",
          "weight": 0.86,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "medical_device",
          "node_name": "医疗器械/IVD",
          "path": [
            "医药医疗",
            "医疗器械/IVD"
          ]
        },
        {
          "name": "医药",
          "role": "核心概念",
          "weight": 0.86,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        },
        {
          "name": "体外诊断",
          "role": "相关概念",
          "weight": 0.72,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "medical_device",
          "node_name": "医疗器械/IVD",
          "path": [
            "医药医疗",
            "医疗器械/IVD"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "医疗器械",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "medical_device",
          "node_name": "医疗器械/IVD",
          "path": [
            "医药医疗",
            "医疗器械/IVD"
          ]
        },
        {
          "name": "IVD",
          "role": "核心概念",
          "weight": 0.86,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "medical_device",
          "node_name": "医疗器械/IVD",
          "path": [
            "医药医疗",
            "医疗器械/IVD"
          ]
        },
        {
          "name": "体外诊断",
          "role": "相关概念",
          "weight": 0.72,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "medical_device",
          "node_name": "医疗器械/IVD",
          "path": [
            "医药医疗",
            "医疗器械/IVD"
          ]
        }
      ]
    },
    {
      "code": "300015",
      "name": "爱尔眼科",
      "batch": "EXISTING86",
      "primary_industry": "医药医疗",
      "primary_leaf": "医疗服务",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 94.0,
      "fundamental_score": 94.0,
      "net_profit_yi": 13.15,
      "net_profit_yoy_pct": 0.12,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "医药医疗",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        },
        {
          "name": "医疗服务",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "medical_service",
          "node_name": "医疗服务",
          "path": [
            "医药医疗",
            "医疗服务"
          ]
        },
        {
          "name": "眼科",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "medical_service",
          "node_name": "医疗服务",
          "path": [
            "医药医疗",
            "医疗服务"
          ]
        },
        {
          "name": "医药",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "医疗服务",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "medical_service",
          "node_name": "医疗服务",
          "path": [
            "医药医疗",
            "医疗服务"
          ]
        },
        {
          "name": "眼科",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "medical_service",
          "node_name": "医疗服务",
          "path": [
            "医药医疗",
            "医疗服务"
          ]
        }
      ]
    },
    {
      "code": "300896",
      "name": "爱美客",
      "batch": "EXISTING86",
      "primary_industry": "医药医疗",
      "primary_leaf": "医疗服务",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 86.0,
      "fundamental_score": 63.0,
      "net_profit_yi": 3.01,
      "net_profit_yoy_pct": -0.33,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "医药医疗",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        },
        {
          "name": "医疗服务",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "medical_service",
          "node_name": "医疗服务",
          "path": [
            "医药医疗",
            "医疗服务"
          ]
        },
        {
          "name": "医美",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "medical_service",
          "node_name": "医疗服务",
          "path": [
            "医药医疗",
            "医疗服务"
          ]
        },
        {
          "name": "医药",
          "role": "核心概念",
          "weight": 0.8,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "医疗服务",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "medical_service",
          "node_name": "医疗服务",
          "path": [
            "医药医疗",
            "医疗服务"
          ]
        },
        {
          "name": "医美",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "medical_service",
          "node_name": "医疗服务",
          "path": [
            "医药医疗",
            "医疗服务"
          ]
        }
      ]
    },
    {
      "code": "600436",
      "name": "片仔癀",
      "batch": "EXISTING86",
      "primary_industry": "医药医疗",
      "primary_leaf": "中药/品牌中药",
      "mount_count": 2,
      "root_count": 2,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 86.0,
      "fundamental_score": 72.0,
      "net_profit_yi": 7.4,
      "net_profit_yoy_pct": -0.26,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "医药医疗",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        },
        {
          "name": "中药",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "tcm",
          "node_name": "中药/品牌中药",
          "path": [
            "医药医疗",
            "中药/品牌中药"
          ]
        },
        {
          "name": "品牌中药",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "tcm",
          "node_name": "中药/品牌中药",
          "path": [
            "医药医疗",
            "中药/品牌中药"
          ]
        },
        {
          "name": "医药",
          "role": "核心概念",
          "weight": 0.83,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "中药",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "tcm",
          "node_name": "中药/品牌中药",
          "path": [
            "医药医疗",
            "中药/品牌中药"
          ]
        },
        {
          "name": "品牌中药",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "tcm",
          "node_name": "中药/品牌中药",
          "path": [
            "医药医疗",
            "中药/品牌中药"
          ]
        }
      ]
    },
    {
      "code": "603939",
      "name": "益丰药房",
      "batch": "EXISTING86",
      "primary_industry": "医药医疗",
      "primary_leaf": "医药商业/药房",
      "mount_count": 2,
      "root_count": 2,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 85.0,
      "fundamental_score": 94.0,
      "net_profit_yi": 5.46,
      "net_profit_yoy_pct": 0.11,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "医药医疗",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        },
        {
          "name": "医药商业",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "pharma_distribution",
          "node_name": "医药商业/药房",
          "path": [
            "医药医疗",
            "医药商业/药房"
          ]
        },
        {
          "name": "药房",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "pharma_distribution",
          "node_name": "医药商业/药房",
          "path": [
            "医药医疗",
            "医药商业/药房"
          ]
        },
        {
          "name": "医药",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "医药商业",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "pharma_distribution",
          "node_name": "医药商业/药房",
          "path": [
            "医药医疗",
            "医药商业/药房"
          ]
        },
        {
          "name": "药房",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "pharma_distribution",
          "node_name": "医药商业/药房",
          "path": [
            "医药医疗",
            "医药商业/药房"
          ]
        }
      ]
    },
    {
      "code": "603883",
      "name": "老百姓",
      "batch": "EXISTING86",
      "primary_industry": "医药医疗",
      "primary_leaf": "医药商业/药房",
      "mount_count": 2,
      "root_count": 2,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 85.0,
      "fundamental_score": 89.0,
      "net_profit_yi": 3.02,
      "net_profit_yoy_pct": 0.05,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "医药医疗",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        },
        {
          "name": "医药商业",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "pharma_distribution",
          "node_name": "医药商业/药房",
          "path": [
            "医药医疗",
            "医药商业/药房"
          ]
        },
        {
          "name": "药房",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "pharma_distribution",
          "node_name": "医药商业/药房",
          "path": [
            "医药医疗",
            "医药商业/药房"
          ]
        },
        {
          "name": "医药",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "医药商业",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "pharma_distribution",
          "node_name": "医药商业/药房",
          "path": [
            "医药医疗",
            "医药商业/药房"
          ]
        },
        {
          "name": "药房",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "pharma_distribution",
          "node_name": "医药商业/药房",
          "path": [
            "医药医疗",
            "医药商业/药房"
          ]
        }
      ]
    },
    {
      "code": "688271",
      "name": "联影医疗",
      "batch": "EXISTING86",
      "primary_industry": "医药医疗",
      "primary_leaf": "医疗器械/IVD",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 94.0,
      "fundamental_score": 79.0,
      "net_profit_yi": 3.92,
      "net_profit_yoy_pct": 0.08,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "医药医疗",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        },
        {
          "name": "医疗器械",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "medical_device",
          "node_name": "医疗器械/IVD",
          "path": [
            "医药医疗",
            "医疗器械/IVD"
          ]
        },
        {
          "name": "医学影像",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "medical_device",
          "node_name": "医疗器械/IVD",
          "path": [
            "医药医疗",
            "医疗器械/IVD"
          ]
        },
        {
          "name": "医药",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        },
        {
          "name": "医疗设备",
          "role": "相关概念",
          "weight": 0.75,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "medical_device",
          "node_name": "医疗器械/IVD",
          "path": [
            "医药医疗",
            "医疗器械/IVD"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "医疗器械",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "medical_device",
          "node_name": "医疗器械/IVD",
          "path": [
            "医药医疗",
            "医疗器械/IVD"
          ]
        },
        {
          "name": "医学影像",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "medical_device",
          "node_name": "医疗器械/IVD",
          "path": [
            "医药医疗",
            "医疗器械/IVD"
          ]
        },
        {
          "name": "医疗设备",
          "role": "相关概念",
          "weight": 0.75,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "medical_device",
          "node_name": "医疗器械/IVD",
          "path": [
            "医药医疗",
            "医疗器械/IVD"
          ]
        }
      ]
    },
    {
      "code": "603259",
      "name": "药明康德",
      "batch": "EXISTING86",
      "primary_industry": "医药医疗",
      "primary_leaf": "CXO/CRO/CDMO",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 86.0,
      "fundamental_score": 95.0,
      "net_profit_yi": 46.71,
      "net_profit_yoy_pct": 0.27,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "医药医疗",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        },
        {
          "name": "CXO",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "cxo",
          "node_name": "CXO/CRO/CDMO",
          "path": [
            "医药医疗",
            "CXO/CRO/CDMO"
          ]
        },
        {
          "name": "CRO",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "cxo",
          "node_name": "CXO/CRO/CDMO",
          "path": [
            "医药医疗",
            "CXO/CRO/CDMO"
          ]
        },
        {
          "name": "CDMO",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "cxo",
          "node_name": "CXO/CRO/CDMO",
          "path": [
            "医药医疗",
            "CXO/CRO/CDMO"
          ]
        },
        {
          "name": "医药",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "CXO",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "cxo",
          "node_name": "CXO/CRO/CDMO",
          "path": [
            "医药医疗",
            "CXO/CRO/CDMO"
          ]
        },
        {
          "name": "CRO",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "cxo",
          "node_name": "CXO/CRO/CDMO",
          "path": [
            "医药医疗",
            "CXO/CRO/CDMO"
          ]
        },
        {
          "name": "CDMO",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "cxo",
          "node_name": "CXO/CRO/CDMO",
          "path": [
            "医药医疗",
            "CXO/CRO/CDMO"
          ]
        }
      ]
    },
    {
      "code": "300760",
      "name": "迈瑞医疗",
      "batch": "EXISTING86",
      "primary_industry": "医药医疗",
      "primary_leaf": "医疗器械/IVD",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 95.0,
      "fundamental_score": 73.0,
      "net_profit_yi": 24.27,
      "net_profit_yoy_pct": -0.11,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "医药医疗",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        },
        {
          "name": "医疗器械",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "medical_device",
          "node_name": "医疗器械/IVD",
          "path": [
            "医药医疗",
            "医疗器械/IVD"
          ]
        },
        {
          "name": "高端设备",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "medical_device",
          "node_name": "医疗器械/IVD",
          "path": [
            "医药医疗",
            "医疗器械/IVD"
          ]
        },
        {
          "name": "医药",
          "role": "核心概念",
          "weight": 0.83,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        },
        {
          "name": "医疗",
          "role": "核心概念",
          "weight": 0.83,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "医疗器械",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "medical_device",
          "node_name": "医疗器械/IVD",
          "path": [
            "医药医疗",
            "医疗器械/IVD"
          ]
        },
        {
          "name": "高端设备",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "medical_device",
          "node_name": "医疗器械/IVD",
          "path": [
            "医药医疗",
            "医疗器械/IVD"
          ]
        }
      ]
    },
    {
      "code": "600763",
      "name": "通策医疗",
      "batch": "EXISTING86",
      "primary_industry": "医药医疗",
      "primary_leaf": "医疗服务",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 85.0,
      "net_profit_yi": 2.21,
      "net_profit_yoy_pct": 0.02,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "医药医疗",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        },
        {
          "name": "医疗服务",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "medical_service",
          "node_name": "医疗服务",
          "path": [
            "医药医疗",
            "医疗服务"
          ]
        },
        {
          "name": "口腔",
          "role": "核心概念",
          "weight": 0.86,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "medical_service",
          "node_name": "医疗服务",
          "path": [
            "医药医疗",
            "医疗服务"
          ]
        },
        {
          "name": "医药",
          "role": "核心概念",
          "weight": 0.86,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "医疗服务",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "medical_service",
          "node_name": "医疗服务",
          "path": [
            "医药医疗",
            "医疗服务"
          ]
        },
        {
          "name": "口腔",
          "role": "核心概念",
          "weight": 0.86,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "medical_service",
          "node_name": "医疗服务",
          "path": [
            "医药医疗",
            "医疗服务"
          ]
        }
      ]
    },
    {
      "code": "000661",
      "name": "长春高新",
      "batch": "EXISTING86",
      "primary_industry": "医药医疗",
      "primary_leaf": "创新药/生物药",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 67.0,
      "net_profit_yi": 2.71,
      "net_profit_yoy_pct": -0.42,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "医药医疗",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        },
        {
          "name": "创新药",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "innovative_drug",
          "node_name": "创新药/生物药",
          "path": [
            "医药医疗",
            "创新药/生物药"
          ]
        },
        {
          "name": "生物药",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "innovative_drug",
          "node_name": "创新药/生物药",
          "path": [
            "医药医疗",
            "创新药/生物药"
          ]
        },
        {
          "name": "生物制药",
          "role": "核心概念",
          "weight": 0.8,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "innovative_drug",
          "node_name": "创新药/生物药",
          "path": [
            "医药医疗",
            "创新药/生物药"
          ]
        },
        {
          "name": "医药",
          "role": "核心概念",
          "weight": 0.8,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "创新药",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "innovative_drug",
          "node_name": "创新药/生物药",
          "path": [
            "医药医疗",
            "创新药/生物药"
          ]
        },
        {
          "name": "生物药",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "innovative_drug",
          "node_name": "创新药/生物药",
          "path": [
            "医药医疗",
            "创新药/生物药"
          ]
        },
        {
          "name": "生物制药",
          "role": "核心概念",
          "weight": 0.8,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "innovative_drug",
          "node_name": "创新药/生物药",
          "path": [
            "医药医疗",
            "创新药/生物药"
          ]
        }
      ]
    },
    {
      "code": "002223",
      "name": "鱼跃医疗",
      "batch": "EXISTING86",
      "primary_industry": "医药医疗",
      "primary_leaf": "医疗器械/IVD",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 67.0,
      "net_profit_yi": 4.29,
      "net_profit_yoy_pct": -0.31,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "医药医疗",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        },
        {
          "name": "医疗器械",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "medical_device",
          "node_name": "医疗器械/IVD",
          "path": [
            "医药医疗",
            "医疗器械/IVD"
          ]
        },
        {
          "name": "家用医疗",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        },
        {
          "name": "医药",
          "role": "核心概念",
          "weight": 0.8,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "healthcare",
          "node_name": "医药医疗",
          "path": [
            "医药医疗"
          ]
        },
        {
          "name": "医疗设备",
          "role": "相关概念",
          "weight": 0.66,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "medical_device",
          "node_name": "医疗器械/IVD",
          "path": [
            "医药医疗",
            "医疗器械/IVD"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "医疗器械",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "medical_device",
          "node_name": "医疗器械/IVD",
          "path": [
            "医药医疗",
            "医疗器械/IVD"
          ]
        },
        {
          "name": "医疗设备",
          "role": "相关概念",
          "weight": 0.66,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "medical_device",
          "node_name": "医疗器械/IVD",
          "path": [
            "医药医疗",
            "医疗器械/IVD"
          ]
        }
      ]
    },
    {
      "code": "688012",
      "name": "中微公司",
      "batch": "EXISTING86",
      "primary_industry": "半导体产业链",
      "primary_leaf": "半导体设备",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 94.0,
      "fundamental_score": 88.0,
      "net_profit_yi": 9.18,
      "net_profit_yoy_pct": 1.97,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "半导体产业链",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "半导体设备",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "semicap",
          "node_name": "半导体设备",
          "path": [
            "半导体",
            "半导体设备"
          ]
        },
        {
          "name": "刻蚀",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "semicap",
          "node_name": "半导体设备",
          "path": [
            "半导体",
            "半导体设备"
          ]
        },
        {
          "name": "MOCVD",
          "role": "二级赛道",
          "weight": 0.78,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "semicap",
          "node_name": "半导体设备",
          "path": [
            "半导体",
            "半导体设备"
          ]
        },
        {
          "name": "半导体",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "国产替代",
          "role": "相关概念",
          "weight": 0.61,
          "evidence_level": "需复核",
          "basis": "主题标签",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "设备",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "semicap",
          "node_name": "半导体设备",
          "path": [
            "半导体",
            "半导体设备"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "半导体设备",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "semicap",
          "node_name": "半导体设备",
          "path": [
            "半导体",
            "半导体设备"
          ]
        },
        {
          "name": "刻蚀",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "semicap",
          "node_name": "半导体设备",
          "path": [
            "半导体",
            "半导体设备"
          ]
        },
        {
          "name": "MOCVD",
          "role": "二级赛道",
          "weight": 0.78,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "semicap",
          "node_name": "半导体设备",
          "path": [
            "半导体",
            "半导体设备"
          ]
        },
        {
          "name": "设备",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "semicap",
          "node_name": "半导体设备",
          "path": [
            "半导体",
            "半导体设备"
          ]
        }
      ]
    },
    {
      "code": "603986",
      "name": "兆易创新",
      "batch": "EXISTING86",
      "primary_industry": "半导体产业链",
      "primary_leaf": "模拟/功率/MCU",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 95.0,
      "net_profit_yi": 14.73,
      "net_profit_yoy_pct": 5.23,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "半导体产业链",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "存储芯片",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "MCU",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "半导体",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "芯片",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "MCU",
          "role": "核心概念",
          "weight": 0.86,
          "evidence_level": "中",
          "basis": "210家质量诊断修正：原样本只挂父级，补充明确叶子节点",
          "node_id": "analog_power",
          "node_name": "模拟/功率/MCU",
          "path": [
            "半导体",
            "模拟 / 功率 / MCU"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "MCU",
          "role": "核心概念",
          "weight": 0.86,
          "evidence_level": "中",
          "basis": "210家质量诊断修正：原样本只挂父级，补充明确叶子节点",
          "node_id": "analog_power",
          "node_name": "模拟/功率/MCU",
          "path": [
            "半导体",
            "模拟 / 功率 / MCU"
          ]
        }
      ]
    },
    {
      "code": "002371",
      "name": "北方华创",
      "batch": "EXISTING86",
      "primary_industry": "半导体产业链",
      "primary_leaf": "半导体设备",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 86.0,
      "fundamental_score": 90.0,
      "net_profit_yi": 15.68,
      "net_profit_yoy_pct": 0.03,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "半导体产业链",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "半导体设备",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "semicap",
          "node_name": "半导体设备",
          "path": [
            "半导体",
            "半导体设备"
          ]
        },
        {
          "name": "国产替代",
          "role": "核心概念",
          "weight": 0.86,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "半导体",
          "role": "核心概念",
          "weight": 0.86,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "设备",
          "role": "核心概念",
          "weight": 0.86,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "semicap",
          "node_name": "半导体设备",
          "path": [
            "半导体",
            "半导体设备"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "半导体设备",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "semicap",
          "node_name": "半导体设备",
          "path": [
            "半导体",
            "半导体设备"
          ]
        },
        {
          "name": "设备",
          "role": "核心概念",
          "weight": 0.86,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "semicap",
          "node_name": "半导体设备",
          "path": [
            "半导体",
            "半导体设备"
          ]
        }
      ]
    },
    {
      "code": "002185",
      "name": "华天科技",
      "batch": "EXISTING86",
      "primary_industry": "半导体产业链",
      "primary_leaf": "封装测试/先进封装",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 96.0,
      "fundamental_score": 83.0,
      "net_profit_yi": 0.75,
      "net_profit_yoy_pct": 5.68,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "半导体产业链",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "封装测试",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "advanced_packaging",
          "node_name": "封装测试/先进封装",
          "path": [
            "半导体",
            "封装测试/先进封装"
          ]
        },
        {
          "name": "先进封装",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "advanced_packaging",
          "node_name": "封装测试/先进封装",
          "path": [
            "半导体",
            "封装测试/先进封装"
          ]
        },
        {
          "name": "半导体",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "封装测试",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "advanced_packaging",
          "node_name": "封装测试/先进封装",
          "path": [
            "半导体",
            "封装测试/先进封装"
          ]
        },
        {
          "name": "先进封装",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "advanced_packaging",
          "node_name": "封装测试/先进封装",
          "path": [
            "半导体",
            "封装测试/先进封装"
          ]
        }
      ]
    },
    {
      "code": "688072",
      "name": "拓荆科技",
      "batch": "EXISTING86",
      "primary_industry": "半导体产业链",
      "primary_leaf": "半导体设备",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 85.0,
      "fundamental_score": 88.0,
      "net_profit_yi": 5.62,
      "net_profit_yoy_pct": 4.88,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "半导体产业链",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "半导体设备",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "semicap",
          "node_name": "半导体设备",
          "path": [
            "半导体",
            "半导体设备"
          ]
        },
        {
          "name": "薄膜沉积",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "semicap",
          "node_name": "半导体设备",
          "path": [
            "半导体",
            "半导体设备"
          ]
        },
        {
          "name": "半导体",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "国产替代",
          "role": "相关概念",
          "weight": 0.61,
          "evidence_level": "需复核",
          "basis": "主题标签",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "设备",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "semicap",
          "node_name": "半导体设备",
          "path": [
            "半导体",
            "半导体设备"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "半导体设备",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "semicap",
          "node_name": "半导体设备",
          "path": [
            "半导体",
            "半导体设备"
          ]
        },
        {
          "name": "薄膜沉积",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "semicap",
          "node_name": "半导体设备",
          "path": [
            "半导体",
            "半导体设备"
          ]
        },
        {
          "name": "设备",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "semicap",
          "node_name": "半导体设备",
          "path": [
            "半导体",
            "半导体设备"
          ]
        }
      ]
    },
    {
      "code": "002156",
      "name": "通富微电",
      "batch": "EXISTING86",
      "primary_industry": "半导体产业链",
      "primary_leaf": "封装测试/先进封装",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 95.0,
      "fundamental_score": 93.0,
      "net_profit_yi": 3.5,
      "net_profit_yoy_pct": 2.25,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "半导体产业链",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "封装测试",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "advanced_packaging",
          "node_name": "封装测试/先进封装",
          "path": [
            "半导体",
            "封装测试/先进封装"
          ]
        },
        {
          "name": "先进封装",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "advanced_packaging",
          "node_name": "封装测试/先进封装",
          "path": [
            "半导体",
            "封装测试/先进封装"
          ]
        },
        {
          "name": "半导体",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "Chiplet",
          "role": "相关概念",
          "weight": 0.77,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "advanced_packaging",
          "node_name": "封装测试/先进封装",
          "path": [
            "半导体",
            "封装测试/先进封装"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "封装测试",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "advanced_packaging",
          "node_name": "封装测试/先进封装",
          "path": [
            "半导体",
            "封装测试/先进封装"
          ]
        },
        {
          "name": "先进封装",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "advanced_packaging",
          "node_name": "封装测试/先进封装",
          "path": [
            "半导体",
            "封装测试/先进封装"
          ]
        },
        {
          "name": "Chiplet",
          "role": "相关概念",
          "weight": 0.77,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "advanced_packaging",
          "node_name": "封装测试/先进封装",
          "path": [
            "半导体",
            "封装测试/先进封装"
          ]
        }
      ]
    },
    {
      "code": "603501",
      "name": "韦尔股份",
      "batch": "EXISTING86",
      "primary_industry": "半导体产业链",
      "primary_leaf": "模拟/功率/MCU",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 86.0,
      "fundamental_score": 67.0,
      "net_profit_yi": 4.94,
      "net_profit_yoy_pct": -0.42,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "半导体产业链",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "模拟芯片",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "CIS",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "半导体",
          "role": "核心概念",
          "weight": 0.8,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "芯片",
          "role": "核心概念",
          "weight": 0.8,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "CIS图像传感器",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "中",
          "basis": "210家质量诊断修正：原样本只挂父级，补充明确叶子节点",
          "node_id": "analog_power",
          "node_name": "模拟/功率/MCU",
          "path": [
            "半导体",
            "模拟 / 功率 / MCU"
          ]
        },
        {
          "name": "模拟 / MCU",
          "role": "核心概念",
          "weight": 0.82,
          "evidence_level": "中",
          "basis": "210家质量诊断修正：原样本只挂父级，补充明确叶子节点",
          "node_id": "analog_power",
          "node_name": "模拟/功率/MCU",
          "path": [
            "半导体",
            "模拟 / 功率 / MCU"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "CIS图像传感器",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "中",
          "basis": "210家质量诊断修正：原样本只挂父级，补充明确叶子节点",
          "node_id": "analog_power",
          "node_name": "模拟/功率/MCU",
          "path": [
            "半导体",
            "模拟 / 功率 / MCU"
          ]
        },
        {
          "name": "模拟 / MCU",
          "role": "核心概念",
          "weight": 0.82,
          "evidence_level": "中",
          "basis": "210家质量诊断修正：原样本只挂父级，补充明确叶子节点",
          "node_id": "analog_power",
          "node_name": "模拟/功率/MCU",
          "path": [
            "半导体",
            "模拟 / 功率 / MCU"
          ]
        }
      ]
    },
    {
      "code": "300750",
      "name": "宁德时代",
      "batch": "EXISTING86",
      "primary_industry": "新能源产业链",
      "primary_leaf": "动力电池/储能",
      "mount_count": 3,
      "root_count": 2,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 96.0,
      "fundamental_score": 95.0,
      "net_profit_yi": 227.37,
      "net_profit_yoy_pct": 0.49,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "新能源产业链",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "renewable_power",
          "node_name": "电力与公用事业",
          "path": [
            "电力与公用事业"
          ]
        },
        {
          "name": "动力电池",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "battery_storage",
          "node_name": "动力电池/储能",
          "path": [
            "新能源汽车",
            "动力电池/储能"
          ]
        },
        {
          "name": "储能",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "battery_storage",
          "node_name": "动力电池/储能",
          "path": [
            "新能源汽车",
            "动力电池/储能"
          ]
        },
        {
          "name": "新能源车",
          "role": "相关概念",
          "weight": 0.77,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "new_energy_vehicle",
          "node_name": "新能源汽车",
          "path": [
            "新能源汽车"
          ]
        },
        {
          "name": "锂电池",
          "role": "相关概念",
          "weight": 0.77,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "battery_storage",
          "node_name": "动力电池/储能",
          "path": [
            "新能源汽车",
            "动力电池/储能"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "动力电池",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "battery_storage",
          "node_name": "动力电池/储能",
          "path": [
            "新能源汽车",
            "动力电池/储能"
          ]
        },
        {
          "name": "储能",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "battery_storage",
          "node_name": "动力电池/储能",
          "path": [
            "新能源汽车",
            "动力电池/储能"
          ]
        },
        {
          "name": "锂电池",
          "role": "相关概念",
          "weight": 0.77,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "battery_storage",
          "node_name": "动力电池/储能",
          "path": [
            "新能源汽车",
            "动力电池/储能"
          ]
        }
      ]
    },
    {
      "code": "300274",
      "name": "阳光电源",
      "batch": "EXISTING86",
      "primary_industry": "新能源产业链",
      "primary_leaf": "光伏",
      "mount_count": 2,
      "root_count": 2,
      "leaf_count": 2,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 94.0,
      "fundamental_score": 73.0,
      "net_profit_yi": 22.46,
      "net_profit_yoy_pct": -0.4,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "新能源产业链",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "renewable_pv",
          "node_name": "光伏",
          "path": [
            "电力与公用事业",
            "光伏"
          ]
        },
        {
          "name": "光伏逆变器",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "renewable_pv",
          "node_name": "光伏",
          "path": [
            "电力与公用事业",
            "光伏"
          ]
        },
        {
          "name": "储能系统",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "battery_storage",
          "node_name": "动力电池/储能",
          "path": [
            "新能源汽车",
            "动力电池/储能"
          ]
        },
        {
          "name": "光伏",
          "role": "核心概念",
          "weight": 0.8,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "renewable_pv",
          "node_name": "光伏",
          "path": [
            "电力与公用事业",
            "光伏"
          ]
        },
        {
          "name": "储能",
          "role": "核心概念",
          "weight": 0.8,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "battery_storage",
          "node_name": "动力电池/储能",
          "path": [
            "新能源汽车",
            "动力电池/储能"
          ]
        },
        {
          "name": "新能源",
          "role": "核心概念",
          "weight": 0.8,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "renewable_pv",
          "node_name": "光伏",
          "path": [
            "电力与公用事业",
            "光伏"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "新能源产业链",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "renewable_pv",
          "node_name": "光伏",
          "path": [
            "电力与公用事业",
            "光伏"
          ]
        },
        {
          "name": "光伏逆变器",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "renewable_pv",
          "node_name": "光伏",
          "path": [
            "电力与公用事业",
            "光伏"
          ]
        },
        {
          "name": "储能系统",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "battery_storage",
          "node_name": "动力电池/储能",
          "path": [
            "新能源汽车",
            "动力电池/储能"
          ]
        },
        {
          "name": "光伏",
          "role": "核心概念",
          "weight": 0.8,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "renewable_pv",
          "node_name": "光伏",
          "path": [
            "电力与公用事业",
            "光伏"
          ]
        },
        {
          "name": "储能",
          "role": "核心概念",
          "weight": 0.8,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "battery_storage",
          "node_name": "动力电池/储能",
          "path": [
            "新能源汽车",
            "动力电池/储能"
          ]
        },
        {
          "name": "新能源",
          "role": "核心概念",
          "weight": 0.8,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "renewable_pv",
          "node_name": "光伏",
          "path": [
            "电力与公用事业",
            "光伏"
          ]
        }
      ]
    },
    {
      "code": "601689",
      "name": "拓普集团",
      "batch": "EXISTING86",
      "primary_industry": "新能源汽车产业链",
      "primary_leaf": "汽车零部件",
      "mount_count": 2,
      "root_count": 2,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 96.0,
      "fundamental_score": 68.0,
      "net_profit_yi": 5.53,
      "net_profit_yoy_pct": -0.02,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "新能源汽车产业链",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "new_energy_vehicle",
          "node_name": "新能源汽车",
          "path": [
            "新能源汽车"
          ]
        },
        {
          "name": "汽车零部件",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "auto_parts",
          "node_name": "汽车零部件",
          "path": [
            "新能源汽车",
            "汽车零部件"
          ]
        },
        {
          "name": "新能源车",
          "role": "相关概念",
          "weight": 0.72,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "new_energy_vehicle",
          "node_name": "新能源汽车",
          "path": [
            "新能源汽车"
          ]
        },
        {
          "name": "智能驾驶",
          "role": "相关概念",
          "weight": 0.72,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "auto_parts",
          "node_name": "汽车零部件",
          "path": [
            "新能源汽车",
            "汽车零部件"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "汽车零部件",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "auto_parts",
          "node_name": "汽车零部件",
          "path": [
            "新能源汽车",
            "汽车零部件"
          ]
        },
        {
          "name": "智能驾驶",
          "role": "相关概念",
          "weight": 0.72,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "auto_parts",
          "node_name": "汽车零部件",
          "path": [
            "新能源汽车",
            "汽车零部件"
          ]
        }
      ]
    },
    {
      "code": "002594",
      "name": "比亚迪",
      "batch": "EXISTING86",
      "primary_industry": "新能源汽车产业链",
      "primary_leaf": "动力电池/储能",
      "mount_count": 3,
      "root_count": 2,
      "leaf_count": 2,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 73.0,
      "net_profit_yi": 40.11,
      "net_profit_yoy_pct": -0.55,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "新能源汽车产业链",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "new_energy_vehicle",
          "node_name": "新能源汽车",
          "path": [
            "新能源汽车"
          ]
        },
        {
          "name": "整车",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "new_energy_vehicle",
          "node_name": "新能源汽车",
          "path": [
            "新能源汽车"
          ]
        },
        {
          "name": "动力电池",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "battery_storage",
          "node_name": "动力电池/储能",
          "path": [
            "新能源汽车",
            "动力电池/储能"
          ]
        },
        {
          "name": "新能源车",
          "role": "相关概念",
          "weight": 0.66,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "new_energy_vehicle",
          "node_name": "新能源汽车",
          "path": [
            "新能源汽车"
          ]
        },
        {
          "name": "储能",
          "role": "观察概念",
          "weight": 0.56,
          "evidence_level": "中",
          "basis": "相关标签",
          "node_id": "battery_storage",
          "node_name": "动力电池/储能",
          "path": [
            "新能源汽车",
            "动力电池/储能"
          ]
        },
        {
          "name": "智能驾驶",
          "role": "相关概念",
          "weight": 0.66,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "auto_parts",
          "node_name": "汽车零部件",
          "path": [
            "新能源汽车",
            "汽车零部件"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "动力电池",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "battery_storage",
          "node_name": "动力电池/储能",
          "path": [
            "新能源汽车",
            "动力电池/储能"
          ]
        },
        {
          "name": "储能",
          "role": "观察概念",
          "weight": 0.56,
          "evidence_level": "中",
          "basis": "相关标签",
          "node_id": "battery_storage",
          "node_name": "动力电池/储能",
          "path": [
            "新能源汽车",
            "动力电池/储能"
          ]
        },
        {
          "name": "智能驾驶",
          "role": "相关概念",
          "weight": 0.66,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "auto_parts",
          "node_name": "汽车零部件",
          "path": [
            "新能源汽车",
            "汽车零部件"
          ]
        }
      ]
    },
    {
      "code": "000651",
      "name": "格力电器",
      "batch": "EXISTING86",
      "primary_industry": "消费电子",
      "primary_leaf": "智能家居 / 白电",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 95.0,
      "net_profit_yi": 60.99,
      "net_profit_yoy_pct": 0.03,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "消费电子",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "consumer_electronics",
          "node_name": "消费电子",
          "path": [
            "消费电子"
          ]
        },
        {
          "name": "家电",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "consumer_smart_home",
          "node_name": "智能家居 / 白电",
          "path": [
            "消费电子",
            "智能家居 / 白电"
          ]
        },
        {
          "name": "白电",
          "role": "核心概念",
          "weight": 0.86,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "consumer_smart_home",
          "node_name": "智能家居 / 白电",
          "path": [
            "消费电子",
            "智能家居 / 白电"
          ]
        },
        {
          "name": "消费电子/家电",
          "role": "相关概念",
          "weight": 0.72,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "consumer_smart_home",
          "node_name": "智能家居 / 白电",
          "path": [
            "消费电子",
            "智能家居 / 白电"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "家电",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "consumer_smart_home",
          "node_name": "智能家居 / 白电",
          "path": [
            "消费电子",
            "智能家居 / 白电"
          ]
        },
        {
          "name": "白电",
          "role": "核心概念",
          "weight": 0.86,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "consumer_smart_home",
          "node_name": "智能家居 / 白电",
          "path": [
            "消费电子",
            "智能家居 / 白电"
          ]
        },
        {
          "name": "消费电子/家电",
          "role": "相关概念",
          "weight": 0.72,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "consumer_smart_home",
          "node_name": "智能家居 / 白电",
          "path": [
            "消费电子",
            "智能家居 / 白电"
          ]
        }
      ]
    },
    {
      "code": "600690",
      "name": "海尔智家",
      "batch": "EXISTING86",
      "primary_industry": "消费电子",
      "primary_leaf": "智能家居 / 白电",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 94.0,
      "fundamental_score": 73.0,
      "net_profit_yi": 48.07,
      "net_profit_yoy_pct": -0.15,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "消费电子",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "consumer_electronics",
          "node_name": "消费电子",
          "path": [
            "消费电子"
          ]
        },
        {
          "name": "家电",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "consumer_smart_home",
          "node_name": "智能家居 / 白电",
          "path": [
            "消费电子",
            "智能家居 / 白电"
          ]
        },
        {
          "name": "智能家居",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "consumer_smart_home",
          "node_name": "智能家居 / 白电",
          "path": [
            "消费电子",
            "智能家居 / 白电"
          ]
        },
        {
          "name": "消费电子/家电",
          "role": "相关概念",
          "weight": 0.69,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "consumer_smart_home",
          "node_name": "智能家居 / 白电",
          "path": [
            "消费电子",
            "智能家居 / 白电"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "家电",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "consumer_smart_home",
          "node_name": "智能家居 / 白电",
          "path": [
            "消费电子",
            "智能家居 / 白电"
          ]
        },
        {
          "name": "智能家居",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "consumer_smart_home",
          "node_name": "智能家居 / 白电",
          "path": [
            "消费电子",
            "智能家居 / 白电"
          ]
        },
        {
          "name": "消费电子/家电",
          "role": "相关概念",
          "weight": 0.69,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "consumer_smart_home",
          "node_name": "智能家居 / 白电",
          "path": [
            "消费电子",
            "智能家居 / 白电"
          ]
        }
      ]
    },
    {
      "code": "000333",
      "name": "美的集团",
      "batch": "EXISTING86",
      "primary_industry": "消费电子",
      "primary_leaf": "智能家居 / 白电",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 94.0,
      "fundamental_score": 95.0,
      "net_profit_yi": 128.6,
      "net_profit_yoy_pct": 0.02,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "消费电子",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "consumer_electronics",
          "node_name": "消费电子",
          "path": [
            "消费电子"
          ]
        },
        {
          "name": "家电",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "consumer_smart_home",
          "node_name": "智能家居 / 白电",
          "path": [
            "消费电子",
            "智能家居 / 白电"
          ]
        },
        {
          "name": "智能家居",
          "role": "核心概念",
          "weight": 0.86,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "consumer_smart_home",
          "node_name": "智能家居 / 白电",
          "path": [
            "消费电子",
            "智能家居 / 白电"
          ]
        },
        {
          "name": "消费电子/家电",
          "role": "相关概念",
          "weight": 0.72,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "consumer_smart_home",
          "node_name": "智能家居 / 白电",
          "path": [
            "消费电子",
            "智能家居 / 白电"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "家电",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "consumer_smart_home",
          "node_name": "智能家居 / 白电",
          "path": [
            "消费电子",
            "智能家居 / 白电"
          ]
        },
        {
          "name": "智能家居",
          "role": "核心概念",
          "weight": 0.86,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "consumer_smart_home",
          "node_name": "智能家居 / 白电",
          "path": [
            "消费电子",
            "智能家居 / 白电"
          ]
        },
        {
          "name": "消费电子/家电",
          "role": "相关概念",
          "weight": 0.72,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "consumer_smart_home",
          "node_name": "智能家居 / 白电",
          "path": [
            "消费电子",
            "智能家居 / 白电"
          ]
        }
      ]
    },
    {
      "code": "002241",
      "name": "歌尔股份",
      "batch": "EXISTING86",
      "primary_industry": "消费电子与终端",
      "primary_leaf": "AI终端 / 消费电子",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 84.0,
      "net_profit_yi": 5.22,
      "net_profit_yoy_pct": 0.07,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "消费电子与终端",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "consumer_electronics",
          "node_name": "消费电子",
          "path": [
            "消费电子"
          ]
        },
        {
          "name": "声学",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "consumer_ai_terminal",
          "node_name": "AI终端 / 消费电子",
          "path": [
            "消费电子",
            "AI终端 / 消费电子"
          ]
        },
        {
          "name": "VRAR",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "consumer_ai_terminal",
          "node_name": "AI终端 / 消费电子",
          "path": [
            "消费电子",
            "AI终端 / 消费电子"
          ]
        },
        {
          "name": "智能硬件",
          "role": "二级赛道",
          "weight": 0.78,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "consumer_ai_terminal",
          "node_name": "AI终端 / 消费电子",
          "path": [
            "消费电子",
            "AI终端 / 消费电子"
          ]
        },
        {
          "name": "消费电子",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "consumer_ai_terminal",
          "node_name": "AI终端 / 消费电子",
          "path": [
            "消费电子",
            "AI终端 / 消费电子"
          ]
        },
        {
          "name": "AI终端",
          "role": "相关概念",
          "weight": 0.75,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "consumer_ai_terminal",
          "node_name": "AI终端 / 消费电子",
          "path": [
            "消费电子",
            "AI终端 / 消费电子"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "声学",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "consumer_ai_terminal",
          "node_name": "AI终端 / 消费电子",
          "path": [
            "消费电子",
            "AI终端 / 消费电子"
          ]
        },
        {
          "name": "VRAR",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "consumer_ai_terminal",
          "node_name": "AI终端 / 消费电子",
          "path": [
            "消费电子",
            "AI终端 / 消费电子"
          ]
        },
        {
          "name": "智能硬件",
          "role": "二级赛道",
          "weight": 0.78,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "consumer_ai_terminal",
          "node_name": "AI终端 / 消费电子",
          "path": [
            "消费电子",
            "AI终端 / 消费电子"
          ]
        },
        {
          "name": "消费电子",
          "role": "核心概念",
          "weight": 0.89,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "consumer_ai_terminal",
          "node_name": "AI终端 / 消费电子",
          "path": [
            "消费电子",
            "AI终端 / 消费电子"
          ]
        },
        {
          "name": "AI终端",
          "role": "相关概念",
          "weight": 0.75,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "consumer_ai_terminal",
          "node_name": "AI终端 / 消费电子",
          "path": [
            "消费电子",
            "AI终端 / 消费电子"
          ]
        }
      ]
    },
    {
      "code": "000858",
      "name": "五粮液",
      "batch": "EXISTING86",
      "primary_industry": "消费白马",
      "primary_leaf": "白酒 / 高端消费",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 89.0,
      "net_profit_yi": 83.24,
      "net_profit_yoy_pct": 0.83,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "消费白马",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "consumer_bluechip",
          "node_name": "消费白马",
          "path": [
            "消费白马"
          ]
        },
        {
          "name": "白酒",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "consumer_liquor_premium",
          "node_name": "白酒 / 高端消费",
          "path": [
            "消费白马",
            "白酒 / 高端消费"
          ]
        },
        {
          "name": "高端消费",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "consumer_liquor_premium",
          "node_name": "白酒 / 高端消费",
          "path": [
            "消费白马",
            "白酒 / 高端消费"
          ]
        },
        {
          "name": "消费",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "consumer_liquor_premium",
          "node_name": "白酒 / 高端消费",
          "path": [
            "消费白马",
            "白酒 / 高端消费"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "白酒",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "consumer_liquor_premium",
          "node_name": "白酒 / 高端消费",
          "path": [
            "消费白马",
            "白酒 / 高端消费"
          ]
        },
        {
          "name": "高端消费",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "consumer_liquor_premium",
          "node_name": "白酒 / 高端消费",
          "path": [
            "消费白马",
            "白酒 / 高端消费"
          ]
        },
        {
          "name": "消费",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "consumer_liquor_premium",
          "node_name": "白酒 / 高端消费",
          "path": [
            "消费白马",
            "白酒 / 高端消费"
          ]
        }
      ]
    },
    {
      "code": "000568",
      "name": "泸州老窖",
      "batch": "EXISTING86",
      "primary_industry": "消费白马",
      "primary_leaf": "白酒 / 高端消费",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 77.0,
      "net_profit_yi": 37.3,
      "net_profit_yoy_pct": -0.19,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "消费白马",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "consumer_bluechip",
          "node_name": "消费白马",
          "path": [
            "消费白马"
          ]
        },
        {
          "name": "白酒",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "consumer_liquor_premium",
          "node_name": "白酒 / 高端消费",
          "path": [
            "消费白马",
            "白酒 / 高端消费"
          ]
        },
        {
          "name": "高端消费",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "consumer_liquor_premium",
          "node_name": "白酒 / 高端消费",
          "path": [
            "消费白马",
            "白酒 / 高端消费"
          ]
        },
        {
          "name": "消费",
          "role": "核心概念",
          "weight": 0.83,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "consumer_liquor_premium",
          "node_name": "白酒 / 高端消费",
          "path": [
            "消费白马",
            "白酒 / 高端消费"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "白酒",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "consumer_liquor_premium",
          "node_name": "白酒 / 高端消费",
          "path": [
            "消费白马",
            "白酒 / 高端消费"
          ]
        },
        {
          "name": "高端消费",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "consumer_liquor_premium",
          "node_name": "白酒 / 高端消费",
          "path": [
            "消费白马",
            "白酒 / 高端消费"
          ]
        },
        {
          "name": "消费",
          "role": "核心概念",
          "weight": 0.83,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "consumer_liquor_premium",
          "node_name": "白酒 / 高端消费",
          "path": [
            "消费白马",
            "白酒 / 高端消费"
          ]
        }
      ]
    },
    {
      "code": "601857",
      "name": "中国石油",
      "batch": "EXISTING86",
      "primary_industry": "资源周期",
      "primary_leaf": "能源资源",
      "mount_count": 3,
      "root_count": 2,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 94.0,
      "fundamental_score": 95.0,
      "net_profit_yi": 534.77,
      "net_profit_yoy_pct": 0.02,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "资源周期",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "resources",
          "node_name": "资源周期",
          "path": [
            "资源周期"
          ]
        },
        {
          "name": "油气",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "resources_energy",
          "node_name": "能源资源",
          "path": [
            "资源周期",
            "能源资源"
          ]
        },
        {
          "name": "炼化",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "resources_energy",
          "node_name": "能源资源",
          "path": [
            "资源周期",
            "能源资源"
          ]
        },
        {
          "name": "资源",
          "role": "核心概念",
          "weight": 0.86,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "resources_energy",
          "node_name": "能源资源",
          "path": [
            "资源周期",
            "能源资源"
          ]
        },
        {
          "name": "高股息",
          "role": "相关概念",
          "weight": 0.72,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "finance",
          "node_name": "金融",
          "path": [
            "金融"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "油气",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "resources_energy",
          "node_name": "能源资源",
          "path": [
            "资源周期",
            "能源资源"
          ]
        },
        {
          "name": "炼化",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "resources_energy",
          "node_name": "能源资源",
          "path": [
            "资源周期",
            "能源资源"
          ]
        },
        {
          "name": "资源",
          "role": "核心概念",
          "weight": 0.86,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "resources_energy",
          "node_name": "能源资源",
          "path": [
            "资源周期",
            "能源资源"
          ]
        }
      ]
    },
    {
      "code": "601088",
      "name": "中国神华",
      "batch": "EXISTING86",
      "primary_industry": "资源周期",
      "primary_leaf": "能源资源",
      "mount_count": 3,
      "root_count": 2,
      "leaf_count": 2,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 94.0,
      "fundamental_score": 77.0,
      "net_profit_yi": 133.12,
      "net_profit_yoy_pct": -0.11,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "资源周期",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "resources",
          "node_name": "资源周期",
          "path": [
            "资源周期"
          ]
        },
        {
          "name": "煤炭",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "resources_energy",
          "node_name": "能源资源",
          "path": [
            "资源周期",
            "能源资源"
          ]
        },
        {
          "name": "高股息",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "finance_high_dividend",
          "node_name": "高股息金融",
          "path": [
            "金融",
            "高股息金融"
          ]
        },
        {
          "name": "资源",
          "role": "核心概念",
          "weight": 0.83,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "resources_energy",
          "node_name": "能源资源",
          "path": [
            "资源周期",
            "能源资源"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "煤炭",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "resources_energy",
          "node_name": "能源资源",
          "path": [
            "资源周期",
            "能源资源"
          ]
        },
        {
          "name": "高股息",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "finance_high_dividend",
          "node_name": "高股息金融",
          "path": [
            "金融",
            "高股息金融"
          ]
        },
        {
          "name": "资源",
          "role": "核心概念",
          "weight": 0.83,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "resources_energy",
          "node_name": "能源资源",
          "path": [
            "资源周期",
            "能源资源"
          ]
        }
      ]
    },
    {
      "code": "002466",
      "name": "天齐锂业",
      "batch": "EXISTING86",
      "primary_industry": "资源周期",
      "primary_leaf": "金属矿产",
      "mount_count": 3,
      "root_count": 2,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 95.0,
      "net_profit_yi": 28.41,
      "net_profit_yoy_pct": 16.99,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "资源周期",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "resources",
          "node_name": "资源周期",
          "path": [
            "资源周期"
          ]
        },
        {
          "name": "锂资源",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "resources_metal_mining",
          "node_name": "金属矿产",
          "path": [
            "资源周期",
            "金属矿产"
          ]
        },
        {
          "name": "资源",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "resources_metal_mining",
          "node_name": "金属矿产",
          "path": [
            "资源周期",
            "金属矿产"
          ]
        },
        {
          "name": "新能源",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "renewable_power",
          "node_name": "电力与公用事业",
          "path": [
            "电力与公用事业"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "锂资源",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "resources_metal_mining",
          "node_name": "金属矿产",
          "path": [
            "资源周期",
            "金属矿产"
          ]
        },
        {
          "name": "资源",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "resources_metal_mining",
          "node_name": "金属矿产",
          "path": [
            "资源周期",
            "金属矿产"
          ]
        }
      ]
    },
    {
      "code": "603993",
      "name": "洛阳钼业",
      "batch": "EXISTING86",
      "primary_industry": "资源周期",
      "primary_leaf": "金属矿产",
      "mount_count": 3,
      "root_count": 2,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 94.0,
      "fundamental_score": 95.0,
      "net_profit_yi": 88.24,
      "net_profit_yoy_pct": 0.97,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "资源周期",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "resources",
          "node_name": "资源周期",
          "path": [
            "资源周期"
          ]
        },
        {
          "name": "有色金属",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "advanced_materials",
          "node_name": "有色金属与新材料",
          "path": [
            "有色金属与新材料"
          ]
        },
        {
          "name": "全球矿业",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "resources_metal_mining",
          "node_name": "金属矿产",
          "path": [
            "资源周期",
            "金属矿产"
          ]
        },
        {
          "name": "资源",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "resources_metal_mining",
          "node_name": "金属矿产",
          "path": [
            "资源周期",
            "金属矿产"
          ]
        },
        {
          "name": "铜钴",
          "role": "相关概念",
          "weight": 0.63,
          "evidence_level": "中",
          "basis": "相关标签",
          "node_id": "resources",
          "node_name": "资源周期",
          "path": [
            "资源周期"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "全球矿业",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "resources_metal_mining",
          "node_name": "金属矿产",
          "path": [
            "资源周期",
            "金属矿产"
          ]
        },
        {
          "name": "资源",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "resources_metal_mining",
          "node_name": "金属矿产",
          "path": [
            "资源周期",
            "金属矿产"
          ]
        }
      ]
    },
    {
      "code": "601899",
      "name": "紫金矿业",
      "batch": "EXISTING86",
      "primary_industry": "资源周期",
      "primary_leaf": "金属矿产",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 95.0,
      "net_profit_yi": 251.66,
      "net_profit_yoy_pct": 0.98,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "资源周期",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "resources",
          "node_name": "资源周期",
          "path": [
            "资源周期"
          ]
        },
        {
          "name": "铜金矿产",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "resources_metal_mining",
          "node_name": "金属矿产",
          "path": [
            "资源周期",
            "金属矿产"
          ]
        },
        {
          "name": "全球矿业",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "resources_metal_mining",
          "node_name": "金属矿产",
          "path": [
            "资源周期",
            "金属矿产"
          ]
        },
        {
          "name": "黄金",
          "role": "相关概念",
          "weight": 0.63,
          "evidence_level": "中",
          "basis": "相关标签",
          "node_id": "resources",
          "node_name": "资源周期",
          "path": [
            "资源周期"
          ]
        },
        {
          "name": "铜",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "resources_metal_mining",
          "node_name": "金属矿产",
          "path": [
            "资源周期",
            "金属矿产"
          ]
        },
        {
          "name": "资源",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "resources_metal_mining",
          "node_name": "金属矿产",
          "path": [
            "资源周期",
            "金属矿产"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "铜金矿产",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "resources_metal_mining",
          "node_name": "金属矿产",
          "path": [
            "资源周期",
            "金属矿产"
          ]
        },
        {
          "name": "全球矿业",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "resources_metal_mining",
          "node_name": "金属矿产",
          "path": [
            "资源周期",
            "金属矿产"
          ]
        },
        {
          "name": "铜",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "resources_metal_mining",
          "node_name": "金属矿产",
          "path": [
            "资源周期",
            "金属矿产"
          ]
        },
        {
          "name": "资源",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "resources_metal_mining",
          "node_name": "金属矿产",
          "path": [
            "资源周期",
            "金属矿产"
          ]
        }
      ]
    },
    {
      "code": "300059",
      "name": "东方财富",
      "batch": "EXISTING86",
      "primary_industry": "金融",
      "primary_leaf": "证券 / 财富管理",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 95.0,
      "net_profit_yi": 37.38,
      "net_profit_yoy_pct": 0.38,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "金融",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "finance",
          "node_name": "金融",
          "path": [
            "金融"
          ]
        },
        {
          "name": "互联网券商",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "finance_broker_wealth",
          "node_name": "证券 / 财富管理",
          "path": [
            "金融",
            "证券 / 财富管理"
          ]
        },
        {
          "name": "财富管理",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "finance_broker_wealth",
          "node_name": "证券 / 财富管理",
          "path": [
            "金融",
            "证券 / 财富管理"
          ]
        },
        {
          "name": "证券",
          "role": "相关概念",
          "weight": 0.63,
          "evidence_level": "中",
          "basis": "相关标签",
          "node_id": "finance",
          "node_name": "金融",
          "path": [
            "金融"
          ]
        },
        {
          "name": "金融科技",
          "role": "相关概念",
          "weight": 0.77,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "finance_broker_wealth",
          "node_name": "证券 / 财富管理",
          "path": [
            "金融",
            "证券 / 财富管理"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "互联网券商",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "finance_broker_wealth",
          "node_name": "证券 / 财富管理",
          "path": [
            "金融",
            "证券 / 财富管理"
          ]
        },
        {
          "name": "财富管理",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "finance_broker_wealth",
          "node_name": "证券 / 财富管理",
          "path": [
            "金融",
            "证券 / 财富管理"
          ]
        },
        {
          "name": "金融科技",
          "role": "相关概念",
          "weight": 0.77,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "finance_broker_wealth",
          "node_name": "证券 / 财富管理",
          "path": [
            "金融",
            "证券 / 财富管理"
          ]
        }
      ]
    },
    {
      "code": "600030",
      "name": "中信证券",
      "batch": "EXISTING86",
      "primary_industry": "金融",
      "primary_leaf": "证券 / 财富管理",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 94.0,
      "fundamental_score": 95.0,
      "net_profit_yi": 104.59,
      "net_profit_yoy_pct": 0.55,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "金融",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "finance",
          "node_name": "金融",
          "path": [
            "金融"
          ]
        },
        {
          "name": "证券",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "finance_broker_wealth",
          "node_name": "证券 / 财富管理",
          "path": [
            "金融",
            "证券 / 财富管理"
          ]
        },
        {
          "name": "财富管理",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "finance_broker_wealth",
          "node_name": "证券 / 财富管理",
          "path": [
            "金融",
            "证券 / 财富管理"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "证券",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "finance_broker_wealth",
          "node_name": "证券 / 财富管理",
          "path": [
            "金融",
            "证券 / 财富管理"
          ]
        },
        {
          "name": "财富管理",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "finance_broker_wealth",
          "node_name": "证券 / 财富管理",
          "path": [
            "金融",
            "证券 / 财富管理"
          ]
        }
      ]
    },
    {
      "code": "601318",
      "name": "中国平安",
      "batch": "EXISTING86",
      "primary_industry": "金融",
      "primary_leaf": "保险 / 综合金融",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 77.0,
      "net_profit_yi": 332.63,
      "net_profit_yoy_pct": -0.07,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "金融",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "finance",
          "node_name": "金融",
          "path": [
            "金融"
          ]
        },
        {
          "name": "保险",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "finance_insurance_fintech",
          "node_name": "保险 / 综合金融",
          "path": [
            "金融",
            "保险 / 综合金融"
          ]
        },
        {
          "name": "综合金融",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "finance_insurance_fintech",
          "node_name": "保险 / 综合金融",
          "path": [
            "金融",
            "保险 / 综合金融"
          ]
        },
        {
          "name": "高股息",
          "role": "相关概念",
          "weight": 0.69,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "finance",
          "node_name": "金融",
          "path": [
            "金融"
          ]
        },
        {
          "name": "金融科技",
          "role": "相关概念",
          "weight": 0.69,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "finance_insurance_fintech",
          "node_name": "保险 / 综合金融",
          "path": [
            "金融",
            "保险 / 综合金融"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "保险",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "finance_insurance_fintech",
          "node_name": "保险 / 综合金融",
          "path": [
            "金融",
            "保险 / 综合金融"
          ]
        },
        {
          "name": "综合金融",
          "role": "二级赛道",
          "weight": 0.84,
          "evidence_level": "中",
          "basis": "报告二级分类",
          "node_id": "finance_insurance_fintech",
          "node_name": "保险 / 综合金融",
          "path": [
            "金融",
            "保险 / 综合金融"
          ]
        },
        {
          "name": "金融科技",
          "role": "相关概念",
          "weight": 0.69,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "finance_insurance_fintech",
          "node_name": "保险 / 综合金融",
          "path": [
            "金融",
            "保险 / 综合金融"
          ]
        }
      ]
    },
    {
      "code": "601398",
      "name": "工商银行",
      "batch": "EXISTING86",
      "primary_industry": "金融",
      "primary_leaf": "高股息金融",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 88.0,
      "fundamental_score": 95.0,
      "net_profit_yi": 880.13,
      "net_profit_yoy_pct": 0.03,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "金融",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "finance",
          "node_name": "金融",
          "path": [
            "金融"
          ]
        },
        {
          "name": "银行",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "finance_high_dividend",
          "node_name": "高股息金融",
          "path": [
            "金融",
            "高股息金融"
          ]
        },
        {
          "name": "高股息",
          "role": "核心概念",
          "weight": 0.86,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "finance_high_dividend",
          "node_name": "高股息金融",
          "path": [
            "金融",
            "高股息金融"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "银行",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "finance_high_dividend",
          "node_name": "高股息金融",
          "path": [
            "金融",
            "高股息金融"
          ]
        },
        {
          "name": "高股息",
          "role": "核心概念",
          "weight": 0.86,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "finance_high_dividend",
          "node_name": "高股息金融",
          "path": [
            "金融",
            "高股息金融"
          ]
        }
      ]
    },
    {
      "code": "601939",
      "name": "建设银行",
      "batch": "EXISTING86",
      "primary_industry": "金融",
      "primary_leaf": "高股息金融",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 87.0,
      "fundamental_score": 95.0,
      "net_profit_yi": 868.21,
      "net_profit_yoy_pct": 0.04,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "金融",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "finance",
          "node_name": "金融",
          "path": [
            "金融"
          ]
        },
        {
          "name": "银行",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "finance_high_dividend",
          "node_name": "高股息金融",
          "path": [
            "金融",
            "高股息金融"
          ]
        },
        {
          "name": "高股息",
          "role": "核心概念",
          "weight": 0.86,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "finance_high_dividend",
          "node_name": "高股息金融",
          "path": [
            "金融",
            "高股息金融"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "银行",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "finance_high_dividend",
          "node_name": "高股息金融",
          "path": [
            "金融",
            "高股息金融"
          ]
        },
        {
          "name": "高股息",
          "role": "核心概念",
          "weight": 0.86,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "finance_high_dividend",
          "node_name": "高股息金融",
          "path": [
            "金融",
            "高股息金融"
          ]
        }
      ]
    },
    {
      "code": "600036",
      "name": "招商银行",
      "batch": "EXISTING86",
      "primary_industry": "金融",
      "primary_leaf": "银行",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 95.0,
      "net_profit_yi": 380.48,
      "net_profit_yoy_pct": 0.02,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "金融",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "finance",
          "node_name": "金融",
          "path": [
            "金融"
          ]
        },
        {
          "name": "股份制银行",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "finance_bank",
          "node_name": "银行",
          "path": [
            "金融",
            "银行"
          ]
        },
        {
          "name": "财富管理",
          "role": "核心概念",
          "weight": 0.86,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "finance_bank",
          "node_name": "银行",
          "path": [
            "金融",
            "银行"
          ]
        },
        {
          "name": "银行",
          "role": "核心概念",
          "weight": 0.86,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "finance_bank",
          "node_name": "银行",
          "path": [
            "金融",
            "银行"
          ]
        },
        {
          "name": "高股息",
          "role": "相关概念",
          "weight": 0.72,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "finance_bank",
          "node_name": "银行",
          "path": [
            "金融",
            "银行"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "股份制银行",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "finance_bank",
          "node_name": "银行",
          "path": [
            "金融",
            "银行"
          ]
        },
        {
          "name": "财富管理",
          "role": "核心概念",
          "weight": 0.86,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "finance_bank",
          "node_name": "银行",
          "path": [
            "金融",
            "银行"
          ]
        },
        {
          "name": "银行",
          "role": "核心概念",
          "weight": 0.86,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "finance_bank",
          "node_name": "银行",
          "path": [
            "金融",
            "银行"
          ]
        },
        {
          "name": "高股息",
          "role": "相关概念",
          "weight": 0.72,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "finance_bank",
          "node_name": "银行",
          "path": [
            "金融",
            "银行"
          ]
        }
      ]
    },
    {
      "code": "688111",
      "name": "金山办公",
      "batch": "EXISTING86",
      "primary_industry": "高端装备与工业",
      "primary_leaf": "工业软件 / 企业软件",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 95.0,
      "net_profit_yi": 21.95,
      "net_profit_yoy_pct": 4.45,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "高端装备与工业",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "industrial_equipment",
          "node_name": "高端装备与工业",
          "path": [
            "高端装备与工业"
          ]
        },
        {
          "name": "办公软件",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "industrial_software",
          "node_name": "工业软件 / 企业软件",
          "path": [
            "高端装备与工业",
            "工业软件 / 企业软件"
          ]
        },
        {
          "name": "AI应用",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "industrial_software",
          "node_name": "工业软件 / 企业软件",
          "path": [
            "高端装备与工业",
            "工业软件 / 企业软件"
          ]
        },
        {
          "name": "企业软件",
          "role": "相关概念",
          "weight": 0.77,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "industrial_software",
          "node_name": "工业软件 / 企业软件",
          "path": [
            "高端装备与工业",
            "工业软件 / 企业软件"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "办公软件",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "industrial_software",
          "node_name": "工业软件 / 企业软件",
          "path": [
            "高端装备与工业",
            "工业软件 / 企业软件"
          ]
        },
        {
          "name": "AI应用",
          "role": "核心概念",
          "weight": 0.91,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "industrial_software",
          "node_name": "工业软件 / 企业软件",
          "path": [
            "高端装备与工业",
            "工业软件 / 企业软件"
          ]
        },
        {
          "name": "企业软件",
          "role": "相关概念",
          "weight": 0.77,
          "evidence_level": "中",
          "basis": "业务相关",
          "node_id": "industrial_software",
          "node_name": "工业软件 / 企业软件",
          "path": [
            "高端装备与工业",
            "工业软件 / 企业软件"
          ]
        }
      ]
    },
    {
      "code": "603019",
      "name": "中科曙光",
      "batch": "NEXT44",
      "primary_industry": "AI算力",
      "primary_leaf": "AI服务器链",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 85.0,
      "fundamental_score": 79.0,
      "net_profit_yi": 1.92,
      "net_profit_yoy_pct": 0.22,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "AI算力",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "中",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "ai_compute",
          "node_name": "AI算力",
          "path": [
            "AI算力"
          ]
        },
        {
          "name": "AI服务器链",
          "role": "二级赛道",
          "weight": 0.82,
          "evidence_level": "中",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "ai_server",
          "node_name": "AI服务器链",
          "path": [
            "AI算力",
            "AI服务器链"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "AI服务器链",
          "role": "二级赛道",
          "weight": 0.82,
          "evidence_level": "中",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "ai_server",
          "node_name": "AI服务器链",
          "path": [
            "AI算力",
            "AI服务器链"
          ]
        }
      ]
    },
    {
      "code": "000938",
      "name": "紫光股份",
      "batch": "NEXT44",
      "primary_industry": "AI算力",
      "primary_leaf": "AI服务器链",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 88.0,
      "net_profit_yi": 9.61,
      "net_profit_yoy_pct": 1.26,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "AI算力",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "ai_compute",
          "node_name": "AI算力",
          "path": [
            "AI算力"
          ]
        },
        {
          "name": "AI服务器链",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "ai_server",
          "node_name": "AI服务器链",
          "path": [
            "AI算力",
            "AI服务器链"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "AI服务器链",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "ai_server",
          "node_name": "AI服务器链",
          "path": [
            "AI算力",
            "AI服务器链"
          ]
        }
      ]
    },
    {
      "code": "601919",
      "name": "中远海控",
      "batch": "NEXT44",
      "primary_industry": "交运物流",
      "primary_leaf": "航运/港口",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 94.0,
      "fundamental_score": 77.0,
      "net_profit_yi": 68.82,
      "net_profit_yoy_pct": -0.5,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "交运物流",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "transportation_logistics",
          "node_name": "交运物流",
          "path": [
            "交运物流"
          ]
        },
        {
          "name": "航运 / 港口",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "shipping_port",
          "node_name": "航运/港口",
          "path": [
            "交运物流",
            "航运/港口"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "航运 / 港口",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "shipping_port",
          "node_name": "航运/港口",
          "path": [
            "交运物流",
            "航运/港口"
          ]
        }
      ]
    },
    {
      "code": "002352",
      "name": "顺丰控股",
      "batch": "NEXT44",
      "primary_industry": "交运物流",
      "primary_leaf": "快递/物流",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 96.0,
      "fundamental_score": 95.0,
      "net_profit_yi": 26.51,
      "net_profit_yoy_pct": 0.13,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "交运物流",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "transportation_logistics",
          "node_name": "交运物流",
          "path": [
            "交运物流"
          ]
        },
        {
          "name": "快递 / 物流",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "express_logistics",
          "node_name": "快递/物流",
          "path": [
            "交运物流",
            "快递 / 物流"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "快递 / 物流",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "express_logistics",
          "node_name": "快递/物流",
          "path": [
            "交运物流",
            "快递 / 物流"
          ]
        }
      ]
    },
    {
      "code": "002555",
      "name": "三七互娱",
      "batch": "NEXT44",
      "primary_industry": "传媒互联网",
      "primary_leaf": "游戏/内容",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 90.0,
      "net_profit_yi": 8.71,
      "net_profit_yoy_pct": 0.59,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "传媒互联网",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "media_internet",
          "node_name": "传媒互联网",
          "path": [
            "传媒互联网"
          ]
        },
        {
          "name": "游戏 / 内容",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "game_content",
          "node_name": "游戏/内容",
          "path": [
            "传媒互联网",
            "游戏 / 内容"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "游戏 / 内容",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "game_content",
          "node_name": "游戏/内容",
          "path": [
            "传媒互联网",
            "游戏 / 内容"
          ]
        }
      ]
    },
    {
      "code": "002027",
      "name": "分众传媒",
      "batch": "NEXT44",
      "primary_industry": "传媒互联网",
      "primary_leaf": "广告营销",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 94.0,
      "net_profit_yi": 17.84,
      "net_profit_yoy_pct": 0.58,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "传媒互联网",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "media_internet",
          "node_name": "传媒互联网",
          "path": [
            "传媒互联网"
          ]
        },
        {
          "name": "广告营销",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "advertising_marketing",
          "node_name": "广告营销",
          "path": [
            "传媒互联网",
            "广告营销"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "广告营销",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "advertising_marketing",
          "node_name": "广告营销",
          "path": [
            "传媒互联网",
            "广告营销"
          ]
        }
      ]
    },
    {
      "code": "600598",
      "name": "北大荒",
      "batch": "NEXT44",
      "primary_industry": "农业养殖",
      "primary_leaf": "种业/种植",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 94.0,
      "fundamental_score": 94.0,
      "net_profit_yi": 5.79,
      "net_profit_yoy_pct": 0.08,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "农业养殖",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "agriculture_food",
          "node_name": "农业养殖",
          "path": [
            "农业养殖"
          ]
        },
        {
          "name": "种业 / 种植",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "seed_planting",
          "node_name": "种业/种植",
          "path": [
            "农业养殖",
            "种业 / 种植"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "种业 / 种植",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "seed_planting",
          "node_name": "种业/种植",
          "path": [
            "农业养殖",
            "种业 / 种植"
          ]
        }
      ]
    },
    {
      "code": "002311",
      "name": "海大集团",
      "batch": "NEXT44",
      "primary_industry": "农业养殖",
      "primary_leaf": "猪禽水产养殖",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 94.0,
      "fundamental_score": 68.0,
      "net_profit_yi": 9.45,
      "net_profit_yoy_pct": -0.31,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "农业养殖",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "agriculture_food",
          "node_name": "农业养殖",
          "path": [
            "农业养殖"
          ]
        },
        {
          "name": "猪禽水产养殖",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "pig_poultry_aquaculture",
          "node_name": "猪禽水产养殖",
          "path": [
            "农业养殖",
            "猪禽水产养殖"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "猪禽水产养殖",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "pig_poultry_aquaculture",
          "node_name": "猪禽水产养殖",
          "path": [
            "农业养殖",
            "猪禽水产养殖"
          ]
        }
      ]
    },
    {
      "code": "688981",
      "name": "中芯国际",
      "batch": "NEXT44",
      "primary_industry": "半导体",
      "primary_leaf": "半导体设备",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 87.0,
      "fundamental_score": 72.0,
      "net_profit_yi": 15.94,
      "net_profit_yoy_pct": 0.0,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "半导体",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "中",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "半导体设备",
          "role": "二级赛道",
          "weight": 0.82,
          "evidence_level": "中",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "semicap",
          "node_name": "半导体设备",
          "path": [
            "半导体",
            "半导体设备"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "半导体设备",
          "role": "二级赛道",
          "weight": 0.82,
          "evidence_level": "中",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "semicap",
          "node_name": "半导体设备",
          "path": [
            "半导体",
            "半导体设备"
          ]
        }
      ]
    },
    {
      "code": "688008",
      "name": "澜起科技",
      "batch": "NEXT44",
      "primary_industry": "半导体",
      "primary_leaf": "半导体设备",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 86.0,
      "fundamental_score": 90.0,
      "net_profit_yi": 8.3,
      "net_profit_yoy_pct": 0.61,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "半导体",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "中",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "半导体设备",
          "role": "二级赛道",
          "weight": 0.82,
          "evidence_level": "中",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "semicap",
          "node_name": "半导体设备",
          "path": [
            "半导体",
            "半导体设备"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "半导体设备",
          "role": "二级赛道",
          "weight": 0.82,
          "evidence_level": "中",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "semicap",
          "node_name": "半导体设备",
          "path": [
            "半导体",
            "半导体设备"
          ]
        }
      ]
    },
    {
      "code": "300144",
      "name": "宋城演艺",
      "batch": "NEXT44",
      "primary_industry": "商贸零售 / 社服",
      "primary_leaf": "旅游/酒店",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 95.0,
      "fundamental_score": 63.0,
      "net_profit_yi": 2.18,
      "net_profit_yoy_pct": -0.15,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "商贸零售 / 社服",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "commercial_retail_social_service",
          "node_name": "商贸零售/社服",
          "path": [
            "商贸零售 / 社服"
          ]
        },
        {
          "name": "旅游 / 酒店",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "tourism_hotel",
          "node_name": "旅游/酒店",
          "path": [
            "商贸零售 / 社服",
            "旅游 / 酒店"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "旅游 / 酒店",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "tourism_hotel",
          "node_name": "旅游/酒店",
          "path": [
            "商贸零售 / 社服",
            "旅游 / 酒店"
          ]
        }
      ]
    },
    {
      "code": "601933",
      "name": "永辉超市",
      "batch": "NEXT44",
      "primary_industry": "商贸零售 / 社服",
      "primary_leaf": "零售/电商",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 94.0,
      "fundamental_score": 89.0,
      "net_profit_yi": 2.85,
      "net_profit_yoy_pct": 0.94,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "商贸零售 / 社服",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "commercial_retail_social_service",
          "node_name": "商贸零售/社服",
          "path": [
            "商贸零售 / 社服"
          ]
        },
        {
          "name": "零售 / 电商",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "retail_ecommerce",
          "node_name": "零售/电商",
          "path": [
            "商贸零售 / 社服",
            "零售 / 电商"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "零售 / 电商",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "retail_ecommerce",
          "node_name": "零售/电商",
          "path": [
            "商贸零售 / 社服",
            "零售 / 电商"
          ]
        }
      ]
    },
    {
      "code": "600258",
      "name": "首旅酒店",
      "batch": "NEXT44",
      "primary_industry": "商贸零售 / 社服",
      "primary_leaf": "旅游/酒店",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 96.0,
      "fundamental_score": 89.0,
      "net_profit_yi": 1.93,
      "net_profit_yoy_pct": 0.19,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "商贸零售 / 社服",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "commercial_retail_social_service",
          "node_name": "商贸零售/社服",
          "path": [
            "商贸零售 / 社服"
          ]
        },
        {
          "name": "旅游 / 酒店",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "tourism_hotel",
          "node_name": "旅游/酒店",
          "path": [
            "商贸零售 / 社服",
            "旅游 / 酒店"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "旅游 / 酒店",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "tourism_hotel",
          "node_name": "旅游/酒店",
          "path": [
            "商贸零售 / 社服",
            "旅游 / 酒店"
          ]
        }
      ]
    },
    {
      "code": "601668",
      "name": "中国建筑",
      "batch": "NEXT44",
      "primary_industry": "地产 / 建筑建材",
      "primary_leaf": "建筑工程/基建",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 94.0,
      "fundamental_score": 67.0,
      "net_profit_yi": 163.6,
      "net_profit_yoy_pct": -0.08,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "地产 / 建筑建材",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "real_estate_construction",
          "node_name": "地产/建筑建材",
          "path": [
            "地产 / 建筑建材"
          ]
        },
        {
          "name": "建筑工程 / 基建",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "construction_engineering",
          "node_name": "建筑工程/基建",
          "path": [
            "地产 / 建筑建材",
            "建筑工程 / 基建"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "建筑工程 / 基建",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "construction_engineering",
          "node_name": "建筑工程/基建",
          "path": [
            "地产 / 建筑建材",
            "建筑工程 / 基建"
          ]
        }
      ]
    },
    {
      "code": "600585",
      "name": "海螺水泥",
      "batch": "NEXT44",
      "primary_industry": "地产 / 建筑建材",
      "primary_leaf": "建材/水泥玻璃",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 68.0,
      "net_profit_yi": 14.42,
      "net_profit_yoy_pct": -0.19,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "地产 / 建筑建材",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "real_estate_construction",
          "node_name": "地产/建筑建材",
          "path": [
            "地产 / 建筑建材"
          ]
        },
        {
          "name": "建材 / 水泥玻璃",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "building_materials",
          "node_name": "建材/水泥玻璃",
          "path": [
            "地产 / 建筑建材",
            "建材 / 水泥玻璃"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "建材 / 水泥玻璃",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "building_materials",
          "node_name": "建材/水泥玻璃",
          "path": [
            "地产 / 建筑建材",
            "建材 / 水泥玻璃"
          ]
        }
      ]
    },
    {
      "code": "300014",
      "name": "亿纬锂能",
      "batch": "NEXT44",
      "primary_industry": "新能源汽车",
      "primary_leaf": "动力电池/储能",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 96.0,
      "fundamental_score": 84.0,
      "net_profit_yi": 14.43,
      "net_profit_yoy_pct": 0.31,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "新能源汽车",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "new_energy_vehicle",
          "node_name": "新能源汽车",
          "path": [
            "新能源汽车"
          ]
        },
        {
          "name": "动力电池 / 储能",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "battery_storage",
          "node_name": "动力电池/储能",
          "path": [
            "新能源汽车",
            "动力电池/储能"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "动力电池 / 储能",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "battery_storage",
          "node_name": "动力电池/储能",
          "path": [
            "新能源汽车",
            "动力电池/储能"
          ]
        }
      ]
    },
    {
      "code": "002747",
      "name": "埃斯顿",
      "batch": "NEXT44",
      "primary_industry": "机器人 / 自动化",
      "primary_leaf": "工业自动化 / 工业母机",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 86.0,
      "fundamental_score": 87.0,
      "net_profit_yi": 0.97,
      "net_profit_yoy_pct": 6.75,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "机器人 / 自动化",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "中",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "robotics",
          "node_name": "机器人/自动化",
          "path": [
            "机器人/自动化"
          ]
        },
        {
          "name": "工业自动化 / 工业母机",
          "role": "二级赛道",
          "weight": 0.82,
          "evidence_level": "中",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "robot_industrial_auto",
          "node_name": "工业自动化 / 工业母机",
          "path": [
            "机器人/自动化",
            "工业自动化 / 工业母机"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "工业自动化 / 工业母机",
          "role": "二级赛道",
          "weight": 0.82,
          "evidence_level": "中",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "robot_industrial_auto",
          "node_name": "工业自动化 / 工业母机",
          "path": [
            "机器人/自动化",
            "工业自动化 / 工业母机"
          ]
        }
      ]
    },
    {
      "code": "688017",
      "name": "绿的谐波",
      "batch": "NEXT44",
      "primary_industry": "机器人 / 自动化",
      "primary_leaf": "减速器 / 丝杠",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 96.0,
      "fundamental_score": 85.0,
      "net_profit_yi": 0.33,
      "net_profit_yoy_pct": 0.61,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "机器人 / 自动化",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "robotics",
          "node_name": "机器人/自动化",
          "path": [
            "机器人/自动化"
          ]
        },
        {
          "name": "减速器 / 丝杠",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "robot_reducer_screw_formal",
          "node_name": "减速器 / 丝杠",
          "path": [
            "机器人/自动化",
            "减速器 / 丝杠"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "减速器 / 丝杠",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "robot_reducer_screw_formal",
          "node_name": "减速器 / 丝杠",
          "path": [
            "机器人/自动化",
            "减速器 / 丝杠"
          ]
        }
      ]
    },
    {
      "code": "603728",
      "name": "鸣志电器",
      "batch": "NEXT44",
      "primary_industry": "机器人 / 自动化",
      "primary_leaf": "执行器",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 85.0,
      "fundamental_score": 65.0,
      "net_profit_yi": 0.14,
      "net_profit_yoy_pct": 0.92,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "机器人 / 自动化",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "中",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "robotics",
          "node_name": "机器人/自动化",
          "path": [
            "机器人/自动化"
          ]
        },
        {
          "name": "执行器",
          "role": "二级赛道",
          "weight": 0.82,
          "evidence_level": "中",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "robot_actuator_formal",
          "node_name": "执行器",
          "path": [
            "机器人/自动化",
            "执行器"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "执行器",
          "role": "二级赛道",
          "weight": 0.82,
          "evidence_level": "中",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "robot_actuator_formal",
          "node_name": "执行器",
          "path": [
            "机器人/自动化",
            "执行器"
          ]
        }
      ]
    },
    {
      "code": "300070",
      "name": "碧水源",
      "batch": "NEXT44",
      "primary_industry": "环保 / 水务固废",
      "primary_leaf": "水务/污水处理",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 94.0,
      "fundamental_score": 65.0,
      "net_profit_yi": 0.85,
      "net_profit_yoy_pct": 0.01,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "环保 / 水务固废",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "environmental_utilities",
          "node_name": "环保/水务固废",
          "path": [
            "环保 / 水务固废"
          ]
        },
        {
          "name": "水务 / 污水处理",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "water_treatment",
          "node_name": "水务/污水处理",
          "path": [
            "环保 / 水务固废",
            "水务 / 污水处理"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "水务 / 污水处理",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "water_treatment",
          "node_name": "水务/污水处理",
          "path": [
            "环保 / 水务固废",
            "水务 / 污水处理"
          ]
        }
      ]
    },
    {
      "code": "003816",
      "name": "中国广核",
      "batch": "NEXT44",
      "primary_industry": "电力与公用事业",
      "primary_leaf": "核电",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 94.0,
      "fundamental_score": 73.0,
      "net_profit_yi": 40.65,
      "net_profit_yoy_pct": -0.09,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "电力与公用事业",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "renewable_power",
          "node_name": "电力与公用事业",
          "path": [
            "电力与公用事业"
          ]
        },
        {
          "name": "核电",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "power_nuclear",
          "node_name": "核电",
          "path": [
            "电力与公用事业",
            "核电"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "核电",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "power_nuclear",
          "node_name": "核电",
          "path": [
            "电力与公用事业",
            "核电"
          ]
        }
      ]
    },
    {
      "code": "601985",
      "name": "中国核电",
      "batch": "NEXT44",
      "primary_industry": "电力与公用事业",
      "primary_leaf": "核电",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 94.0,
      "fundamental_score": 77.0,
      "net_profit_yi": 41.39,
      "net_profit_yoy_pct": -0.34,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "电力与公用事业",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "renewable_power",
          "node_name": "电力与公用事业",
          "path": [
            "电力与公用事业"
          ]
        },
        {
          "name": "核电",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "power_nuclear",
          "node_name": "核电",
          "path": [
            "电力与公用事业",
            "核电"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "核电",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "power_nuclear",
          "node_name": "核电",
          "path": [
            "电力与公用事业",
            "核电"
          ]
        }
      ]
    },
    {
      "code": "002563",
      "name": "森马服饰",
      "batch": "NEXT44",
      "primary_industry": "纺服轻工",
      "primary_leaf": "服装/纺织",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 94.0,
      "fundamental_score": 89.0,
      "net_profit_yi": 3.1,
      "net_profit_yoy_pct": 0.45,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "纺服轻工",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "textile_light_industry",
          "node_name": "纺服轻工",
          "path": [
            "纺服轻工"
          ]
        },
        {
          "name": "服装 / 纺织",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "apparel_textile",
          "node_name": "服装/纺织",
          "path": [
            "纺服轻工",
            "服装/纺织"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "服装 / 纺织",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "apparel_textile",
          "node_name": "服装/纺织",
          "path": [
            "纺服轻工",
            "服装/纺织"
          ]
        }
      ]
    },
    {
      "code": "600398",
      "name": "海澜之家",
      "batch": "NEXT44",
      "primary_industry": "纺服轻工",
      "primary_leaf": "服装/纺织",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 94.0,
      "fundamental_score": 94.0,
      "net_profit_yi": 9.19,
      "net_profit_yoy_pct": 0.02,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "纺服轻工",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "textile_light_industry",
          "node_name": "纺服轻工",
          "path": [
            "纺服轻工"
          ]
        },
        {
          "name": "服装 / 纺织",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "apparel_textile",
          "node_name": "服装/纺织",
          "path": [
            "纺服轻工",
            "服装/纺织"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "服装 / 纺织",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "apparel_textile",
          "node_name": "服装/纺织",
          "path": [
            "纺服轻工",
            "服装/纺织"
          ]
        }
      ]
    },
    {
      "code": "603816",
      "name": "顾家家居",
      "batch": "NEXT44",
      "primary_industry": "纺服轻工",
      "primary_leaf": "家居/轻工制造",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 68.0,
      "net_profit_yi": 5.04,
      "net_profit_yoy_pct": -0.04,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "纺服轻工",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "textile_light_industry",
          "node_name": "纺服轻工",
          "path": [
            "纺服轻工"
          ]
        },
        {
          "name": "家居 / 轻工制造",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "home_light_industry",
          "node_name": "家居/轻工制造",
          "path": [
            "纺服轻工",
            "家居 / 轻工制造"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "家居 / 轻工制造",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "home_light_industry",
          "node_name": "家居/轻工制造",
          "path": [
            "纺服轻工",
            "家居 / 轻工制造"
          ]
        }
      ]
    },
    {
      "code": "601288",
      "name": "农业银行",
      "batch": "NEXT44",
      "primary_industry": "金融",
      "primary_leaf": "保险 / 综合金融",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 94.0,
      "fundamental_score": 95.0,
      "net_profit_yi": 755.8,
      "net_profit_yoy_pct": 0.05,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "金融",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "finance",
          "node_name": "金融",
          "path": [
            "金融"
          ]
        },
        {
          "name": "保险 / 综合金融",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "finance_insurance_fintech",
          "node_name": "保险 / 综合金融",
          "path": [
            "金融",
            "保险 / 综合金融"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "保险 / 综合金融",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "下一轮轻量证据卡：主营/产品/财务字段自动确认",
          "node_id": "finance_insurance_fintech",
          "node_name": "保险 / 综合金融",
          "path": [
            "金融",
            "保险 / 综合金融"
          ]
        }
      ]
    },
    {
      "code": "603328",
      "name": "依顿电子",
      "batch": "P0",
      "primary_industry": "AI算力",
      "primary_leaf": "PCB/高速PCB",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 96.0,
      "fundamental_score": 67.0,
      "net_profit_yi": 0.38,
      "net_profit_yoy_pct": -0.68,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "AI算力",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "ai_compute",
          "node_name": "AI算力",
          "path": [
            "AI算力"
          ]
        },
        {
          "name": "PCB / 高速PCB",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "pcb_high_speed",
          "node_name": "PCB/高速PCB",
          "path": [
            "AI算力",
            "PCB / 高速PCB"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "PCB / 高速PCB",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "pcb_high_speed",
          "node_name": "PCB/高速PCB",
          "path": [
            "AI算力",
            "PCB / 高速PCB"
          ]
        }
      ]
    },
    {
      "code": "001389",
      "name": "广合科技",
      "batch": "P0",
      "primary_industry": "AI算力",
      "primary_leaf": "PCB/高速PCB",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 94.0,
      "fundamental_score": 85.0,
      "net_profit_yi": 3.93,
      "net_profit_yoy_pct": 0.63,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "AI算力",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "ai_compute",
          "node_name": "AI算力",
          "path": [
            "AI算力"
          ]
        },
        {
          "name": "PCB / 高速PCB",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "pcb_high_speed",
          "node_name": "PCB/高速PCB",
          "path": [
            "AI算力",
            "PCB / 高速PCB"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "PCB / 高速PCB",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "pcb_high_speed",
          "node_name": "PCB/高速PCB",
          "path": [
            "AI算力",
            "PCB / 高速PCB"
          ]
        }
      ]
    },
    {
      "code": "600009",
      "name": "上海机场",
      "batch": "P0",
      "primary_industry": "交运物流",
      "primary_leaf": "快递/物流",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 95.0,
      "fundamental_score": 94.0,
      "net_profit_yi": 6.42,
      "net_profit_yoy_pct": 0.11,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "交运物流",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "transportation_logistics",
          "node_name": "交运物流",
          "path": [
            "交运物流"
          ]
        },
        {
          "name": "机场",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "express_logistics",
          "node_name": "快递/物流",
          "path": [
            "交运物流",
            "快递 / 物流"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "机场",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "express_logistics",
          "node_name": "快递/物流",
          "path": [
            "交运物流",
            "快递 / 物流"
          ]
        }
      ]
    },
    {
      "code": "601156",
      "name": "东航物流",
      "batch": "P0",
      "primary_industry": "交运物流",
      "primary_leaf": "快递/物流",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 94.0,
      "fundamental_score": 94.0,
      "net_profit_yi": 6.58,
      "net_profit_yoy_pct": 0.06,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "交运物流",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "transportation_logistics",
          "node_name": "交运物流",
          "path": [
            "交运物流"
          ]
        },
        {
          "name": "航空物流",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "express_logistics",
          "node_name": "快递/物流",
          "path": [
            "交运物流",
            "快递 / 物流"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "航空物流",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "express_logistics",
          "node_name": "快递/物流",
          "path": [
            "交运物流",
            "快递 / 物流"
          ]
        }
      ]
    },
    {
      "code": "000582",
      "name": "北部湾港",
      "batch": "P0",
      "primary_industry": "交运物流",
      "primary_leaf": "快递/物流",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 95.0,
      "fundamental_score": 89.0,
      "net_profit_yi": 2.66,
      "net_profit_yoy_pct": 0.17,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "交运物流",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "transportation_logistics",
          "node_name": "交运物流",
          "path": [
            "交运物流"
          ]
        },
        {
          "name": "港口",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "express_logistics",
          "node_name": "快递/物流",
          "path": [
            "交运物流",
            "快递 / 物流"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "港口",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "express_logistics",
          "node_name": "快递/物流",
          "path": [
            "交运物流",
            "快递 / 物流"
          ]
        }
      ]
    },
    {
      "code": "002928",
      "name": "华夏航空",
      "batch": "P0",
      "primary_industry": "交运物流",
      "primary_leaf": "快递/物流",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 89.0,
      "net_profit_yi": 1.34,
      "net_profit_yoy_pct": 0.64,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "交运物流",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "transportation_logistics",
          "node_name": "交运物流",
          "path": [
            "交运物流"
          ]
        },
        {
          "name": "航空",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "express_logistics",
          "node_name": "快递/物流",
          "path": [
            "交运物流",
            "快递 / 物流"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "航空",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "express_logistics",
          "node_name": "快递/物流",
          "path": [
            "交运物流",
            "快递 / 物流"
          ]
        }
      ]
    },
    {
      "code": "002040",
      "name": "南京港",
      "batch": "P0",
      "primary_industry": "交运物流",
      "primary_leaf": "快递/物流",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 95.0,
      "fundamental_score": 85.0,
      "net_profit_yi": 0.64,
      "net_profit_yoy_pct": 0.28,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "交运物流",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "transportation_logistics",
          "node_name": "交运物流",
          "path": [
            "交运物流"
          ]
        },
        {
          "name": "港口",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "express_logistics",
          "node_name": "快递/物流",
          "path": [
            "交运物流",
            "快递 / 物流"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "港口",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "express_logistics",
          "node_name": "快递/物流",
          "path": [
            "交运物流",
            "快递 / 物流"
          ]
        }
      ]
    },
    {
      "code": "600029",
      "name": "南方航空",
      "batch": "P0",
      "primary_industry": "交运物流",
      "primary_leaf": "快递/物流",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 95.0,
      "net_profit_yi": 21.51,
      "net_profit_yoy_pct": 2.98,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "交运物流",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "transportation_logistics",
          "node_name": "交运物流",
          "path": [
            "交运物流"
          ]
        },
        {
          "name": "航空",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "express_logistics",
          "node_name": "快递/物流",
          "path": [
            "交运物流",
            "快递 / 物流"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "航空",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "express_logistics",
          "node_name": "快递/物流",
          "path": [
            "交运物流",
            "快递 / 物流"
          ]
        }
      ]
    },
    {
      "code": "000905",
      "name": "厦门港务",
      "batch": "P0",
      "primary_industry": "交运物流",
      "primary_leaf": "快递/物流",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 96.0,
      "fundamental_score": 63.0,
      "net_profit_yi": 2.34,
      "net_profit_yoy_pct": -0.0,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "交运物流",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "transportation_logistics",
          "node_name": "交运物流",
          "path": [
            "交运物流"
          ]
        },
        {
          "name": "港口",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "express_logistics",
          "node_name": "快递/物流",
          "path": [
            "交运物流",
            "快递 / 物流"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "港口",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "express_logistics",
          "node_name": "快递/物流",
          "path": [
            "交运物流",
            "快递 / 物流"
          ]
        }
      ]
    },
    {
      "code": "603885",
      "name": "吉祥航空",
      "batch": "P0",
      "primary_industry": "交运物流",
      "primary_leaf": "快递/物流",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 89.0,
      "net_profit_yi": 4.4,
      "net_profit_yoy_pct": 0.28,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "交运物流",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "transportation_logistics",
          "node_name": "交运物流",
          "path": [
            "交运物流"
          ]
        },
        {
          "name": "航空",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "express_logistics",
          "node_name": "快递/物流",
          "path": [
            "交运物流",
            "快递 / 物流"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "航空",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "express_logistics",
          "node_name": "快递/物流",
          "path": [
            "交运物流",
            "快递 / 物流"
          ]
        }
      ]
    },
    {
      "code": "601000",
      "name": "唐山港",
      "batch": "P0",
      "primary_industry": "交运物流",
      "primary_leaf": "快递/物流",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 94.0,
      "fundamental_score": 94.0,
      "net_profit_yi": 5.23,
      "net_profit_yoy_pct": 0.23,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "交运物流",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "transportation_logistics",
          "node_name": "交运物流",
          "path": [
            "交运物流"
          ]
        },
        {
          "name": "港口",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "express_logistics",
          "node_name": "快递/物流",
          "path": [
            "交运物流",
            "快递 / 物流"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "港口",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "express_logistics",
          "node_name": "快递/物流",
          "path": [
            "交运物流",
            "快递 / 物流"
          ]
        }
      ]
    },
    {
      "code": "600717",
      "name": "天津港",
      "batch": "P0",
      "primary_industry": "交运物流",
      "primary_leaf": "快递/物流",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 94.0,
      "fundamental_score": 90.0,
      "net_profit_yi": 5.2,
      "net_profit_yoy_pct": 0.28,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "交运物流",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "transportation_logistics",
          "node_name": "交运物流",
          "path": [
            "交运物流"
          ]
        },
        {
          "name": "港口",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "express_logistics",
          "node_name": "快递/物流",
          "path": [
            "交运物流",
            "快递 / 物流"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "港口",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "express_logistics",
          "node_name": "快递/物流",
          "path": [
            "交运物流",
            "快递 / 物流"
          ]
        }
      ]
    },
    {
      "code": "600377",
      "name": "宁沪高速",
      "batch": "P0",
      "primary_industry": "交运物流",
      "primary_leaf": "快递/物流",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 94.0,
      "net_profit_yi": 14.19,
      "net_profit_yoy_pct": 0.13,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "交运物流",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "transportation_logistics",
          "node_name": "交运物流",
          "path": [
            "交运物流"
          ]
        },
        {
          "name": "高速公路",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "express_logistics",
          "node_name": "快递/物流",
          "path": [
            "交运物流",
            "快递 / 物流"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "高速公路",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "express_logistics",
          "node_name": "快递/物流",
          "path": [
            "交运物流",
            "快递 / 物流"
          ]
        }
      ]
    },
    {
      "code": "601018",
      "name": "宁波港",
      "batch": "P0",
      "primary_industry": "交运物流",
      "primary_leaf": "快递/物流",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 95.0,
      "fundamental_score": 62.0,
      "net_profit_yi": 13.12,
      "net_profit_yoy_pct": -0.0,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "交运物流",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "transportation_logistics",
          "node_name": "交运物流",
          "path": [
            "交运物流"
          ]
        },
        {
          "name": "港口",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "express_logistics",
          "node_name": "快递/物流",
          "path": [
            "交运物流",
            "快递 / 物流"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "港口",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "express_logistics",
          "node_name": "快递/物流",
          "path": [
            "交运物流",
            "快递 / 物流"
          ]
        }
      ]
    },
    {
      "code": "601333",
      "name": "广深铁路",
      "batch": "P0",
      "primary_industry": "交运物流",
      "primary_leaf": "快递/物流",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 84.0,
      "net_profit_yi": 5.8,
      "net_profit_yoy_pct": 0.24,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "交运物流",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "transportation_logistics",
          "node_name": "交运物流",
          "path": [
            "交运物流"
          ]
        },
        {
          "name": "铁路",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "express_logistics",
          "node_name": "快递/物流",
          "path": [
            "交运物流",
            "快递 / 物流"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "铁路",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "express_logistics",
          "node_name": "快递/物流",
          "path": [
            "交运物流",
            "快递 / 物流"
          ]
        }
      ]
    },
    {
      "code": "601021",
      "name": "春秋航空",
      "batch": "P0",
      "primary_industry": "交运物流",
      "primary_leaf": "快递/物流",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 94.0,
      "net_profit_yi": 9.83,
      "net_profit_yoy_pct": 0.45,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "交运物流",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "transportation_logistics",
          "node_name": "交运物流",
          "path": [
            "交运物流"
          ]
        },
        {
          "name": "航空",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "express_logistics",
          "node_name": "快递/物流",
          "path": [
            "交运物流",
            "快递 / 物流"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "航空",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "express_logistics",
          "node_name": "快递/物流",
          "path": [
            "交运物流",
            "快递 / 物流"
          ]
        }
      ]
    },
    {
      "code": "000089",
      "name": "深圳机场",
      "batch": "P0",
      "primary_industry": "交运物流",
      "primary_leaf": "快递/物流",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 89.0,
      "net_profit_yi": 2.63,
      "net_profit_yoy_pct": 0.59,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "交运物流",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "transportation_logistics",
          "node_name": "交运物流",
          "path": [
            "交运物流"
          ]
        },
        {
          "name": "机场",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "express_logistics",
          "node_name": "快递/物流",
          "path": [
            "交运物流",
            "快递 / 物流"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "机场",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "express_logistics",
          "node_name": "快递/物流",
          "path": [
            "交运物流",
            "快递 / 物流"
          ]
        }
      ]
    },
    {
      "code": "600548",
      "name": "深高速",
      "batch": "P0",
      "primary_industry": "交运物流",
      "primary_leaf": "快递/物流",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 86.0,
      "fundamental_score": 94.0,
      "net_profit_yi": 5.55,
      "net_profit_yoy_pct": 0.07,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "交运物流",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "中",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "transportation_logistics",
          "node_name": "交运物流",
          "path": [
            "交运物流"
          ]
        },
        {
          "name": "高速公路",
          "role": "二级赛道",
          "weight": 0.82,
          "evidence_level": "中",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "express_logistics",
          "node_name": "快递/物流",
          "path": [
            "交运物流",
            "快递 / 物流"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "高速公路",
          "role": "二级赛道",
          "weight": 0.82,
          "evidence_level": "中",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "express_logistics",
          "node_name": "快递/物流",
          "path": [
            "交运物流",
            "快递 / 物流"
          ]
        }
      ]
    },
    {
      "code": "002607",
      "name": "中公教育",
      "batch": "P0",
      "primary_industry": "传媒互联网",
      "primary_leaf": "教育/人力服务",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 89.0,
      "net_profit_yi": 0.47,
      "net_profit_yoy_pct": 0.18,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "传媒互联网",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "media_internet",
          "node_name": "传媒互联网",
          "path": [
            "传媒互联网"
          ]
        },
        {
          "name": "教育 / 人力服务",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "education_human_service",
          "node_name": "教育/人力服务",
          "path": [
            "传媒互联网",
            "教育 / 人力服务"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "教育 / 人力服务",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "education_human_service",
          "node_name": "教育/人力服务",
          "path": [
            "传媒互联网",
            "教育 / 人力服务"
          ]
        }
      ]
    },
    {
      "code": "605098",
      "name": "行动教育",
      "batch": "P0",
      "primary_industry": "传媒互联网",
      "primary_leaf": "教育/人力服务",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 89.0,
      "net_profit_yi": 0.4,
      "net_profit_yoy_pct": 0.38,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "传媒互联网",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "media_internet",
          "node_name": "传媒互联网",
          "path": [
            "传媒互联网"
          ]
        },
        {
          "name": "教育 / 人力服务",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "education_human_service",
          "node_name": "教育/人力服务",
          "path": [
            "传媒互联网",
            "教育 / 人力服务"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "教育 / 人力服务",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "education_human_service",
          "node_name": "教育/人力服务",
          "path": [
            "传媒互联网",
            "教育 / 人力服务"
          ]
        }
      ]
    },
    {
      "code": "002041",
      "name": "登海种业",
      "batch": "P0",
      "primary_industry": "农业养殖",
      "primary_leaf": "种业/种植",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 95.0,
      "fundamental_score": 79.0,
      "net_profit_yi": 0.37,
      "net_profit_yoy_pct": 0.37,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "农业养殖",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "agriculture_food",
          "node_name": "农业养殖",
          "path": [
            "农业养殖"
          ]
        },
        {
          "name": "种业 / 种植",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "seed_planting",
          "node_name": "种业/种植",
          "path": [
            "农业养殖",
            "种业 / 种植"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "种业 / 种植",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "seed_planting",
          "node_name": "种业/种植",
          "path": [
            "农业养殖",
            "种业 / 种植"
          ]
        }
      ]
    },
    {
      "code": "001313",
      "name": "粤海饲料",
      "batch": "P0",
      "primary_industry": "农业养殖",
      "primary_leaf": "猪禽水产养殖",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 94.0,
      "fundamental_score": 69.0,
      "net_profit_yi": 0.12,
      "net_profit_yoy_pct": 1.46,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "农业养殖",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "agriculture_food",
          "node_name": "农业养殖",
          "path": [
            "农业养殖"
          ]
        },
        {
          "name": "饲料 / 水产饲料",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "pig_poultry_aquaculture",
          "node_name": "猪禽水产养殖",
          "path": [
            "农业养殖",
            "猪禽水产养殖"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "饲料 / 水产饲料",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "pig_poultry_aquaculture",
          "node_name": "猪禽水产养殖",
          "path": [
            "农业养殖",
            "猪禽水产养殖"
          ]
        }
      ]
    },
    {
      "code": "002326",
      "name": "永太科技",
      "batch": "P0",
      "primary_industry": "化工材料",
      "primary_leaf": "新材料化工",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 84.0,
      "fundamental_score": 93.0,
      "net_profit_yi": 1.22,
      "net_profit_yoy_pct": 8.9,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "化工材料",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "中",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "chemical_materials",
          "node_name": "化工材料",
          "path": [
            "化工材料"
          ]
        },
        {
          "name": "氟化工 / 电子化学品",
          "role": "二级赛道",
          "weight": 0.82,
          "evidence_level": "中",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "chemical_new_material",
          "node_name": "新材料化工",
          "path": [
            "化工材料",
            "新材料化工"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "氟化工 / 电子化学品",
          "role": "二级赛道",
          "weight": 0.82,
          "evidence_level": "中",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "chemical_new_material",
          "node_name": "新材料化工",
          "path": [
            "化工材料",
            "新材料化工"
          ]
        }
      ]
    },
    {
      "code": "002250",
      "name": "联化科技",
      "batch": "P0",
      "primary_industry": "化工材料",
      "primary_leaf": "农化/化肥农药",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 94.0,
      "fundamental_score": 83.0,
      "net_profit_yi": 1.41,
      "net_profit_yoy_pct": 1.05,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "化工材料",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "chemical_materials",
          "node_name": "化工材料",
          "path": [
            "化工材料"
          ]
        },
        {
          "name": "农化 / 化肥农药",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "fertilizer_pesticide",
          "node_name": "农化/化肥农药",
          "path": [
            "化工材料",
            "农化 / 化肥农药"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "农化 / 化肥农药",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "fertilizer_pesticide",
          "node_name": "农化/化肥农药",
          "path": [
            "化工材料",
            "农化 / 化肥农药"
          ]
        }
      ]
    },
    {
      "code": "601208",
      "name": "东材科技",
      "batch": "P0",
      "primary_industry": "半导体",
      "primary_leaf": "半导体材料",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 94.0,
      "fundamental_score": 83.0,
      "net_profit_yi": 1.84,
      "net_profit_yoy_pct": 1.03,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "半导体",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "半导体材料",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "semimaterial",
          "node_name": "半导体材料",
          "path": [
            "半导体",
            "半导体材料"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "半导体材料",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "semimaterial",
          "node_name": "半导体材料",
          "path": [
            "半导体",
            "半导体材料"
          ]
        }
      ]
    },
    {
      "code": "600563",
      "name": "法拉电子",
      "batch": "P0",
      "primary_industry": "半导体",
      "primary_leaf": "模拟/功率/MCU",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 94.0,
      "fundamental_score": 85.0,
      "net_profit_yi": 2.68,
      "net_profit_yoy_pct": 0.01,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "半导体",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "模拟 / 功率 / MCU",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "analog_power",
          "node_name": "模拟/功率/MCU",
          "path": [
            "半导体",
            "模拟 / 功率 / MCU"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "模拟 / 功率 / MCU",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "analog_power",
          "node_name": "模拟/功率/MCU",
          "path": [
            "半导体",
            "模拟 / 功率 / MCU"
          ]
        }
      ]
    },
    {
      "code": "603005",
      "name": "晶方科技",
      "batch": "P0",
      "primary_industry": "半导体产业链",
      "primary_leaf": "封装测试/先进封装",
      "mount_count": 3,
      "root_count": 1,
      "leaf_count": 2,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 86.0,
      "fundamental_score": 67.0,
      "net_profit_yi": 0.66,
      "net_profit_yoy_pct": 0.0,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "半导体产业链",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "封装测试",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "advanced_packaging",
          "node_name": "封装测试/先进封装",
          "path": [
            "半导体",
            "封装测试/先进封装"
          ]
        },
        {
          "name": "CIS封装",
          "role": "核心概念",
          "weight": 0.86,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "advanced_packaging",
          "node_name": "封装测试/先进封装",
          "path": [
            "半导体",
            "封装测试/先进封装"
          ]
        },
        {
          "name": "半导体",
          "role": "核心概念",
          "weight": 0.86,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "半导体",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "中",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "封测 / 设备零部件",
          "role": "二级赛道",
          "weight": 0.82,
          "evidence_level": "中",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "semicap",
          "node_name": "半导体设备",
          "path": [
            "半导体",
            "半导体设备"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "封装测试",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "advanced_packaging",
          "node_name": "封装测试/先进封装",
          "path": [
            "半导体",
            "封装测试/先进封装"
          ]
        },
        {
          "name": "CIS封装",
          "role": "核心概念",
          "weight": 0.86,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "advanced_packaging",
          "node_name": "封装测试/先进封装",
          "path": [
            "半导体",
            "封装测试/先进封装"
          ]
        },
        {
          "name": "封测 / 设备零部件",
          "role": "二级赛道",
          "weight": 0.82,
          "evidence_level": "中",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "semicap",
          "node_name": "半导体设备",
          "path": [
            "半导体",
            "半导体设备"
          ]
        }
      ]
    },
    {
      "code": "002409",
      "name": "雅克科技",
      "batch": "P0",
      "primary_industry": "半导体产业链",
      "primary_leaf": "半导体材料",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 79.0,
      "net_profit_yi": 2.99,
      "net_profit_yoy_pct": 0.02,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "半导体产业链",
          "role": "主行业",
          "weight": 1,
          "evidence_level": "高",
          "basis": "报告主分类",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "半导体材料",
          "role": "二级赛道",
          "weight": 0.9,
          "evidence_level": "高",
          "basis": "报告二级分类",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "电子特气",
          "role": "核心概念",
          "weight": 0.86,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "半导体",
          "role": "核心概念",
          "weight": 0.86,
          "evidence_level": "高",
          "basis": "核心赛道",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "半导体",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "semiconductor",
          "node_name": "半导体",
          "path": [
            "半导体"
          ]
        },
        {
          "name": "半导体材料",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "semimaterial",
          "node_name": "半导体材料",
          "path": [
            "半导体",
            "半导体材料"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "半导体材料",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "semimaterial",
          "node_name": "半导体材料",
          "path": [
            "半导体",
            "半导体材料"
          ]
        }
      ]
    },
    {
      "code": "601888",
      "name": "中国中免",
      "batch": "P0",
      "primary_industry": "商贸零售 / 社服",
      "primary_leaf": "零售/电商",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 95.0,
      "fundamental_score": 95.0,
      "net_profit_yi": 23.7,
      "net_profit_yoy_pct": 0.21,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "商贸零售",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "commercial_retail_social_service",
          "node_name": "商贸零售/社服",
          "path": [
            "商贸零售 / 社服"
          ]
        },
        {
          "name": "免税 / 旅游零售",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "retail_ecommerce",
          "node_name": "零售/电商",
          "path": [
            "商贸零售 / 社服",
            "零售 / 电商"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "免税 / 旅游零售",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "retail_ecommerce",
          "node_name": "零售/电商",
          "path": [
            "商贸零售 / 社服",
            "零售 / 电商"
          ]
        }
      ]
    },
    {
      "code": "600415",
      "name": "小商品城",
      "batch": "P0",
      "primary_industry": "商贸零售 / 社服",
      "primary_leaf": "零售/电商",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 90.0,
      "net_profit_yi": 9.93,
      "net_profit_yoy_pct": 0.23,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "商贸零售",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "commercial_retail_social_service",
          "node_name": "商贸零售/社服",
          "path": [
            "商贸零售 / 社服"
          ]
        },
        {
          "name": "商品市场 / 零售",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "retail_ecommerce",
          "node_name": "零售/电商",
          "path": [
            "商贸零售 / 社服",
            "零售 / 电商"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "商品市场 / 零售",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "retail_ecommerce",
          "node_name": "零售/电商",
          "path": [
            "商贸零售 / 社服",
            "零售 / 电商"
          ]
        }
      ]
    },
    {
      "code": "600754",
      "name": "锦江酒店",
      "batch": "P0",
      "primary_industry": "商贸零售 / 社服",
      "primary_leaf": "旅游/酒店",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 94.0,
      "fundamental_score": 93.0,
      "net_profit_yi": 1.51,
      "net_profit_yoy_pct": 2.8,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "商贸零售",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "commercial_retail_social_service",
          "node_name": "商贸零售/社服",
          "path": [
            "商贸零售 / 社服"
          ]
        },
        {
          "name": "酒店",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "tourism_hotel",
          "node_name": "旅游/酒店",
          "path": [
            "商贸零售 / 社服",
            "旅游 / 酒店"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "酒店",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "tourism_hotel",
          "node_name": "旅游/酒店",
          "path": [
            "商贸零售 / 社服",
            "旅游 / 酒店"
          ]
        }
      ]
    },
    {
      "code": "601789",
      "name": "宁波建工",
      "batch": "P0",
      "primary_industry": "地产 / 建筑建材",
      "primary_leaf": "建筑工程/基建",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 79.0,
      "net_profit_yi": 1.89,
      "net_profit_yoy_pct": 0.77,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "地产",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "real_estate_construction",
          "node_name": "地产/建筑建材",
          "path": [
            "地产 / 建筑建材"
          ]
        },
        {
          "name": "建筑工程 / 基建",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "construction_engineering",
          "node_name": "建筑工程/基建",
          "path": [
            "地产 / 建筑建材",
            "建筑工程 / 基建"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "建筑工程 / 基建",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "construction_engineering",
          "node_name": "建筑工程/基建",
          "path": [
            "地产 / 建筑建材",
            "建筑工程 / 基建"
          ]
        }
      ]
    },
    {
      "code": "600502",
      "name": "安徽建工",
      "batch": "P0",
      "primary_industry": "地产 / 建筑建材",
      "primary_leaf": "建筑工程/基建",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 94.0,
      "fundamental_score": 79.0,
      "net_profit_yi": 4.64,
      "net_profit_yoy_pct": 0.07,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "地产",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "real_estate_construction",
          "node_name": "地产/建筑建材",
          "path": [
            "地产 / 建筑建材"
          ]
        },
        {
          "name": "建筑工程 / 基建",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "construction_engineering",
          "node_name": "建筑工程/基建",
          "path": [
            "地产 / 建筑建材",
            "建筑工程 / 基建"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "建筑工程 / 基建",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "construction_engineering",
          "node_name": "建筑工程/基建",
          "path": [
            "地产 / 建筑建材",
            "建筑工程 / 基建"
          ]
        }
      ]
    },
    {
      "code": "600463",
      "name": "空港股份",
      "batch": "P0",
      "primary_industry": "地产 / 建筑建材",
      "primary_leaf": "物业/园区运营",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 79.0,
      "net_profit_yi": 0.04,
      "net_profit_yoy_pct": 0.72,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "地产",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "real_estate_construction",
          "node_name": "地产/建筑建材",
          "path": [
            "地产 / 建筑建材"
          ]
        },
        {
          "name": "园区 / 物业运营",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "property_management",
          "node_name": "物业/园区运营",
          "path": [
            "地产 / 建筑建材",
            "物业 / 园区运营"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "园区 / 物业运营",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "property_management",
          "node_name": "物业/园区运营",
          "path": [
            "地产 / 建筑建材",
            "物业 / 园区运营"
          ]
        }
      ]
    },
    {
      "code": "002590",
      "name": "万安科技",
      "batch": "P0",
      "primary_industry": "新能源汽车",
      "primary_leaf": "汽车零部件",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 89.0,
      "net_profit_yi": 0.34,
      "net_profit_yoy_pct": 0.01,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "新能源汽车",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "new_energy_vehicle",
          "node_name": "新能源汽车",
          "path": [
            "新能源汽车"
          ]
        },
        {
          "name": "汽车零部件",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "vehicle_parts",
          "node_name": "汽车零部件",
          "path": [
            "新能源汽车",
            "汽车零部件"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "汽车零部件",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "vehicle_parts",
          "node_name": "汽车零部件",
          "path": [
            "新能源汽车",
            "汽车零部件"
          ]
        }
      ]
    },
    {
      "code": "600699",
      "name": "均胜电子",
      "batch": "P0",
      "primary_industry": "新能源汽车",
      "primary_leaf": "汽车零部件",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 96.0,
      "fundamental_score": 89.0,
      "net_profit_yi": 4.63,
      "net_profit_yoy_pct": 0.18,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "新能源汽车",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "new_energy_vehicle",
          "node_name": "新能源汽车",
          "path": [
            "新能源汽车"
          ]
        },
        {
          "name": "汽车零部件",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "vehicle_parts",
          "node_name": "汽车零部件",
          "path": [
            "新能源汽车",
            "汽车零部件"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "汽车零部件",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "vehicle_parts",
          "node_name": "汽车零部件",
          "path": [
            "新能源汽车",
            "汽车零部件"
          ]
        }
      ]
    },
    {
      "code": "000550",
      "name": "江铃汽车",
      "batch": "P0",
      "primary_industry": "新能源汽车",
      "primary_leaf": "整车",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 96.0,
      "fundamental_score": 79.0,
      "net_profit_yi": 3.38,
      "net_profit_yoy_pct": 0.11,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "新能源汽车",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "new_energy_vehicle",
          "node_name": "新能源汽车",
          "path": [
            "新能源汽车"
          ]
        },
        {
          "name": "整车 / 商用车",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "whole_vehicle",
          "node_name": "整车",
          "path": [
            "新能源汽车",
            "整车"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "整车 / 商用车",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "whole_vehicle",
          "node_name": "整车",
          "path": [
            "新能源汽车",
            "整车"
          ]
        }
      ]
    },
    {
      "code": "600166",
      "name": "福田汽车",
      "batch": "P0",
      "primary_industry": "新能源汽车",
      "primary_leaf": "整车",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 89.0,
      "net_profit_yi": 4.79,
      "net_profit_yoy_pct": 0.15,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "新能源汽车",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "new_energy_vehicle",
          "node_name": "新能源汽车",
          "path": [
            "新能源汽车"
          ]
        },
        {
          "name": "整车 / 商用车",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "whole_vehicle",
          "node_name": "整车",
          "path": [
            "新能源汽车",
            "整车"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "整车 / 商用车",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "whole_vehicle",
          "node_name": "整车",
          "path": [
            "新能源汽车",
            "整车"
          ]
        }
      ]
    },
    {
      "code": "601827",
      "name": "三峰环境",
      "batch": "P0",
      "primary_industry": "环保 / 水务固废",
      "primary_leaf": "固废/再生资源",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 96.0,
      "fundamental_score": 85.0,
      "net_profit_yi": 4.63,
      "net_profit_yoy_pct": 0.05,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "环保",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "environmental_utilities",
          "node_name": "环保/水务固废",
          "path": [
            "环保 / 水务固废"
          ]
        },
        {
          "name": "固废 / 垃圾焚烧",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "solid_waste_recycling",
          "node_name": "固废/再生资源",
          "path": [
            "环保 / 水务固废",
            "固废 / 再生资源"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "固废 / 垃圾焚烧",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "solid_waste_recycling",
          "node_name": "固废/再生资源",
          "path": [
            "环保 / 水务固废",
            "固废 / 再生资源"
          ]
        }
      ]
    },
    {
      "code": "601200",
      "name": "上海环境",
      "batch": "P0",
      "primary_industry": "环保 / 水务固废",
      "primary_leaf": "环保工程",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 87.0,
      "fundamental_score": 63.0,
      "net_profit_yi": 1.88,
      "net_profit_yoy_pct": 0.0,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "环保",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "中",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "environmental_utilities",
          "node_name": "环保/水务固废",
          "path": [
            "环保 / 水务固废"
          ]
        },
        {
          "name": "环保综合",
          "role": "二级赛道",
          "weight": 0.82,
          "evidence_level": "中",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "environmental_engineering",
          "node_name": "环保工程",
          "path": [
            "环保 / 水务固废",
            "环保工程"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "环保综合",
          "role": "二级赛道",
          "weight": 0.82,
          "evidence_level": "中",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "environmental_engineering",
          "node_name": "环保工程",
          "path": [
            "环保 / 水务固废",
            "环保工程"
          ]
        }
      ]
    },
    {
      "code": "600874",
      "name": "创业环保",
      "batch": "P0",
      "primary_industry": "环保 / 水务固废",
      "primary_leaf": "水务/污水处理",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 95.0,
      "fundamental_score": 79.0,
      "net_profit_yi": 2.9,
      "net_profit_yoy_pct": 0.08,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "环保",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "environmental_utilities",
          "node_name": "环保/水务固废",
          "path": [
            "环保 / 水务固废"
          ]
        },
        {
          "name": "水务 / 污水处理",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "water_treatment",
          "node_name": "水务/污水处理",
          "path": [
            "环保 / 水务固废",
            "水务 / 污水处理"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "水务 / 污水处理",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "water_treatment",
          "node_name": "水务/污水处理",
          "path": [
            "环保 / 水务固废",
            "水务 / 污水处理"
          ]
        }
      ]
    },
    {
      "code": "000885",
      "name": "城发环境",
      "batch": "P0",
      "primary_industry": "环保 / 水务固废",
      "primary_leaf": "环保工程",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 85.0,
      "net_profit_yi": 3.43,
      "net_profit_yoy_pct": 0.15,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "环保",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "environmental_utilities",
          "node_name": "环保/水务固废",
          "path": [
            "环保 / 水务固废"
          ]
        },
        {
          "name": "环保综合",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "environmental_engineering",
          "node_name": "环保工程",
          "path": [
            "环保 / 水务固废",
            "环保工程"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "环保综合",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "environmental_engineering",
          "node_name": "环保工程",
          "path": [
            "环保 / 水务固废",
            "环保工程"
          ]
        }
      ]
    },
    {
      "code": "002034",
      "name": "旺能环境",
      "batch": "P0",
      "primary_industry": "环保 / 水务固废",
      "primary_leaf": "固废/再生资源",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 96.0,
      "fundamental_score": 67.0,
      "net_profit_yi": 2.04,
      "net_profit_yoy_pct": 0.0,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "环保",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "environmental_utilities",
          "node_name": "环保/水务固废",
          "path": [
            "环保 / 水务固废"
          ]
        },
        {
          "name": "固废 / 垃圾焚烧",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "solid_waste_recycling",
          "node_name": "固废/再生资源",
          "path": [
            "环保 / 水务固废",
            "固废 / 再生资源"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "固废 / 垃圾焚烧",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "solid_waste_recycling",
          "node_name": "固废/再生资源",
          "path": [
            "环保 / 水务固废",
            "固废 / 再生资源"
          ]
        }
      ]
    },
    {
      "code": "002340",
      "name": "格林美",
      "batch": "P0",
      "primary_industry": "环保 / 水务固废",
      "primary_leaf": "固废/再生资源",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 90.0,
      "net_profit_yi": 6.48,
      "net_profit_yoy_pct": 0.02,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "环保",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "environmental_utilities",
          "node_name": "环保/水务固废",
          "path": [
            "环保 / 水务固废"
          ]
        },
        {
          "name": "固废 / 再生资源",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "solid_waste_recycling",
          "node_name": "固废/再生资源",
          "path": [
            "环保 / 水务固废",
            "固废 / 再生资源"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "固废 / 再生资源",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "solid_waste_recycling",
          "node_name": "固废/再生资源",
          "path": [
            "环保 / 水务固废",
            "固废 / 再生资源"
          ]
        }
      ]
    },
    {
      "code": "600323",
      "name": "瀚蓝环境",
      "batch": "P0",
      "primary_industry": "环保 / 水务固废",
      "primary_leaf": "水务/污水处理",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 95.0,
      "fundamental_score": 90.0,
      "net_profit_yi": 7.13,
      "net_profit_yoy_pct": 0.38,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "环保",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "environmental_utilities",
          "node_name": "环保/水务固废",
          "path": [
            "环保 / 水务固废"
          ]
        },
        {
          "name": "水务 / 固废",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "water_treatment",
          "node_name": "水务/污水处理",
          "path": [
            "环保 / 水务固废",
            "水务 / 污水处理"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "水务 / 固废",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "water_treatment",
          "node_name": "水务/污水处理",
          "path": [
            "环保 / 水务固废",
            "水务 / 污水处理"
          ]
        }
      ]
    },
    {
      "code": "000967",
      "name": "盈峰环境",
      "batch": "P0",
      "primary_industry": "环保 / 水务固废",
      "primary_leaf": "环保工程",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 94.0,
      "fundamental_score": 79.0,
      "net_profit_yi": 2.2,
      "net_profit_yoy_pct": 0.19,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "环保",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "environmental_utilities",
          "node_name": "环保/水务固废",
          "path": [
            "环保 / 水务固废"
          ]
        },
        {
          "name": "环卫装备 / 环境服务",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "environmental_engineering",
          "node_name": "环保工程",
          "path": [
            "环保 / 水务固废",
            "环保工程"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "环卫装备 / 环境服务",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "environmental_engineering",
          "node_name": "环保工程",
          "path": [
            "环保 / 水务固废",
            "环保工程"
          ]
        }
      ]
    },
    {
      "code": "600388",
      "name": "龙净环保",
      "batch": "P0",
      "primary_industry": "环保 / 水务固废",
      "primary_leaf": "环保工程",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 79.0,
      "net_profit_yi": 2.45,
      "net_profit_yoy_pct": 0.32,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "环保",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "environmental_utilities",
          "node_name": "环保/水务固废",
          "path": [
            "环保 / 水务固废"
          ]
        },
        {
          "name": "大气治理",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "environmental_engineering",
          "node_name": "环保工程",
          "path": [
            "环保 / 水务固废",
            "环保工程"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "大气治理",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "environmental_engineering",
          "node_name": "环保工程",
          "path": [
            "环保 / 水务固废",
            "环保工程"
          ]
        }
      ]
    },
    {
      "code": "600578",
      "name": "京能电力",
      "batch": "P0",
      "primary_industry": "电力与公用事业",
      "primary_leaf": "火电 / 煤电",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 94.0,
      "net_profit_yi": 13.56,
      "net_profit_yoy_pct": 0.02,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "电力与公用事业",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "renewable_power",
          "node_name": "电力与公用事业",
          "path": [
            "电力与公用事业"
          ]
        },
        {
          "name": "火电 / 煤电",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "power_thermal",
          "node_name": "火电 / 煤电",
          "path": [
            "电力与公用事业",
            "火电 / 煤电"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "火电 / 煤电",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "power_thermal",
          "node_name": "火电 / 煤电",
          "path": [
            "电力与公用事业",
            "火电 / 煤电"
          ]
        }
      ]
    },
    {
      "code": "600726",
      "name": "华电能源",
      "batch": "P0",
      "primary_industry": "电力与公用事业",
      "primary_leaf": "火电 / 煤电",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 90.0,
      "net_profit_yi": 8.83,
      "net_profit_yoy_pct": 0.26,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "电力与公用事业",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "renewable_power",
          "node_name": "电力与公用事业",
          "path": [
            "电力与公用事业"
          ]
        },
        {
          "name": "火电 / 煤电",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "power_thermal",
          "node_name": "火电 / 煤电",
          "path": [
            "电力与公用事业",
            "火电 / 煤电"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "火电 / 煤电",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "power_thermal",
          "node_name": "火电 / 煤电",
          "path": [
            "电力与公用事业",
            "火电 / 煤电"
          ]
        }
      ]
    },
    {
      "code": "600744",
      "name": "华银电力",
      "batch": "P0",
      "primary_industry": "电力与公用事业",
      "primary_leaf": "火电 / 煤电",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 89.0,
      "net_profit_yi": 1.5,
      "net_profit_yoy_pct": 0.83,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "电力与公用事业",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "renewable_power",
          "node_name": "电力与公用事业",
          "path": [
            "电力与公用事业"
          ]
        },
        {
          "name": "火电 / 煤电",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "power_thermal",
          "node_name": "火电 / 煤电",
          "path": [
            "电力与公用事业",
            "火电 / 煤电"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "火电 / 煤电",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "power_thermal",
          "node_name": "火电 / 煤电",
          "path": [
            "电力与公用事业",
            "火电 / 煤电"
          ]
        }
      ]
    },
    {
      "code": "000690",
      "name": "宝新能源",
      "batch": "P0",
      "primary_industry": "电力与公用事业",
      "primary_leaf": "火电 / 煤电",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 86.0,
      "fundamental_score": 89.0,
      "net_profit_yi": 3.1,
      "net_profit_yoy_pct": 0.03,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "电力与公用事业",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "中",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "renewable_power",
          "node_name": "电力与公用事业",
          "path": [
            "电力与公用事业"
          ]
        },
        {
          "name": "新能源发电",
          "role": "二级赛道",
          "weight": 0.82,
          "evidence_level": "中",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "power_thermal",
          "node_name": "火电 / 煤电",
          "path": [
            "电力与公用事业",
            "火电 / 煤电"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "新能源发电",
          "role": "二级赛道",
          "weight": 0.82,
          "evidence_level": "中",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "power_thermal",
          "node_name": "火电 / 煤电",
          "path": [
            "电力与公用事业",
            "火电 / 煤电"
          ]
        }
      ]
    },
    {
      "code": "000767",
      "name": "晋控电力",
      "batch": "P0",
      "primary_industry": "电力与公用事业",
      "primary_leaf": "火电 / 煤电",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 94.0,
      "fundamental_score": 89.0,
      "net_profit_yi": 0.56,
      "net_profit_yoy_pct": 0.04,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "电力与公用事业",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "renewable_power",
          "node_name": "电力与公用事业",
          "path": [
            "电力与公用事业"
          ]
        },
        {
          "name": "火电 / 煤电",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "power_thermal",
          "node_name": "火电 / 煤电",
          "path": [
            "电力与公用事业",
            "火电 / 煤电"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "火电 / 煤电",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "power_thermal",
          "node_name": "火电 / 煤电",
          "path": [
            "电力与公用事业",
            "火电 / 煤电"
          ]
        }
      ]
    },
    {
      "code": "600236",
      "name": "桂冠电力",
      "batch": "P0",
      "primary_industry": "电力与公用事业",
      "primary_leaf": "水电",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 95.0,
      "fundamental_score": 94.0,
      "net_profit_yi": 9.79,
      "net_profit_yoy_pct": 0.58,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "电力与公用事业",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "renewable_power",
          "node_name": "电力与公用事业",
          "path": [
            "电力与公用事业"
          ]
        },
        {
          "name": "水电",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "power_hydro",
          "node_name": "水电",
          "path": [
            "电力与公用事业",
            "水电"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "水电",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "power_hydro",
          "node_name": "水电",
          "path": [
            "电力与公用事业",
            "水电"
          ]
        }
      ]
    },
    {
      "code": "000027",
      "name": "深圳能源",
      "batch": "P0",
      "primary_industry": "电力与公用事业",
      "primary_leaf": "火电 / 煤电",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 85.0,
      "fundamental_score": 68.0,
      "net_profit_yi": 16.38,
      "net_profit_yoy_pct": -0.08,
      "improvement": "盈利但同比未改善",
      "tags": [
        {
          "name": "电力与公用事业",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "中",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "renewable_power",
          "node_name": "电力与公用事业",
          "path": [
            "电力与公用事业"
          ]
        },
        {
          "name": "综合电力",
          "role": "二级赛道",
          "weight": 0.82,
          "evidence_level": "中",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "power_thermal",
          "node_name": "火电 / 煤电",
          "path": [
            "电力与公用事业",
            "火电 / 煤电"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "综合电力",
          "role": "二级赛道",
          "weight": 0.82,
          "evidence_level": "中",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "power_thermal",
          "node_name": "火电 / 煤电",
          "path": [
            "电力与公用事业",
            "火电 / 煤电"
          ]
        }
      ]
    },
    {
      "code": "000883",
      "name": "湖北能源",
      "batch": "P0",
      "primary_industry": "电力与公用事业",
      "primary_leaf": "火电 / 煤电",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 93.0,
      "fundamental_score": 94.0,
      "net_profit_yi": 6.06,
      "net_profit_yoy_pct": 0.42,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "电力与公用事业",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "renewable_power",
          "node_name": "电力与公用事业",
          "path": [
            "电力与公用事业"
          ]
        },
        {
          "name": "综合电力",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "power_thermal",
          "node_name": "火电 / 煤电",
          "path": [
            "电力与公用事业",
            "火电 / 煤电"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "综合电力",
          "role": "二级赛道",
          "weight": 0.92,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "power_thermal",
          "node_name": "火电 / 煤电",
          "path": [
            "电力与公用事业",
            "火电 / 煤电"
          ]
        }
      ]
    },
    {
      "code": "000791",
      "name": "甘肃能源",
      "batch": "P0",
      "primary_industry": "电力与公用事业",
      "primary_leaf": "火电 / 煤电",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 94.0,
      "fundamental_score": 94.0,
      "net_profit_yi": 7.68,
      "net_profit_yoy_pct": 0.02,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "电力与公用事业",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "renewable_power",
          "node_name": "电力与公用事业",
          "path": [
            "电力与公用事业"
          ]
        },
        {
          "name": "综合电力",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "power_thermal",
          "node_name": "火电 / 煤电",
          "path": [
            "电力与公用事业",
            "火电 / 煤电"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "综合电力",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "power_thermal",
          "node_name": "火电 / 煤电",
          "path": [
            "电力与公用事业",
            "火电 / 煤电"
          ]
        }
      ]
    },
    {
      "code": "600966",
      "name": "博汇纸业",
      "batch": "P0",
      "primary_industry": "纺服轻工",
      "primary_leaf": "造纸/印刷",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 95.0,
      "fundamental_score": 89.0,
      "net_profit_yi": 0.55,
      "net_profit_yoy_pct": 0.04,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "纺服轻工",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "textile_light_industry",
          "node_name": "纺服轻工",
          "path": [
            "纺服轻工"
          ]
        },
        {
          "name": "造纸 / 印刷",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "paper_printing",
          "node_name": "造纸/印刷",
          "path": [
            "纺服轻工",
            "造纸 / 印刷"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "造纸 / 印刷",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "paper_printing",
          "node_name": "造纸/印刷",
          "path": [
            "纺服轻工",
            "造纸 / 印刷"
          ]
        }
      ]
    },
    {
      "code": "002969",
      "name": "嘉美包装",
      "batch": "P0",
      "primary_industry": "纺服轻工",
      "primary_leaf": "家居/轻工制造",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 95.0,
      "fundamental_score": 93.0,
      "net_profit_yi": 0.67,
      "net_profit_yoy_pct": 4.32,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "纺服轻工",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "textile_light_industry",
          "node_name": "纺服轻工",
          "path": [
            "纺服轻工"
          ]
        },
        {
          "name": "包装",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "home_light_industry",
          "node_name": "家居/轻工制造",
          "path": [
            "纺服轻工",
            "家居 / 轻工制造"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "包装",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "home_light_industry",
          "node_name": "家居/轻工制造",
          "path": [
            "纺服轻工",
            "家居 / 轻工制造"
          ]
        }
      ]
    },
    {
      "code": "002078",
      "name": "太阳纸业",
      "batch": "P0",
      "primary_industry": "纺服轻工",
      "primary_leaf": "造纸/印刷",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 94.0,
      "fundamental_score": 90.0,
      "net_profit_yi": 9.1,
      "net_profit_yoy_pct": 0.02,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "纺服轻工",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "textile_light_industry",
          "node_name": "纺服轻工",
          "path": [
            "纺服轻工"
          ]
        },
        {
          "name": "造纸 / 印刷",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "paper_printing",
          "node_name": "造纸/印刷",
          "path": [
            "纺服轻工",
            "造纸 / 印刷"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "造纸 / 印刷",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "paper_printing",
          "node_name": "造纸/印刷",
          "path": [
            "纺服轻工",
            "造纸 / 印刷"
          ]
        }
      ]
    },
    {
      "code": "002067",
      "name": "景兴纸业",
      "batch": "P0",
      "primary_industry": "纺服轻工",
      "primary_leaf": "造纸/印刷",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 94.0,
      "fundamental_score": 79.0,
      "net_profit_yi": 0.22,
      "net_profit_yoy_pct": 1.91,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "纺服轻工",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "textile_light_industry",
          "node_name": "纺服轻工",
          "path": [
            "纺服轻工"
          ]
        },
        {
          "name": "造纸 / 印刷",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "paper_printing",
          "node_name": "造纸/印刷",
          "path": [
            "纺服轻工",
            "造纸 / 印刷"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "造纸 / 印刷",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "paper_printing",
          "node_name": "造纸/印刷",
          "path": [
            "纺服轻工",
            "造纸 / 印刷"
          ]
        }
      ]
    },
    {
      "code": "002831",
      "name": "裕同科技",
      "batch": "P0",
      "primary_industry": "纺服轻工",
      "primary_leaf": "家居/轻工制造",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 95.0,
      "fundamental_score": 89.0,
      "net_profit_yi": 2.48,
      "net_profit_yoy_pct": 0.02,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "纺服轻工",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "textile_light_industry",
          "node_name": "纺服轻工",
          "path": [
            "纺服轻工"
          ]
        },
        {
          "name": "包装",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "home_light_industry",
          "node_name": "家居/轻工制造",
          "path": [
            "纺服轻工",
            "家居 / 轻工制造"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "包装",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "home_light_industry",
          "node_name": "家居/轻工制造",
          "path": [
            "纺服轻工",
            "家居 / 轻工制造"
          ]
        }
      ]
    },
    {
      "code": "002065",
      "name": "东华软件",
      "batch": "P0",
      "primary_industry": "计算机 / 软件服务",
      "primary_leaf": "行业软件/应用软件",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 94.0,
      "fundamental_score": 89.0,
      "net_profit_yi": 0.68,
      "net_profit_yoy_pct": 0.12,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "计算机",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "computer_software_it",
          "node_name": "计算机/软件服务",
          "path": [
            "计算机 / 软件服务"
          ]
        },
        {
          "name": "行业软件 / 应用软件",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "software_application",
          "node_name": "行业软件/应用软件",
          "path": [
            "计算机 / 软件服务",
            "行业软件 / 应用软件"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "行业软件 / 应用软件",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "software_application",
          "node_name": "行业软件/应用软件",
          "path": [
            "计算机 / 软件服务",
            "行业软件 / 应用软件"
          ]
        }
      ]
    },
    {
      "code": "600570",
      "name": "恒生电子",
      "batch": "P0",
      "primary_industry": "计算机 / 软件服务",
      "primary_leaf": "行业软件/应用软件",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 83.0,
      "fundamental_score": 83.0,
      "net_profit_yi": 1.65,
      "net_profit_yoy_pct": 3.43,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "计算机",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "中",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "computer_software_it",
          "node_name": "计算机/软件服务",
          "path": [
            "计算机 / 软件服务"
          ]
        },
        {
          "name": "金融IT / 行业软件",
          "role": "二级赛道",
          "weight": 0.82,
          "evidence_level": "中",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "software_application",
          "node_name": "行业软件/应用软件",
          "path": [
            "计算机 / 软件服务",
            "行业软件 / 应用软件"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "金融IT / 行业软件",
          "role": "二级赛道",
          "weight": 0.82,
          "evidence_level": "中",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "software_application",
          "node_name": "行业软件/应用软件",
          "path": [
            "计算机 / 软件服务",
            "行业软件 / 应用软件"
          ]
        }
      ]
    },
    {
      "code": "603881",
      "name": "数据港",
      "batch": "P0",
      "primary_industry": "计算机 / 软件服务",
      "primary_leaf": "IT基础设施/云计算",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 96.0,
      "fundamental_score": 89.0,
      "net_profit_yi": 0.45,
      "net_profit_yoy_pct": 0.02,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "计算机",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "computer_software_it",
          "node_name": "计算机/软件服务",
          "path": [
            "计算机 / 软件服务"
          ]
        },
        {
          "name": "IDC / 数据中心",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "it_infrastructure",
          "node_name": "IT基础设施/云计算",
          "path": [
            "计算机 / 软件服务",
            "IT基础设施 / 云计算"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "IDC / 数据中心",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "it_infrastructure",
          "node_name": "IT基础设施/云计算",
          "path": [
            "计算机 / 软件服务",
            "IT基础设施 / 云计算"
          ]
        }
      ]
    },
    {
      "code": "603039",
      "name": "泛微网络",
      "batch": "P0",
      "primary_industry": "计算机 / 软件服务",
      "primary_leaf": "行业软件/应用软件",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 95.0,
      "fundamental_score": 83.0,
      "net_profit_yi": 0.62,
      "net_profit_yoy_pct": 1.38,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "计算机",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "computer_software_it",
          "node_name": "计算机/软件服务",
          "path": [
            "计算机 / 软件服务"
          ]
        },
        {
          "name": "行业软件 / 应用软件",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "software_application",
          "node_name": "行业软件/应用软件",
          "path": [
            "计算机 / 软件服务",
            "行业软件 / 应用软件"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "行业软件 / 应用软件",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "software_application",
          "node_name": "行业软件/应用软件",
          "path": [
            "计算机 / 软件服务",
            "行业软件 / 应用软件"
          ]
        }
      ]
    },
    {
      "code": "000977",
      "name": "浪潮信息",
      "batch": "P0",
      "primary_industry": "计算机 / 软件服务",
      "primary_leaf": "IT基础设施/云计算",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 94.0,
      "fundamental_score": 84.0,
      "net_profit_yi": 6.05,
      "net_profit_yoy_pct": 0.31,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "计算机",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "computer_software_it",
          "node_name": "计算机/软件服务",
          "path": [
            "计算机 / 软件服务"
          ]
        },
        {
          "name": "IT基础设施 / 云计算",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "it_infrastructure",
          "node_name": "IT基础设施/云计算",
          "path": [
            "计算机 / 软件服务",
            "IT基础设施 / 云计算"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "IT基础设施 / 云计算",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "it_infrastructure",
          "node_name": "IT基础设施/云计算",
          "path": [
            "计算机 / 软件服务",
            "IT基础设施 / 云计算"
          ]
        }
      ]
    },
    {
      "code": "002153",
      "name": "石基信息",
      "batch": "P0",
      "primary_industry": "计算机 / 软件服务",
      "primary_leaf": "行业软件/应用软件",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 86.0,
      "fundamental_score": 65.0,
      "net_profit_yi": 0.26,
      "net_profit_yoy_pct": 0.12,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "计算机",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "中",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "computer_software_it",
          "node_name": "计算机/软件服务",
          "path": [
            "计算机 / 软件服务"
          ]
        },
        {
          "name": "IT服务 / 酒店信息化",
          "role": "二级赛道",
          "weight": 0.82,
          "evidence_level": "中",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "software_application",
          "node_name": "行业软件/应用软件",
          "path": [
            "计算机 / 软件服务",
            "行业软件 / 应用软件"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "IT服务 / 酒店信息化",
          "role": "二级赛道",
          "weight": 0.82,
          "evidence_level": "中",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "software_application",
          "node_name": "行业软件/应用软件",
          "path": [
            "计算机 / 软件服务",
            "行业软件 / 应用软件"
          ]
        }
      ]
    },
    {
      "code": "000708",
      "name": "中信特钢",
      "batch": "P0",
      "primary_industry": "资源周期",
      "primary_leaf": "钢铁/建材",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 95.0,
      "fundamental_score": 90.0,
      "net_profit_yi": 15.22,
      "net_profit_yoy_pct": 0.09,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "资源周期",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "resources",
          "node_name": "资源周期",
          "path": [
            "资源周期"
          ]
        },
        {
          "name": "钢铁 / 建材",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "steel_building_materials",
          "node_name": "钢铁/建材",
          "path": [
            "资源周期",
            "钢铁 / 建材"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "钢铁 / 建材",
          "role": "二级赛道",
          "weight": 0.96,
          "evidence_level": "高",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "steel_building_materials",
          "node_name": "钢铁/建材",
          "path": [
            "资源周期",
            "钢铁 / 建材"
          ]
        }
      ]
    },
    {
      "code": "600392",
      "name": "盛和资源",
      "batch": "P0",
      "primary_industry": "资源周期",
      "primary_leaf": "能源资源",
      "mount_count": 2,
      "root_count": 1,
      "leaf_count": 1,
      "severity": "low",
      "diagnosis_label": "收入结构待补 / 合作对象待补 / 公告源待补",
      "issue_types": [
        "revenue_mix_gap",
        "partner_gap",
        "announcement_gap"
      ],
      "diagnosis_reasons": [
        "当前节点结构和轻量证据暂未发现高优先级问题。"
      ],
      "recommended_action": "分类暂可通过，后续补收入结构、公告源、客户/供应商证据。",
      "theme_purity_score": 87.0,
      "fundamental_score": 79.0,
      "net_profit_yi": 3.33,
      "net_profit_yoy_pct": 0.94,
      "improvement": "净利润同比改善",
      "tags": [
        {
          "name": "资源周期",
          "role": "主行业",
          "weight": 1.0,
          "evidence_level": "中",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "resources",
          "node_name": "资源周期",
          "path": [
            "资源周期"
          ]
        },
        {
          "name": "有色 / 稀土资源",
          "role": "二级赛道",
          "weight": 0.82,
          "evidence_level": "中",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "resources_energy",
          "node_name": "能源资源",
          "path": [
            "资源周期",
            "能源资源"
          ]
        }
      ],
      "leaf_tags": [
        {
          "name": "有色 / 稀土资源",
          "role": "二级赛道",
          "weight": 0.82,
          "evidence_level": "中",
          "basis": "P0结构化证据卡：主营/产品/财务字段自动确认",
          "node_id": "resources_energy",
          "node_name": "能源资源",
          "path": [
            "资源周期",
            "能源资源"
          ]
        }
      ]
    }
  ]
};
